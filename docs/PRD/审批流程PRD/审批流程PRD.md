...
第四步调用网关的返回结果分三类（场景未配置或未启用时网关直接报错，不返回以下值）：

```plaintext
PASS：命中"无需审批"条件分支，审批实例直接进入通过状态，由 handler 执行业务动作
PENDING：审批已创建，等待审批人处理，当前不要执行业务动作
EXCEPTION：审批已创建但进入异常处理（分支未命中或审批人为空），当前不要执行业务动作
```

场景未配置或已配置但未启用时，网关直接返回错误"此功能未开放申请"，不创建任何审批实例，不执行业务动作。

所以不是把原逻辑删掉，而是把原逻辑挪到两个地方：

```plaintext
命中无需审批分支时（PASS）：handler 直接执行业务动作
审批节点全部通过后：handler 再执行
```
...
3. 如果审批场景未配置或已配置但未启用，直接提示"此功能未开放申请"，不创建审批实例，不执行业务动作。
...
菜单权限申请在未启用时，前端展示申请入口并提示"当前未开放申请"，与上述行为一致。
