...
class TestUserRow(SQLModel, table=True):
    __tablename__ = "knowhow_script_test_user"
...
async def direct(monkeypatch):
    ...
    monkeypatch.setattr(user_module, "User", TestUserRow)
    ...
    async with connection() as session:
        session.add(TestUserRow(user_id=9, user_name="owner", password="test", delete=0))
        session.add_all(
            [
        ...
        session.add(
            KnowledgeSpaceScope(
                space_id=20, level="public", owner_type="tenant_root_department", owner_id=1, tenant_id=1
            )
        )
        session.add(
            KnowledgeDocument(
    ...
    unit = {
        ...
        "category_code": "JQ",
        "status": "candidate",
        "files": [{"file_id": i, "space_id": 10, "subcategory_code": "JQ01"} for i in [101, 106]],
    }
    checkpoints = []
...
@pytest.mark.parametrize(
    "change", ["missing", "wrong_file", "text", "generation", "busy", "category", "tenant", "deleted"]
)
    ...
    if change == "missing":
        ...
        async with connection() as session:
            ...
            key, value = {
                ...
                "category": ("file_subcategory_code", "JQ02"),
                "tenant": ("tenant_id", 2),
...
async def test_name_conflict_skips_without_modifying_existing_file(direct):
    backend, unit, checkpoint, connection, _, _ = direct
    async with connection() as session:
        session.add(KnowledgeFile(id=300, knowledge_id=20, tenant_id=1, file_name="101.pdf", file_level_path="/200"))
        await session.commit()
    await backend.move_unit(unit, checkpoint)
