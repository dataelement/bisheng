...
async def test_team_space_create_accepts_no_user_group_for_any_user():
    ...
    with (
        ...
        patch.object(
            ...
            new_callable=AsyncMock,
            return_value=[],
        ) as mock_member_groups,
        patch.object(
            UserGroupDao,
            "aget_user_admin_group",
            new_callable=AsyncMock,
            return_value=[],
        ) as mock_admin_groups,
        patch(
            "bisheng.knowledge.domain.services.knowledge_space_service.LLMService.get_workbench_llm",
            new_callable=AsyncMock,
...
async def test_update_file_encoding_bumps_serial_when_duplicate_across_all_spaces(service):
    def _normalize_file_category_code(value):
        ...
        return code if code and len(code) <= 16 else None

    service.normalize_file_category_code = _normalize_file_category_code
    ...
    with (
        ...
        patch(
            ...
            new_callable=AsyncMock,
            return_value=_make_space(space_id=10),
        ),
        patch(
    ...
    assert mock_count.await_args_list[0].args == ("SGGF-RPT-PP-20260600000001",)
    assert mock_count.await_args_list[1].args == ("SGGF-RPT-PP-20260600000002",)
    assert updated.file_encoding == "SGGF-RPT-PP-20260600000002"
...
async def test_update_file_encoding_rejects_unparseable_duplicate(service):
    ...
    file_record.file_encoding = "LEGACY-CODE"
    ...
    with (
        ...
        with pytest.raises(BaseErrorCode) as exc_info:
            await service.update_file_encoding(501, "LEGACY-CODE-2")
...
async def test_move_file_folder_updates_file_and_document_folder(service):
    ...
    with (
        ...
        patch.object(
            ...
            new_callable=AsyncMock,
        ) as mock_sync_document,
        patch.object(
            service,
            "update_folder_update_time",
            new_callable=AsyncMock,
        ) as mock_update_folder_time,
        patch(
...
async def test_shougang_portal_search_emits_permission_performance_log(service):
    ...
    perf_calls = [
        call for call in mock_info.call_args_list if call.args and "[perf][portal.search]" in str(call.args[0])
    ]
    assert perf_calls
...
async def test_shougang_portal_file_search_returns_top_50_without_pagination(service):
    space = _make_space(space_id=12, user_id=7)
    files = [
        _make_file(file_id=2000 + index, knowledge_id=12, file_name=f"语义检索文件{index}.pdf") for index in range(60)
    ]
    file_map = {int(file.id): file for file in files}
    es_chunks = [
        SimpleNamespace(
...
async def test_shougang_portal_semantic_search_filters_candidates_in_batches(service):
    space = _make_space(space_id=12, user_id=7)
    files = [
        _make_file(file_id=3000 + index, knowledge_id=12, file_name=f"候选文件{index}.pdf") for index in range(120)
    ]
    file_map = {int(file.id): file for file in files}
    es_chunks = [
        SimpleNamespace(
            ...
            return_value=[
                {
                    **files[0].model_dump(),
                    "tags": [],
                    "user_metadata": {"shougang_portal_publish": {"source_space_id": 14, "source_file_id": 42}},
                },
                {
                    **files[1].model_dump(),
                    "tags": [],
                    "user_metadata": {"shougang_portal_publish": {"source_space_id": 14, "source_file_id": 43}},
                },
            ],
        ),
    ):
...
async def test_shougang_portal_file_search_uses_document_folder_path_when_file_is_root(service):
    ...
    service.doc_repo = SimpleNamespace(
        find_by_ids=AsyncMock(return_value=[SimpleNamespace(id=9001, knowledge_id=109, file_level_path="/37", level=1)])
    )
...
async def test_shougang_portal_source_folder_paths_resolves_breadcrumb(service):
    ...
    folders = [
        _make_file(file_id=37, knowledge_id=14, file_type=FileType.DIR.value, file_name="C011", file_level_path=""),
        _make_file(file_id=38, knowledge_id=14, file_type=FileType.DIR.value, file_name="C0001", file_level_path="/37"),
    ]
...
class TestCreateSpace:
    ...
    @pytest.mark.asyncio
    async def test_create_does_not_check_user_space_count(self, service):
        with (
            patch(
                "bisheng.knowledge.domain.services.knowledge_space_service.KnowledgeDao.async_count_spaces_by_user",
                new_callable=AsyncMock,
                side_effect=AssertionError("knowledge-space count must not be read"),
            ) as mock_count,
            patch(
                "bisheng.knowledge.domain.services.knowledge_space_service.LLMService.get_workbench_llm",
                new_callable=AsyncMock,
...
class TestDeleteSpace:
    ...
    async def test_uses_can_delete_permission_instead_of_creator_id(self, service):
        ...
        with (
            ...
            patch.object(
                service,
                "_send_space_event_notification",
                new_callable=AsyncMock,
            ),
            patch(
...
class TestSpaceListings:
    ...
    async def test_get_grouped_spaces_hides_other_users_favorite_space(self, service):
        ...
        with (
            ...
            patch(
                "bisheng.knowledge.domain.services.knowledge_space_service.KnowledgeDao.async_get_spaces_by_ids",
                new_callable=AsyncMock,
                return_value=[my_fav, other_fav, my_normal],
            ),
            patch(
                "bisheng.knowledge.domain.services.knowledge_space_service.PermissionService.get_permission_levels",
                new_callable=AsyncMock,
    ...
    async def test_format_accessible_spaces_excludes_membership_without_required_permission_id(self, service):
        ...
        mock_permission_ids.assert_awaited_once_with(
            ...
            2,
            tuple_cache={},
            precomputed_permission_level=None,
        )
...
class TestTupleLifecycle:
    ...
    async def test_get_file_preview_uses_knowledge_file_can_read(self, service):
        ...
        assert ("knowledge_file", 96) in [(call.args[0], call.args[1]) for call in mock_effective.await_args_list]
    ...
    async def test_retry_space_files_relies_on_edit_permission_only(self, service):
        ...
        with (
            ...
            patch.object(
                ...
                new_callable=AsyncMock,
            ) as mock_require_resource_permission,
            patch(
                "bisheng.knowledge.domain.services.knowledge_space_service.KnowledgeDao.async_update_knowledge_update_time_by_id",
                new_callable=AsyncMock,
            ),
...
def test_hide_restricted_status_items_member_view():
    ...
    items = [
        ...
        _make_file(
            file_id=7, user_id=99, status=KnowledgeFileStatus.VIOLATION.value, file_type=FileType.DIR.value
        ),  # 文件夹 -> 保留
    ]
...
async def test_filter_visible_child_items_hides_restricted_for_member(service):
    ...
    with patch.object(
        ...
        kept = await service._filter_visible_child_items(
            items,
            space_id=1,
            context={"can_view_all_statuses": False},
        )
...
async def test_filter_visible_child_items_manager_sees_all(service):
    ...
    with patch.object(
        ...
        kept = await service._filter_visible_child_items(
            items,
            space_id=1,
            context={"can_view_all_statuses": True},
        )
