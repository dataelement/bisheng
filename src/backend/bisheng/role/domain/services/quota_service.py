...
DEFAULT_ROLE_QUOTA: dict[str, int] = {
    ...
    "knowledge_space_download_daily": 20,  # per-user daily watermark PDF downloads
    "channel": 10,
...
class QuotaResourceType:
    ...
    KNOWLEDGE_SPACE_DOWNLOAD_DAILY = "knowledge_space_download_daily"
    CHANNEL = "channel"
...
# Daily counters live in Redis — not SQL resource inventory counts.
NON_INVENTORY_RESOURCE_TYPES = frozenset({QuotaResourceType.KNOWLEDGE_SPACE_DOWNLOAD_DAILY})
...
class QuotaService:
    ...
    async def get_all_effective_quotas(
        ...
        # Batch all resource count queries concurrently (skip Redis-only counters)
        resource_types = list(DEFAULT_ROLE_QUOTA.keys())
        inventory_types = [rt for rt in resource_types if rt not in NON_INVENTORY_RESOURCE_TYPES]
        tenant_counts, user_counts = await asyncio.gather(
            asyncio.gather(*(cls.get_tenant_resource_count(tenant_id, rt) for rt in inventory_types)),
            asyncio.gather(*(cls.get_user_resource_count(user_id, rt) for rt in inventory_types)),
        )
        inventory_index = {rt: i for i, rt in enumerate(inventory_types)}

        items = []
        for resource_type in resource_types:
            role_q = role_quotas.get(resource_type, DEFAULT_ROLE_QUOTA.get(resource_type, -1))
            if resource_type in NON_INVENTORY_RESOURCE_TYPES:
                tenant_used = 0
                user_used = 0
            else:
                idx = inventory_index[resource_type]
                tenant_used = tenant_counts[idx]
                user_used = user_counts[idx]
    ...
    @classmethod
    async def get_knowledge_space_download_daily_limit(cls, login_user) -> int:
        """Per-user daily watermark-PDF download cap from role.quota_config (max merge).

        Returns ``-1`` for unlimited. Admin / tenant-admin exemption is enforced
        by the download service, not here.
        """
        default = int(DEFAULT_ROLE_QUOTA["knowledge_space_download_daily"])
        if not login_user:
            return default
        user_roles = await UserRoleDao.aget_user_roles(login_user.user_id)
        if not user_roles:
            return default
        roles = await RoleDao.aget_role_by_ids([r.role_id for r in user_roles])
        quotas = cls._compute_role_quotas(roles)
        return int(quotas.get("knowledge_space_download_daily", default))
