...
class TestPermissionApiIntegration:
    ...
    def test_grant_subject_departments_helper_omits_member_count(self):
        # F038: per-department member_count was removed from this helper. It must
        # NOT issue the `COUNT(*) ... WHERE department_id IN (<all subtree ids>)
        # GROUP BY` query — that giant `.in_(...)` serializes catastrophically on
        # DM8's fake-async driver (~66s for 50k departments) and the grant-subject
        # picker never renders the count. The returned nodes must not carry a
        # member_count key, and only the 3 structural queries (tenant, root dept,
        # department subtree) may run.
        ...
        class FakeSession:
            ...
            async def exec(self, _statement):
                ...
                raise AssertionError("unexpected 4th query: the member_count COUNT/GROUP BY must be gone")
        ...
        # Only the 3 structural queries ran — no per-department member_count count query.
        assert FakeSession.calls == 3
        # Contract: nodes no longer carry member_count.
        assert "member_count" not in tree[0]
        assert "member_count" not in tree[0]["children"][0]
        # Tree shape preserved.
        assert tree[0]["name"] == "默认组织"
