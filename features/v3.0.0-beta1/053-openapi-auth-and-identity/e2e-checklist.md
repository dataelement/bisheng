...
- [ ] 列表可滚动加载并显示有效密钥数、资源归属人、最后调用、创建人/时间；零密钥、长期未调用和归属人失效分别有就地提示。
...
## Platform：AC-R14 管理列表滚动加载（2026-09-16）

本地 Chrome 使用实际页面组件、与系统标签页一致的限高容器和模拟分页响应：三个列表均完成 55 条数据的滚动追加，观察到请求页码 `1 → 2（失败）→ 2（重试）→ 3`，验证已加载行保留、失败不自动重试、加载完停止请求、空/短列表、首屏失败重试、后续页有效密钥对应的全部吊销按钮及搜索竞态，浏览器无运行时异常。以下复选项保留为真实部署联调清单，不将模拟测试视为真实中间件 E2E。

- [ ] 在真实系统管理标签页分别准备超过 20 条服务账号、同一服务账号 API 密钥、个人 PAT 台账记录；确认页面内部可滚动，内容不被面板裁切。
- [ ] 滚动到底时在已有行之后追加下一页，尾部显示“正在加载…”；反复触底不重复请求同一页。
- [ ] 断网或模拟下一页失败，已有行不消失；尾部显示“加载失败，点击重试”，不自动循环重试；恢复后点击或键盘 Enter 重试同一页。
- [ ] 全部记录加载后显示“已展示全部内容”，继续滚动不再请求；不足一屏也显示结束文案；零条显示“暂无记录”。
- [ ] 首屏请求失败时提供重试，不误显示空态；快速搜索与切换账号时旧响应不得覆盖新结果。
- [ ] 签发、编辑、吊销后刷新列表；首屏密钥全部失效、后续页仍有有效密钥时，“全部吊销”仍可用。
- [ ] 密钥的委托范围保持固定列宽、超长省略与悬停完整提示；资源授权页使用原全量密钥接口仍正常。

**自动化覆盖**：`src/frontend/platform/src/test/openApiInfiniteList.test.tsx` 覆盖并发防重、追加去重、同页重试、搜索/账号旧响应隔离、操作刷新时旧追加请求失败、分页无进展保护、滚动父容器与键盘重试；现有管理页回归合计 25 项通过。`src/backend/test/open_api/test_key_pagination.py` 和租户边界/凭据回归合计 19 项通过（SQLite + ASGI），覆盖数据库分页、全量有效数、掩码响应、参数范围、管理员准入与跨租户拒绝。

**真实部署入口**：`F053_E2E=1 ... pytest test/e2e/test_e2e_f053_openapi_auth_identity.py -k ac_r14_key_pagination -v`。复用专用服务账号 fixture 和前后清理，仅对本测试新签发的密钥执行吊销；核对分页与原全量接口一致。当前未提供专用部署参数，整套 12 项 live E2E 已按环境开关跳过，不计为通过。
