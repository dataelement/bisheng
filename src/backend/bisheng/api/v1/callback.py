...
class AsyncStreamingLLMCallbackHandler(AsyncCallbackHandler):
    ...
    async def on_llm_new_token(self, token: str, **kwargs: Any) -> None:
        ...
        if token is None:
            token = ""
        resp = ChatResponse(
            message={"content": token, "reasoning_content": reasoning_content},
            type="stream",
    ...
    async def on_tool_start(self, serialized: dict[str, Any], input_str: str, **kwargs: Any) -> Any:
        ...
        logger.info(
            "k=s act=on_tool_start flow_id={} tool_name={} input_str={}",
            self.flow_id,
            serialized.get("name"),
            input_str,
        )
        ...
        resp = ChatResponse(
            type="stream", intermediate_steps=f"Tool input: {input_str}", flow_id=self.flow_id, chat_id=self.chat_id
        )
        await self.websocket.send_json(resp.dict())
    ...
    async def on_tool_end(self, output: str, **kwargs: Any) -> Any:
        ...
        resp = ChatResponse(
            type="stream", intermediate_steps=intermediate_steps, flow_id=self.flow_id, chat_id=self.chat_id
        )
    ...
    async def on_text(self, text: str, **kwargs: Any) -> Any:
        ...
        if kwargs.get("sender"):
            log = ChatResponse(
                message=text, type="end", sender=sender, receiver=receiver, flow_id=self.flow_id, chat_id=self.chat_id
            )
            start = ChatResponse(
                type="start", sender=sender, receiver=receiver, flow_id=self.flow_id, chat_id=self.chat_id
            )
            ...
            if "autogen" == kwargs["category"]:
                log = ChatResponse(message=text, type="stream", flow_id=self.flow_id, chat_id=self.chat_id)
                await self.websocket.send_json(log.dict())
                if kwargs.get("type"):
                    # Under compatibility
                    start = ChatResponse(
                        type="start", category=kwargs.get("type"), flow_id=self.flow_id, chat_id=self.chat_id
                    )
                    end = ChatResponse(
                        type="end",
    ...
    async def on_agent_action(self, action: AgentAction, **kwargs: Any):
        ...
        log = log.replace("\n", "\n\n")
        resp = ChatResponse(type="stream", intermediate_steps=log, flow_id=self.flow_id, chat_id=self.chat_id)
        await self.websocket.send_json(resp.dict())
    ...
    async def on_agent_finish(self, finish: AgentFinish, **kwargs: Any) -> Any:
        ...
        resp = ChatResponse(flow_id=self.flow_id, chat_id=self.chat_id, type="stream", intermediate_steps=finish.log)
        await self.websocket.send_json(resp.dict())
    ...
    async def on_retriever_start(self, serialized: dict[str, Any], query: str, **kwargs: Any) -> Any:
        ...
        logger.info(
            "k=s act=on_retriever_start flow_id={} query={} meta={}", self.flow_id, query, serialized.get("repr")
        )
    ...
    async def on_retriever_end(self, result: list[Document], **kwargs: Any) -> Any:
        ...
        if result:
            ...
            logger.info("k=s act=on_retriever_end flow_id={} result_without_bbox={}", self.flow_id, tmp_result)
...
class StreamingLLMCallbackHandler(BaseCallbackHandler):
    ...
    def on_llm_new_token(self, token: str, **kwargs: Any) -> None:
        ...
        if token is None:
            return
        resp = ChatResponse(message=token, type="stream", flow_id=self.flow_id, chat_id=self.chat_id)
        if self.websocket:
    ...
    def on_agent_finish(self, finish: AgentFinish, **kwargs: Any) -> Any:
        """Run on agent end."""
        resp = ChatResponse(type="stream", intermediate_steps=finish.log, flow_id=self.flow_id, chat_id=self.chat_id)
        if self.websocket:
            loop = asyncio.get_event_loop()
            coroutine = self.websocket.send_json(resp.dict())
            asyncio.run_coroutine_threadsafe(coroutine, loop)
...
def _dump_run_log(payload: dict) -> str:
    """Serialize a run-log frame so an exotic tool result cannot suppress it.

    A tool is free to answer with anything — the knowledge retriever returns
    ``Document`` objects. Plain ``json.dumps`` raised on those, the end frame was
    never sent, and the chat card spun forever with no way to recover.
    """
...
class AsyncGptsDebugCallbackHandler(AsyncGptsLLMCallbackHandler):
    ...
    def parse_tool_category(tool_name) -> (str, str):
        ...
        tool_category = "tool"
        if not tool_name:
            # `on_tool_end` reads the name out of kwargs, which is not always
            # populated; without this the end frame would never be sent and the
            # chat card would spin forever.
    ...
    async def on_chat_model_start(
        ...
        # await self.websocket.send_json(stream.dict())
        logger.debug(f"on_chat_model_start serialized={serialized} messages={messages} kwargs={kwargs}")
        resp = ChatResponse(type="start", category="processing", flow_id=self.flow_id, chat_id=self.chat_id)
        await self.websocket.send_json(resp.dict())

    async def on_llm_end(self, response: LLMResult, **kwargs: Any) -> Any:
        """Run when LLM ends running."""
        logger.debug(f"llm_end response={response}")
        resp = ChatResponse(type="end", category="processing", flow_id=self.flow_id, chat_id=self.chat_id)
        await self.websocket.send_json(resp.dict())
    ...
    async def on_tool_end(self, output: ToolMessage, **kwargs: Any) -> Any:
        ...
        resp = ChatResponse(
            ...
            message=_dump_run_log(output_info),
            flow_id=self.flow_id,
            chat_id=self.chat_id,
            extra=json.dumps({"run_id": kwargs.get("run_id").hex}),
        )
