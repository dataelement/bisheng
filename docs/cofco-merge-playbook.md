...
> **先读 [`release-trains.md`](release-trains.md) 再动手。** 那份讲哪条分支是本期发版、
> 哪条是下一批 —— 分支名的 `-test` 后缀读起来是反的,2026-09-15 就因为读反而把下一批的
> 开放 API 和授权模型推进了正在发版的客户包。合并方向对了,这份手册才用得上。
>
...
### 3.20 后端 · 文件夹授权修复与已有定制代码相邻

**2026-09-14 实例**：主线 `a180a91eb` 合入 909，共同祖先为 `f7f0017fa`。
主线修正指定文件夹列表/搜索的容器鉴权，同时经过 Ruff 格式化；909 保留
批量统计常量与可见性判定注释，因此出现两处相邻文本冲突：

- `_FOLDER_COUNT_UNION_CHUNK_SIZE` 与 `_WEB_LINK_SEPARATORS`：保留批量统计常量，
  同时保留中文分隔符的说明与 `RUF001` 精确豁免。
- 广场 `visible_map.get(...)` 条件：保留 909 的布尔返回值说明和格式化后的条件。

**处理方式**：按三方版本逐块合并，保留主线 `_require_container_read_permission`
及其列表/搜索调用，保留 909 的文件变更过滤、租户守卫和文件夹批量统计。
不要整文件取主线或共同祖先。没有冲突标记也不等于已解决：检查 Git 索引中的
未合并状态，并确认工作文件没有意外回到共同祖先后，才标记为已解决。

主线新增的 `test_knowledge_space_container_access.py` 按 §3.14 补齐租户上下文和
文件变更隐藏集合；额外验证继承可见的文件仍会被中粮隐藏规则过滤。
这项适配只留在 909，不回流要求主线具备中粮专用方法。
