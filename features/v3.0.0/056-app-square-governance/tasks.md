...
- [x] **T024**: 审计列表列扩展（对象列显示应用名 + 标识 / 名称快照；操作人列对服务账号显示账号名 + 密钥掩码并附 owner）
  ...
  **覆盖 AC**: AC-17, AC-29
  **证据**：后端 `AuditLogService._enrich_audit_rows` 只吐派生字段（`app_id/app_name/app_slug/app_state/app_owner_name/version_no/tenant_name/operator_kind/operator_key_mask`），**不吐原始 `metadata`**（AC-26）；服务账号识别三种形态（写入方今天实际落的 `metadata.actor_kind='service_account'`〔`OpenApiAuditMiddleware` / `app.release.submit`〕、`metadata.operator.kind='service_account'`、或 beta2 约定 `operator_id=0` + 有名字）。密钥掩码：优先转发写入方已产出的 `metadata.operator.key_mask`；否则按写入方落的 `metadata.credential_id` 批量读 `api_credential` 行，用其**已存**的 `key_prefix + last4` 拼掩码（`CredentialRepository.get_by_ids`，绕过租户过滤；明文从未落库，无更长值可泄）——审查时补，此前掩码字段没有任何写入方会填、永远为空。前端渲染规则集中在 `systemLog/auditRow.ts`（表格与 CSV 共用），三语 key 在 `bs.json` `log.*`。测试 `platform/src/test/auditRowRendering.test.ts`、`test/audit/test_audit_export.py::TestExportContent::test_mask_resolved_from_credential_id_for_writer_shape`。
  **附带**：`platform/src/test/auditSingleTenantFilter.test.ts` 原硬编码 `app.release.*` 数量 16（家族已增至 17）导致基线常红、把 i18n 齐全性检查挡在后面；改为与 `V2_ACTIONS` 登记对齐，现通过（AC-27 的三语齐全性由该用例实际验证）。
...
## 2026-09-18 · 验收核查发现：三类高频记录「只写不读」（未修，待拍板）

对 PRD-1 二十一项功能逐项核查实现侧接线时发现：**GOV-04 事件清单里有三类记录写入齐备、查询侧零接线**，管理面一条也查不到。按 §2.4 与 spec §1 的归属规则（「本 Feature 拥有…访问记录、运行期能力调用双归属、模型调用逐条…在审计面可查的验收」），这是本 Feature 的缺口，不是上游没写。

| 记录 | 写入侧 | 查询侧现状 | 114 实测 |
|---|---|---|---|
| 模型调用 | `model_call_record` 表，字段齐全（`credential_id` / `app_id` / 模型 / token 三项） | `ModelCallRecordRepository.alist` / `aiter_export` **存在但零调用方**（全仓只有测试引用），无 API 端点、无前端入口 | 1 行（09-16 手验那次） |
| 运行期能力调用（检索） | `AppCapabilityCallRecord`，表结构完整（`app_id` / `capability` / `targets` / `result` / `chunk_count` / `latency_ms`） | `AppCapabilityCallRecordDao` **只有 `ainsert`**，连查询方法都没有 | 0 行 |
| 访问记录 | `AppAccessLogDao.ainsert`（fire-and-forget，有测试） | `AppAccessLogDao.alist_page` **零调用方**（同上） | 52 行 |

**为什么不是「已顺延」**：§5.2 顺延的是**聚合用量账单**，且顺延理由写的是「本版以**逐条调用审计**替代，管理员**仍可追溯**谁在用」；另一条顺延的是 **owner 自助查询**（「本版审计查询仅管理面」）。两条都把「管理面逐条可查」留在了本版。PRD-1 GOV-04 验收要点 1（「上述每类事件发生后在审计面可查到记录」）与验收要点 3（「模型调用可按 key / 按应用追溯到具体调用记录，含模型与 token 数」）因此**当前不可达**。

**为什么会漏**：写入侧与查询侧各自有测试且各自都绿——`test_app_access_log.py` 证明写得进去，审计页测试证明白名单里的 action 筛得出来，但没有一条测试问「这张表能不能被人读到」。与 T015 当年发现的「后端白名单 17 条、前端 0 条」是同一种失效的另一面。

**不是随手能补的原因**（所以留给用户拍板，没有自行实现）：三类都是高频写入，`model_call_record` 的模块注释已写明它**刻意不进 `audit_log`**（「a local coding agent calls it in a loop，混进去会淹没审计页存在的意义」），`open_api.call` 出于同样理由没进 `_UI_VISIBLE_V2_ACTIONS`。因此「把 action 加进白名单」是最省但最坏的一种解法——它正好毁掉当初分表的理由。可选形态：

- **A｜审计页加一个独立 tab/子查询面**（推荐）：默认视图不变，按 key / 应用 / 时间查这三张表，复用既有导出。工作量集中在一个端点族 + 一个前端页。
- **B｜只补端点、先不做前端**：让 AC「可追溯」在 API 层成立，管理员经导出拿数。最小，但 §2.4「在既有审计页上扩展」这句仍不成立。
- **C｜判定为本版不做**：需要同步改 PRD-1 GOV-04 验收要点 1 与 3、§5.2 补一行顺延，并把 F056 spec §1 归属清单里这三项划掉——否则下一轮验收还会再发现一次。

**一个附带项**（AC-58 修复时浮现）：应用运行期凭据的审计行（本轮新加，`open_api.api_key.issue` / `.revoke`）target 是凭据、`metadata` 里只有 `subject_id`，**没有 `app_id`**，因此按「对象应用」筛不出来。如果选 A 或 B，这一条顺带补进去（additive，改 `CredentialService._audit` 的 metadata 即可）。
