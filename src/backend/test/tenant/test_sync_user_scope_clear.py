...
This is a wiring test — service internals (permission relations, audit payload) are
...
def _patch_deps(monkeypatch, new_leaf_id: int, current_leaf_id=None):
    ...
    current = None if current_leaf_id is None else SimpleNamespace(tenant_id=current_leaf_id)
    monkeypatch.setattr(
        mod.UserTenantDao,
    ...
    permissions = MagicMock(apply_changes=AsyncMock())
    monkeypatch.setattr(mod, "get_permission_relation_api", AsyncMock(return_value=permissions))
    monkeypatch.setattr(
        mod.AuditLogDao,
        "ainsert_v2",
        AsyncMock(),
