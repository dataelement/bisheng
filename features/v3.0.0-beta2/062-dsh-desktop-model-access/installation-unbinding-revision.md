...
日期：2026-09-11。状态：用户已确认需求边界；本文为目标设计，尚未修改运行代码或环境。
...
| Redis | DSH 统一使用固定模块前缀，例如 `{dsh}:...`；票据、Nonce、授权事务、席位激活及额度状态不得使用 License ID、指纹或安装标识分区 |
...
更换 License 只更新能力、有效期和上限。授权 jti/digest 可保留用于状态展示、审计和副本加载版本核对，不能作为业务数据归属、查询过滤条件或 Redis 主键前缀。降配超限继续按现设计阻断并由管理员处理，不自动释放席位。
...
Gateway 所有 `@ConditionalOnProperty(... installation-id)` 装配条件改为已有 dsh.enabled 部署条件；业务开关与部署开关职责保持现状，不引入新开关。两端需要同步更新，不能混跑旧内部协议。现有副本版本协调机制仍须工作；本次不悄然重写激活/恢复流程或新增自动扩缩容协议。
...
4. 直接更新建表脚本及目标表结构、索引；保持 seat_id、user_id、模型额度、历史用量及审计。Redis 账本、投影位置、去重/幂等状态迁移并核对数量和金额无关的 Token 合计，不将已有用量归零。
5. 清退旧 access/refresh 对应会话及未完成授权事务，保留会话历史和席位；新登录可使用已分配席位。本次不兼容旧测试凭证、旧 HMAC 或旧 DSH License。
6. 生成 schema 2 测试 License，配套切换 Gateway 与毕昇 API/Worker；按现有机制重新核对授权版本与 Redis 恢复证据，再恢复准入。毕昇部署按用户安排执行。
