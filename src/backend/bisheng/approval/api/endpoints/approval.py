...
@router.get('/department-knowledge-space/settings')
async def get_department_knowledge_space_approval_settings(
    login_user: UserPayload = Depends(UserPayload.get_login_user),
):
    settings = await ApprovalService.get_department_knowledge_space_settings()
    return resp_200(settings.model_dump())


@router.put('/department-knowledge-space/settings')
