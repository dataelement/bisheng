...
@pytest.mark.asyncio
async def test_success_outbox_skips_handler_setup_and_cannot_be_downgraded(monkeypatch: pytest.MonkeyPatch):
    outbox = _outbox()
    outbox.status = ApprovalOutboxStatus.SUCCESS
    build_handler = AsyncMock(side_effect=RuntimeError("handler setup failed"))
    finalize = AsyncMock()
    monkeypatch.setattr(ApprovalInstanceRepository, "get_outbox", AsyncMock(return_value=outbox))
    ...
    assert await _execute_approval_outbox_async(1) is True
    finalize.assert_awaited_once_with(outbox_id=1)
