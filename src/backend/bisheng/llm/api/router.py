...
@router.post("/verify")
async def verify_model_status(
    request: Request,
    login_user: UserPayload = Depends(UserPayload.get_tenant_admin_user),
    model_id: int = Body(..., embed=True, description="Model UniqueID"),
):
    """Probe the model once and return its refreshed status.

    A failing probe is a business outcome (status becomes abnormal), not an
    API error -- the response stays 200 and carries the reason.
    """
    ret = await LLMService.verify_model_status(model_id, login_user)
    return resp_200(data=ret)
