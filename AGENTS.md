...
>
> **合并任何分支进来之前,先读 [`docs/cofco-merge-playbook.md`](docs/cofco-merge-playbook.md)** —— 合并顺序、每次必冲突的点与处理原则、合并后检查清单(有几类问题会静默通过 lint 和 CI)。**每次合并结束都要更新手册**:只记每次都会冲突的点,一次性冲突不记。
>
