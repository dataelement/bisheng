...
| 2 | 「移动后子树最深 ≤ 10 层」**只数文件夹、且 level 是 0-based**（UI 第1层 = level 0 ⇒ 最深文件夹 level 9）：目标 level + 子树内最深**文件夹**相对深度 > 9 即拒；**文件不算一层**——文件移入第 10 层文件夹合法、文件夹（含空文件夹）移入第 10 层必须拒。2026-06-11 修复过 off-by-one（`>10` 放过了空文件夹）+ 误把文件计入子树深度 | 空文件夹能移进第 10 层（建出第 11 层）；或最深层带文件的合法移动被误拒 | move_items 子树 max(level) 查询加 `file_type=DIR` 过滤,阈值 9;upload_folder_items 同口径 |
