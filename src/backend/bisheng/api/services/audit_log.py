    ...
    def review_one_session(cls, chat_id: str, all_message: bool = False):
        ...
        if update_violations_messages:
            for one in update_violations_messages:
                ChatMessageDao.update_review_status([one['id']], ReviewStatus.VIOLATIONS.value, json.dumps(one['reason']))
        if update_failed_messages:
            for one in update_failed_messages:
                ChatMessageDao.update_review_status([one['id']], ReviewStatus.FAILED.value, json.dumps(one['reason']))
