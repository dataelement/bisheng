...
class OrgSyncService:
    ...
    async def execute_sync(
        ...
        try:
            ...
            if log:
                ...
                if errors:
                    final_status = "partial" if any(stats[k] > 0 for k in stats) else "failed"
    ...
    async def _move_dept(
        ...
        await aassert_default_root_parent_immutable(op.local.id, new_parent_id)
    ...
    async def _archive_dept(cls, op: ArchiveDept) -> None:
        ...
        try:
            ...
            logger.warning(
                f"DepartmentDeletionHandler.on_deleted failed for dept {op.local.id}: {exc}",
            )
        if op.local.id is not None:
            await DepartmentArchiveCleanupService.arun_for_archived_department(
                int(op.local.id),
                reason="org_sync_archive",
            )
    ...
    async def _ensure_default_user_group_membership(cls, user_id: int) -> list[int]:
        ...
        try:
            ...
            existing_ids = {int(row[0]) if isinstance(row, (tuple, list)) else int(row) for row in existing}
            ...
            logger.warning(
                f"Failed to attach synced user {user_id} to default group: {exc}",
            )
            return []
    ...
    @classmethod
    async def _acquire_redis_lock(cls, config_id: int) -> str | None:
