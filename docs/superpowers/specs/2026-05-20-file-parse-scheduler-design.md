...
### 4.4 直接 dispatch 路径的 OCR 路由

当 `ocr_queue_enabled = true` 且 `fair_scheduler_enabled = false` 时，文件不经过虚拟队列，直接在上传时 dispatch。此时 `knowledge_service.py` 中的 dispatch 入口需要同样做路由判断：

```python
# knowledge_service.py — aprocess_knowledge_file / process_knowledge_file
for index, one in enumerate(process_files):
    queue = decide_queue(one.file_name)   # 新增：判断 OCR / non-OCR
    file_worker.parse_knowledge_file_celery.apply_async(
        args=[one.id, preview_cache_keys[index], req_data.callback_url],
        queue=queue
    )
```

`decide_queue` 封装在 `scheduler.py` 中，`ocr_queue_enabled=false` 时返回 `knowledge_celery`。
...
| `{bisheng_fs}:payload:{file_id}` | Hash | `preview_cache_key`, `callback_url`, `user_id`, `file_ext` | enqueue → dispatch成功（4h TTL兜底） |
| `{bisheng_fs}:inflight:{user_id}` | Set | `file_id`（字符串）| dispatch → complete |
| `{bisheng_fs}:active_users` | Set | `user_id`（字符串）| enqueue → queue清空 |
| `{bisheng_fs}:inflight_users` | Set | `user_id`（字符串）| dispatch → inflight归零 |
| `{bisheng_fs}:dispatch_lock` | String（NX EX）| `{worker_id}` | 调度期间持有 |

`inflight_users` 专供对账任务枚举"有在途文件的用户"，避免在 Redis Cluster 中 SCAN 跨节点查询 `inflight:*` key。
...
-- 所有 key 使用 {bisheng_fs} hash tag，确保落在同一 Redis slot（Cluster 兼容）
...
redis.call('HSET',  prefix .. 'payload:' .. ARGV[1],
    ...
    'user_id',           user_id,
    'file_ext',          ARGV[4])
redis.call('EXPIRE', prefix .. 'payload:' .. ARGV[1], 14400)  -- 4h TTL，防止孤儿 key
redis.call('SADD',  prefix .. 'active_users', user_id)
redis.call('SADD',  prefix .. 'inflight_users', user_id)  -- 供对账任务枚举（见 5.7）
return 1
```
...
-- 放回队列尾部（RPUSH = 放到右端，下次 RPOP 优先取出，快速重试）
...
redis.call('SREM', prefix .. 'inflight:' .. user_id, file_id)
-- 若 inflight 已清空，从 inflight_users 移除（对账任务枚举用）
...
`trigger_dispatch_task` 是一个轻量 Celery 任务，统一执行一轮完整的 `run_dispatch_round()`（轮询所有 active_users）：

1. **上传触发**：`enqueue_file` Lua 执行成功后，立即调用 `trigger_dispatch_task.delay()`。新文件入队后立刻触发全局一轮调度，所有有待处理文件的用户都能被及时调度，而不是只处理刚上传的用户。
2. **完成触发**：Celery 任务 `finally` 块中，`complete_file` Lua 执行后，调用 `trigger_dispatch_task.delay()`。某文件完成后立即补充下一批任务，保持 worker 满载。

两种触发均执行相同的 `run_dispatch_round()`，通过分布式锁保证不会并发，逻辑统一。
...
**user_id 归属**：重试任务只收到 `file_id`，入队时用 `KnowledgeFile.user_id`（原上传者）作为调度 key。这意味着无论谁触发重试（用户本人或管理员），并发配额均计入原上传者。这是有意为之：公平调度的语义是"同一个上传者的文件不会霸占所有 worker"，与谁触发重试无关。
...
function reconcile():
    ...
    for user_id in SMEMBERS('{bisheng_fs}:inflight_users'):
...
knowledge_file_worker:
  ...
  fair_scheduler:
    ...
    # Redis key 前缀固定为 {bisheng_fs}:，不可配置（保证 Cluster slot 一致性）
