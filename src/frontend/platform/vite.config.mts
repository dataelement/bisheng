...
function buildProxyTargets(mode: string) {
  // .env.local 不会在进入本文件时写入 process.env，需用 loadEnv（cwd 为 platform 目录）
  const env = loadEnv(mode, process.cwd(), "VITE_");
  const target =
    env.VITE_PROXY_TARGET || process.env.VITE_PROXY_TARGET || "http://127.0.0.1:7860";
  const fileServiceTarget =
...
export default defineConfig(({ mode }) => {
  const proxyTargets = buildProxyTargets(mode);
  return {
