    ...
    def _run(self, unique_id: str) -> Dict[str, Any]:
        if self._user_info is None and self.user_id:
            ...
            if user_group:
                ...
                user_groups = [one.group_name for one in groups]
            user_role = UserRoleDao.get_user_roles(int(self.user_id))
            user_roles = []
            if user_role:
                roles = RoleDao.get_role_by_ids([role.role_id for role in user_role])
                for one in roles:
                    user_roles.append(f"{one.role_name}")
            user_depatment = ""
            ud = UserDepartmentDao.get_user_primary_department(self.user_id)
            if ud:
                dept = DepartmentDao.get_by_id(ud.department_id)
                if dept:
                    user_depatment = dept.name
            if user_db:
                self._user_info = json.dumps({
                    "user_name": user_db.user_name,
                    "user_group": user_groups,
                    "department": user_depatment,
