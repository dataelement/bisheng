...
4. 步骤说明 - 提供清晰的步骤指导，最后一个步骤一定要是交付与汇报，在这个步骤里明确描述出交付物是什么。
   4.1 由于每个步骤是不同的Agent来进行处理，不同的Agent之间可传递的信息不宜过多，所以阅读原文和分析必须包含在一个步骤中。
   4.2 因此拆分任务的需求，需要先用一个步骤列出所有需要分步处理内容（例如生成目录，或生成文件名），然后下一个步骤针对此步骤进行处理，所有的具体处理应该都放在一个步骤里面，如果需要写入文件，需要在此步骤说明是每判断一个追加写入文件。
5. 如果用户意图需要输出很长的内容，推荐使用写文档的工具，写文档的工具需要先拆解需要写的目录结构，然后给定一个文件名，然后一步一步的完成，每一步都写入同一个文件。
6. 你需要先输出思考过程，说清楚你生成SOP的逻辑，然后再输出SOP。思考和SOP之间必须使用<SOP_START>来进行分割。
7. 使用@来说明使用什么工具，文件以及知识库，必须遵循以下格式：
工具的表示方法：@{{工具名}}@
文件的表示方法：@{{文件名}}的文件储存信息：{{'文件储存在语义检索库中的id':'{{id}}','文件储存地址':'{{文件路径}}'}}@
知识库的表示方法：@{{知识库名}}的储存信息:{{'知识库储存在语义检索库中的id':'{{id}}'}}@
...
你需要先思考，然后再输出SOP。使用<SOP_START>来分割思考和SOP。
思考过程："""
...
请基于以上内容和反馈创建一个清晰、结构化，且更好满足用户需求的SOP文档：
需要包含以下内容：
...
3. 步骤说明 - 提供清晰的步骤指导，最后一个步骤一定要是交付与汇报，在这个步骤里明确描述出交付物是什么。
   3.1 由于每个步骤是不同的Agent来进行处理，不同的Agent之间可传递的信息不宜过多，所以阅读原文和分析必须包含在一个步骤中。
   3.2 因此拆分任务的需求，需要先用一个步骤列出所有需要分步处理内容（例如生成目录，或生成文件名），然后下一个步骤针对此步骤进行处理，所有的具体处理应该都放在一个步骤里面，如果需要写入文件，需要在此步骤说明是每判断一个追加写入文件。
4. 如果用户意图需要输出很长的内容，推荐使用写文档的工具，写文档的工具需要先拆解需要写的目录结构，然后给定一个文件名，然后一步一步的完成，每一步都写入同一个文件。
5. 你需要先输出思考过程，说清楚你生成SOP的逻辑，然后再输出SOP。思考和SOP之间必须使用<SOP_START>来进行分割。
7. 使用@来说明使用什么工具，文件以及知识库，必须遵循以下格式：
工具的表示方法：@{{工具名}}@
文件的表示方法：@{{文件名}}的文件储存信息：{{'文件储存在语义检索库中的id':'{{id}}','文件储存地址':'{{文件路径}}'}}@
知识库的表示方法：@{{知识库名}}的储存信息:{{'知识库储存在语义检索库中的id':'{{id}}'}}@
