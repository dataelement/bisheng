...
**代码事实口径**: 本文所有 `文件:行号` 沿用 design.md（`3.0-vibe` HEAD `fe10f75ea`，2026-09-16 核实；路径以 `src/backend/bisheng/` 为根，前端另注 `platform/` = `src/frontend/platform/src/`）。行号会漂移，符号名不会——落地前以符号名重定位。
...
| design.md | ✅ 已定案（全自动模式） | 2026-09-16 初版，★ 由用户豁免、D1–D12 留痕；接手时的第一入口 |
| tasks.md | ✅ 已拆解（2026-09-16） | 本文；三条线（共享基础 / 门面 / MCP 面）+ 合流波 |
...
**后端 Test-First**：`a` 后缀 = 配对测试任务，物理排在实现任务前、先执行；`覆盖 AC` 逐条全写（禁 `AC-01~05` 范围写法）。基础设施任务（错误码 / schema / conftest / 注册表）无测试配对。单测放 `src/backend/test/open_api/` 与 `src/backend/test/knowledge/`（不放 `test/` 根），`asyncio_mode=auto`。本地无中间件（HARNESS.md）：集合相等 / fail-closed 端到端标 `@pytest.mark.e2e`，CI 中间件分组跑，本地默认跳过。跑法：`cd src/backend && env -u ALL_PROXY -u all_proxy -u HTTP_PROXY -u HTTPS_PROXY -u http_proxy -u https_proxy .venv/bin/python -m pytest test/open_api test/knowledge -q -p no:cacheprovider`（`| tail` 退出码不可信，读最后一行）；基线 = 主检出同选择。
...
- **T103**（`open_endpoints/api/endpoints/filelib.py:687-735 retrieve_chunks` 改调门面；`RetrieveReq` / `RetrieveResp` 不改；F066 `test_data_scope_matrix.py` 必须仍绿）
- **T201**（`open_api/api/dependencies.py:64-163 open_api_access_context` 拆成 `admit_open_api_principal` + `open_api_execution_scope` 两段——**对外行为逐字不变**，`test/open_api` 全部 40 个测试文件是护栏）
- **T203**（`main.py` 条件 `app.mount` + lifespan 进 `session_manager.run()`；`open_api/api/middleware.py:22-27` 对 `/api/v2/mcp` 前缀短路；`test_open_api_route_matrix.py:37-41` 改遍历方式）
...
- **T209 → F054 回写请求**：`AppDataService` 需 `insert_row / delete_row`（manager `POST /v1/apps/{id}/db/tables/{t}/rows`、`DELETE …/rows/{pk}`）——登记到 F054 tasks「跨 Feature 回写受理」表；F054 T086/T087 落地前 T209 阻塞
...
- [ ] **T103a**: v2 `POST /filelib/retrieve` 收敛测试
  **文件**: `src/backend/test/open_api/test_filelib_retrieve_facade.py`（新）
  **逻辑**: 用 `v2_client`（`TestClient(app, raise_server_exceptions=False)`）+ monkeypatch `RetrievalFacadeService.retrieve` 为 spy：`test_endpoint_builds_identity_from_principal_mode_s`（`RetrievalIdentity.actor.subject_type == "service_account"`）→ AC-25；`test_endpoint_mode_d_identity_is_target_user`（模式 D principal → `("user", effective_user_id)`）→ AC-43（能力交付，验收归 F050）；`test_response_shape_unchanged`（`RetrieveResp{chunks[{content,knowledge_id,document_id,document_name,chunk_index,document_update_time}],total}` 字段集与 HEAD 快照相等）→ AC-25；`test_unreachable_maps_to_26321_http_404`（旧 404 `NotFoundError` / 403 `SpacePermissionDeniedError` / 10962 三态不再出现）→ AC-11；`test_permission_unavailable_is_503_with_19002_not_26030`（D6：删包装）→ AC-24, AC-44；`test_empty_knowledge_base_ids_still_400`（契约不变）→ AC-25；`test_no_bypass_of_facade_in_open_face`（静态：`grep` `open_endpoints/` 与 `open_api/` 下不存在对 `aretrieve_chunks` / `RetrievalEngine` 的直接调用，只允许 `RetrievalFacadeService`）→ AC-26；`test_data_scope_matrix_still_green`（直接 `pytest.importorskip` 并复跑 `test_data_scope_matrix.py` 的 retrieve 用例——或在该文件里保留断言，二选一）→ AC-25。
  **覆盖 AC**: AC-11, AC-24, AC-25, AC-26, AC-43, AC-44
  **依赖**: T002, T102

- [ ] **T103**: v2 端点改调门面 + 文档订正
  **文件**: `src/backend/bisheng/open_endpoints/api/endpoints/filelib.py`（`retrieve_chunks :689-`）, `src/backend/bisheng/open_endpoints/domain/schemas/filelib.py`（`RetrieveReq` 不改；`RetrieveChunk` 不改）, `docs/api/filelib-retrieve.md`（`:181-186` 错误表加 26321 / 26322 / 26323 行、删 404「知识库不存在」与 403「默认操作员」两行；`:287` 「default operator 是否对该 KB 有 view 权限」→「统一检索门面按执行身份做可及性判定（知识空间 `visible`、文档库 `use`）+ 文件级双层过滤」）
  ...
  **测试**: T103a 全部通过；`test/open_api/test_data_scope_matrix.py` 全绿
  **覆盖 AC**: AC-11, AC-24, AC-25, AC-26, AC-43, AC-44
  **依赖**: T103a
...
- [ ] **T208**: 工具 ⑥ + 服务层 `entry` owner-only
  ...
  **逻辑**: design D9。actor 构造同 `app_publish/api/endpoints/deploy.py:200-213`（`resource_owner_of(principal)`）。`bisheng_app_status(app_id)`：`instance = get_instance(app_id, actor=actor, entry="mcp")` + `publish = get_publish_status(app_id, actor=actor, entry="mcp")` → 合并；`bisheng_app_logs(app_id, tail, since, keyword)` → `get_logs(..., entry=LOG_ENTRY_MCP)`。异常折叠：`AppNotFoundError(16101) / AppLogForbiddenError(16161) / AppPublishOwnerOnlyError(16254) / 16205` → `McpAppNotOwnedError`（26305，`data` 只含 `app_id`）。
  **测试**: T208a 全部通过；`test/app_runtime test/app_publish` 与基线无新增失败
  **覆盖 AC**: AC-16, AC-17, AC-18, AC-34, AC-35, AC-36
  **依赖**: T208a

- [ ] **T209**: 工具 ⑤ 应用数据（**阻塞于 F054 T086/T087 `AppDataService`**——HEAD `fe10f75ea` 树上不存在，本波次 data-plane 切片正在同期实现；合流时以其落地签名为准）
  **文件**: `src/backend/bisheng/open_api/mcp/tools/apps.py`（增量）, `src/backend/test/open_api/test_mcp_tools_app_data.py`（新）
  **逻辑**: design D9 假设签名（模块 `bisheng/app_runtime/domain/services/app_data_service.py`）：`AppDataService.list_tables / get_schema / read_rows / update_row(..., actor, entry="mcp")`；六工具直接透传返回体；写操作审计由 F054 侧 `app.data_row_edit` 记（本工具**不**另记数据审计，只记 `open_api.mcp.tool_call`）；`insert_row / delete_row` 待 F054 回写受理后接线（未落地时两工具 `available()=False` → 不进清单）。测试：owner-only（含管理员名下 SA 拒）→ AC-34, AC-35；非 owner 26305 → AC-34；写调用触发 F054 审计（spy `AppDataService.update_row`）→ AC-15；无 DDL 入口（注册表无 `create/alter/drop` 工具）→ AC-15；运行时层关 → 16207 → AC-17；`test_mcp_face_reuses_same_service_method`（对应 F054 T087a 同名用例：工具不 import `orchestrator_client`）→ AC-15。**测试降级**：F054 未落地前本任务整体 `[ ]`，注册表 `available()` 由 `importlib.util.find_spec("bisheng.app_runtime.domain.services.app_data_service")` 判定。
...
- [ ] **T212**: MCP 注册表驱动的逐位矩阵（Line B 内可先跑，T301 后补 ①②）
  ...
  **逻辑**: 参数化 `TOOL_REGISTRY × {仅 knowledge:read, 仅 model:invoke, 仅 identity:read, 仅 app:manage, 空}`：持位 → 在清单且可调（handler monkeypatch 为 stub）；不持位 → 不在清单且直调 26302 指明缺位；断言矩阵行数 == `len(TOOL_REGISTRY) × 5`（防漏，新增工具未进表即红）。
  **覆盖 AC**: AC-04, AC-45
  **依赖**: T203, T207, T208
...
| AC | 任务 | AC | 任务 | AC | 任务 |
|---|---|---|---|---|---|
| AC-01 | T203, T302, T303 | AC-17 | T203a/T203, T208a/T208, T209 | AC-33 | T207a/T207 |
...
| AC-10 | T301a/T301 | AC-26 | T103a/T103 | AC-42 | T104（F055 承接运行期） |
| AC-11 | T102a/T102, T103a/T103, T301a/T301 | AC-27 | T102a/T102 | AC-43 | T103a/T103, T104（F050 承接） |
| AC-12 | T301a/T301 | AC-28 | T203a/T203 | AC-44 | T103a/T103, T104, T302 |
...
> 推翻已定案的决策时，先停下与用户重新确认（本轮 ★ 已豁免，但 D1 传输形态 / D5 抽出引擎 / D6 契约不变 / D9 owner-only entry 四条属「产品可见」决策，翻案仍须确认）。
