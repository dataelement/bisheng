...
- **结构演进（`database.tables[]`）本轮后置**：MVP 期该字段**允许声明但不建表、不做破坏性变更检测**——预检对非空 `database.tables[]` 给出 `hints`「本环境暂不由平台建表，请用 `BISHENG_APP_DB_URL` 自行建表」而**不拒绝**（应用自己在 SQLite 里 `CREATE TABLE IF NOT EXISTS` 完全可行，拒绝反而挡死剧本）。改 / 删列的显式确认（AC-09 / AC-42）与迁移前生产数据快照随后置波次，落点 = 预检新增 `precheck_schema` 阶段 + `POST /api/v2/apps/deploy` 的 `confirm_schema_change` 参数（端点参数**本期就留**，避免 CLI 侧改两次）。**T061 已落地显式确认**（`schema_evolution_service.py`）：闸在 `accept()` 同步腿、以**在线版本**的 `database.tables[]` 为基准，删表 / 删列 / 改列（type / nullable / default 任一变化）未确认拒 `16229`，加表 / 加列不问；worker 侧 `precheck_schema` 只推导摘要写入审批单与发布面。
  **T062 已落地平台建表与迁移前快照**（2026-09-16）：`schema_evolution_service` 增量 `migration_plan()`（同一份 diff → 每表一条计划：`create_table` / `add_columns` / `rebuild_table` / `drop_table`）+ `migrate_for_release()`，在 `PublishOnlineService._settle` 里、**拉起之前**调用。**DDL 一行都不在 backend**——库文件在 manager 宿主机上、多节点下常与 backend 不同机，这正是 D14 对日志、D10-C 对数据行编辑成立的同一条论证；执行方是 runtime-manager 的 `appdb.AppDbSchemaService`，经新 intent RPC **`POST /v1/intents/db-migrate`**（不挂在数据面 `/v1/apps/{app_id}/db/*` 下，否则那组路由"没有 POST、因而没有 DDL"的断言就废了）。快照用 `Connection.backup` 而非裸 tar（盘上的文件是半个 WAL 库，边写边打包出来的副本打不开），键 `apps/{app_id}/db-snapshots/{ts}.tar`，**存不进对象存储就拒绝迁移**；破坏性计划由 manager 自己判定要不要快照，不依赖调用方记得传标志。整份计划一个事务、每步幂等（手动上线会重放同一份计划）。**迁移被拒 = 失败的管线运行（`16259`），不是待上线**：待上线会以 16252 拒掉那次携带修复的重新提交；应用本来在线时按 `iteration_failed` 发站内信，否则不发（理由见 §8 的 AC-64 复核）。
  两条**计划形状的硬约束**，两条都是「写错了照样全绿」的那种：① **计划的每一条都带整表目标列，不带增量**——计划的动词来自两份*声明*的 diff，而线上库的实际形状可以与参照声明不一致（应用自己 `CREATE TABLE IF NOT EXISTS` 建的、或在线版本发布于「声明了但平台不建表」的 MVP 阶段）；执行器遇到「表不存在」会照计划条目建表，若条目只带增量列就会建出一张只有一列的表。所以计划陈述目的地，执行器对着实际库自己算增量（只补缺的列）。② **`resume` 也走迁移**——审批落在已下线应用上只 stage 不启动（AC-36），上线腿返回 `staged_only`、迁移不跑；真正把那个版本拉起来的是 `AppStateService.resume`，迁移因此也挂在 `_start` 的 RESUME 分支上（拉起之前，被拒即 16259、应用留在 `stopped`），否则这条路径上的应用会拿到一个从没建过表的库。
...
- **模型注入**：经 F051 的平台 OpenAI 兼容面；应用侧用**行业标准客户端**（`openai` SDK）+ 平台注入的 **`OPENAI_BASE_URL`**（= `{origin}/api/v2/model/v1`，另以平台保留名 `BISHENG_MODEL_BASE_URL` 同值注入；**不是** `BISHENG_PLATFORM_API_BASE`——那是 v2 业务面的地址，模型面另有前缀）+ **`OPENAI_API_KEY`**（= `BISHENG_APP_TOKEN` 同值，官方客户端零配置直读的就是这个名字），平台不做薄封装（PRD-1 DEV-07 已定"模型调用与应用数据库刻意不进 SDK"）。名字的唯一定义方是 F051 design D2 与 F054 `contracts-runtime-manager.md` §5。
  **运行期模型错误码一律走 262 段**：模型「已收回」= `26212` / `26213`、「未声明」= `26215`（F051 spec AC-13 / AC-34 在模型面的执行点）。本 Feature 的 `16273`（能力已被收回）/ `16274`（未在能力声明中的能力）**只用于知识库等非模型能力**——两边各留一对码是刻意的，不要把模型的两种拒绝翻译成 162 段。
...
- **审读视图（AC-25）—— ✅ 2026-09-16 已落码**（T052 后端半 + 前端半 + T063 + T064）：
  - **案 A（选定并已落地）**：同一处界面切 `viewMode`，不新开路由。**案 B（client 另开路由页）被否**：脱离决议-3「处理面在既有处理界面内」的口径，且要新建路由 + 权限守卫 + 返回态。
  - **⚠️ 原文的「弹窗放宽到 `md:max-w-[1200px]`」已作废**：beta2 把审批中心由弹窗改成设置页内嵌 `ApprovalPane`（K11 ③），没有弹窗可以放宽。按页面重估的结论是**审读视图 `col-span-full` 接管整个设置内容区**（原本的 300px 列表 + 详情两栏），「返回审批单」还原列表。案 A 的实质——同一处界面的第二种模式——原样保留。
  - 落点：`client/src/components/approval/{AppReviewView,ReviewSourcePane}.tsx` + `reviewTree.ts`（扁平条目折树的纯函数）+ `api/hostedAppReview.ts`；入口按钮在 `AppPublishDetailPanel` 的基本信息区（**不与通过 / 驳回同区**：只读动作混进两个决策按钮之间是误点来源），经 `ApprovalDetailPanels` 透传到 `ApprovalPane` 的 `reviewTarget` 状态。
  - 四个 tab = 代码（左树右只读正文）· 能力声明 · 版本历史 · 文件差异。差异用 `@bisheng/file-viewers` 的 `VersionDiffView`，**与 platform 版本 tab 同一个组件**（D15 的 WB-15 段）。
  - **只读是结构性的**：正文渲染在 `<pre>` 里，没有编辑器、没有下载入口；二进制 / 超大文件按树上已给的 `previewable:false` 直接就地说明原因，**不再白跑一次单文件请求**。
  - 「预览试用」置顶按钮仍属 T054（未做）。
...
- **WB-15 版本差异（AC-41）—— ✅ 2026-09-16 已落码**（T063 后端 + T064 前端）：数据源 = 服务端比对两个版本快照的 tar 条目（`GET /api/v1/apps/{app_id}/versions/{a}/diff/{b}`），**服务端算 diff 不下发两份 tar**（下发等于把代码明文交给浏览器，且 50MB 包会直接卡死页面）；精确契约与三道体积闸见 **§4.2 ⑨**。
  - **⚠️ 呈现组件不在 `hostedApp/publish/VersionDiff.tsx`，在 `packages/file-viewers/src/VersionDiffView.tsx`**。原落点让「版本 tab 与审读视图同一呈现」只能靠两份拷贝实现——两个 SPA 不共享 `src/`（platform 甚至不依赖 `@bisheng/ui`），而两份 diff 渲染器在第一次改 bug 时就会分叉。`@bisheng/file-viewers` 是两端都已依赖的源码型共享包，其契约正合：props 进、无 HTTP 客户端、文案走 `packages/locales` 的 `shared:` 命名空间。**未新建 workspace 包**：新包要 `pnpm install` 才被解析，而 client 的 jest 一旦 install 就会被未编译的 `canvas` 整体拖垮（memory `reference_frontend_pnpm_install_breaks_canvas`）。
  - `hostedApp/publish/VersionDiff.tsx` 保留，但只做 platform 侧的**选版与取数**：默认「运行中版本 → 待生效版本」，两侧可改；client 审读视图不给选版器（审批人在审一次发布，不是在翻历史），固定「上一已发布版本 → 待上线版本」。
  - 组件内的 class **只用 Tailwind 默认色板**：两端没有共同的主题层（client 有设计令牌 preset，platform 没有），`text-text-1` 在其中一端会渲染成空。另：两端 `tailwind.config` 的 `content` 本来都没扫 `packages/file-viewers/src`——共享组件的 class 此前只在「恰好本应用别处也用了」时才生成，已一并补上（`ExcelPreview` 同受益）。
...
| 16250–16269 | 发布流程 | `16251` 已存在在途审批单 · `16252` 待上线态不接受新提交 · `16253` 版本记录不存在 · `16254` 仅 owner 可执行 · `16255` 当前应用态不允许该动作 · **`16256` 版本快照不可用**（版本行在、对象不在 / 解析失败，`details.reason ∈ {missing, storage_error, unreadable_archive}`）· **`16257` 无权查看该版本代码**（审读视图 / 差异；含应用不存在 `reason: not_found`，不区分以免探测）· **`16258` 快照中没有该文件**（`reason ∈ {not_found, illegal_path}`；目录也算 not_found）· **`16259` 应用数据表迁移失败**（T062，上线前的结构演进被拒；`details.reason ∈ {notnull_without_default, invalid_type, invalid_identifier, invalid_plan, invalid_default, sqlite_error, snapshot_failed}` + `table` / `column`。**与 16229 不是一回事**：16229 = 破坏性变更未确认、在上传那一刻回答、解法是 `--confirm-schema-change`；16259 = 已确认但库不肯接受、在上线那一刻回答、解法是改 `bisheng-app.yaml`）|
| 16270–16289 | 能力总线 | `16273` 该能力已被收回 · `16274` 未在能力声明中的能力（**两码只用于知识库等非模型能力**；模型能力的运行期判定一律走 262 段，见下方一码一义第四条与 D13）|
...
> - **`16273` / `16274` 只管非模型能力**。模型能力的运行期判定**一律走 262 段**：「已收回」= `26212`（已下线）/ `26213`（提供方已删），「未声明」= `26215`（F051 spec AC-13 / AC-34 在模型面的执行点）。两边各留一对码是刻意的——16273 ⟷ 26212/26213、16274 ⟷ 26215 是一一对应关系，**不要把模型的两种拒绝翻译成 162 段，也不要只登记其中一半**（只说 16273 会留下和原来一样的歧义）。AC-53 在模型面的表现即 26212 / 26213，AC-51 的表现即 26215；发布面的「已失效」标记按 `declared ∖ 当前可解析` 按需计算，判据取自 F051 的 `resolve_model_name`，与面上的拒绝同源。守卫：`test/app_publish/test_capability_model.py::test_this_features_capability_codes_are_never_raised_for_a_model`（AST 断言每处 raise 都带 `kind=CAPABILITY_KIND_KNOWLEDGE`）。
>
> `withdraw` 终态守卫用 **approval 段 181xx**，不占 162（D10）。

**⑨ 审读视图只读接口 + 版本差异**（`/api/v1`，登录态；AC-25 / AC-41；2026-09-16 落码 T063 + T052 后端半）

四个端点共用一条访问规则 `ReviewAccess.require(app, actor, version_ids)`（`snapshot_browse_service.py`）：**owner / 该应用租户的管理员 / 平台超管 / 持有该版本发布申请审批任务的审批人**——"持有任务"按字面：`approval_task.approver_user_id = 调用人` 且所在 instance 的 `payload_snapshot.version_id ∈ version_ids`（差异接口传两个版本，审批人只需持有其一——AC-41 要比的正是"待上线 vs 上一已发布"，后者没有自己的审批单）。任务状态不限（已决的审批人仍可回看自己审过的东西）。**拒绝一律 200 + 业务码**（K11 ②）：无权 / 应用不存在 → `16257`；版本不存在 → `16253`；快照对象缺失或解析失败 → `16256`；文件不存在 / 路径非法 → `16258`。四个响应都带 `role ∈ owner|super_admin|tenant_admin|approver`，前端据此决定还画不画只有 owner 才有的动作。

> **⚠️ 第四个端点 `review-context` 是 2026-09-16 前端半落码时补的**（T052 前端切片，属对本节的追加而非改写）。原三端点足以画「文件树 + 只读代码」，但画不出 AC-25 要求的另外两个 tab：**版本历史**与**文件差异**——`GET /api/v1/apps/{app_id}/publish-status`（`_require_viewer`）与 F054 的 `GET /api/v1/apps/{app_id}/versions`（`AppQueryService._load_visible`）**都只放 owner / 租户管理员 / 平台超管，审批人一律被拒**（16254 / 16106）。于是审批人既拿不到版本清单，也拿不到「上一已发布版本」的 id 去调差异接口。补这一读是让审批人侧的 AC-25 成立的最小动作，**不新增错误码、不碰快照对象**（纯元数据读，快照被清理时仍正常返回）。
...
- **已知短板（暂不投入，理由如实）**：
  ...
  - **AC-08 不做静态依赖分析**：读 `requirements.txt` 猜"是不是连了 MySQL"是幻觉级判据，判据下沉为探活失败 + `hints`（D4）。
    **2026-09-16 复议结论（T068）：维持原判，且已按落码核实。** 复议问的是"落地之后这条判据够不够用"，答案分三段：① **检出**——应用连不上自带中间件就起不来，`precheck_probe` 阶段以 `16228` 拒绝，这是真实发生的失败而不是猜测；② **指引**——`precheck_service.PROBE_HINTS` 逐条点名托管契约排除的三类中间件（数据库 / 消息队列 / 缓存）与替代物 `BISHENG_APP_DB_URL`，并提示监听 `0.0.0.0` 与用 `bisheng dev` 本地复现；两处 `AppStartupProbeFailedError` 抛出点都挂着同一份 hints（一处漏挂＝一半的拒绝不再解释自己）；③ **没有长出扫描器**——预检不读任何依赖清单做中间件判断。三段均由 `test/app_publish/test_notification_matrix.py::TestAc08StaysProbeBased` 钉住。**何时重新考虑**：出现"探活其实通过了、但应用在运行期才连自带中间件"的真实案例（例如懒连接 + 首个请求才建连），届时判据仍不该回到读 `requirements.txt`，而应是出站白名单的实际拦截（F054 D12）。
  - **两阶段补偿不是原子事务**：Gate 自带 commit，无法与 `app_version` 的 INSERT 同事务（D6）——补偿路径已写死并有单测，但极端情况下（补偿本身失败）会留一个孤儿审批单，靠管理端异常处理兜。
  - **迁移成功、启动随后失败时，线上老版本会对着新结构跑**（T062 残留风险，**未决**）：迁移必须在拉起之前（拒绝时应用一分钱不花），可这样一来一次已确认的删列如果后面探活挂了，仍在服务的老版本就会遇到一张少了列的表。反过来「先拉起再迁移」只会把问题换成新代码遇到老结构，两头都不干净；根治是 expand / contract 两段式发布，本册范围外。当前兜底：快照已经存了（键在 §4.2 ⑧），发布面与 `bisheng deploy` 报 `iteration_failed`，应急止血是下线。**何时重新考虑**：出现第一例真实的「确认删列 + 启动失败」事故，或平台开始支持两段式发布。
...
- **明确不做**：
  - **平台侧回滚**（RT-05 顺延；等价手段 = 本地 `git checkout` 后重新 deploy；应急止血始终是下线）· **迭代审批单变更摘要** · **应用标签设置** · **租户级实例数配额** · **审批单催办 / 超时提醒 / 升级机制** · **任何免审配置项**（INV-34）。
    **2026-09-16 复核（T068，AC-64 触达全表）：催办 / 超时提醒 / 升级机制确认不做，并已上机器闸。** 这类决定不会因为被推翻而报错——最先想到"挂了三天的审批单该催一下吧"的人接上一个定时任务，平台就多了一个没人定过的调度、没人写过的文案和没人拍过的收件人。`test/app_publish/test_notification_matrix.py::TestNoReminderMechanism` 从三个方向钉住它：`worker/app_publish` 与 `worker/approval` 两个包里没有任何 `crontab` / `beat_schedule` / periodic（两个包各有一个任务，但都是"事情发生时 `delay` 一次"）；发布与审批的 service 层没有 remind / escalat / overdue / deadline / 催办 / 逾期 这类标识符或字面量；`ApprovalScenario` 模型没有任何可以承载截止时间的字段，所以别的服务也长不出读它的定时器。同一文件把 AC-64 全表（8 条会发 + 3 条不主动提示）写成数据并逐条断言：**不主动提示**的三条是「资源释放后可手动上线」「能力被收回」「数据表迁移被拒且应用本来就不在线」，各自的理由与该行同在一处，加发送方就必须先删掉那一行。
...
| 日期 | 改动 | 触发原因 |
|---|---|---|
| 2026-09-16 | **T052 前端半 + T064 落码，AC-25 / AC-41 前后端闭环**：§4.2 ⑨ 追加第四个端点 `…/versions/{version_id}/review-context`（版本历史 + `current`/`pending` 两侧 id，同一条 `ReviewAccess`，不新增错误码、不读快照对象）——**补它的原因是 AC-25 的「版本历史」与「文件差异」两个 tab 在审批人身上原本无法成立**：`publish-status` 的 `_require_viewer` 与 F054 的 `AppQueryService._load_visible` 都只放 owner / 租户管理员 / 平台超管。D14 改写：**「弹窗放宽到 1200px」作废**（beta2 后已无弹窗），案 A 的实质改为「审读视图 `col-span-full` 接管整个设置内容区」。D15 的 WB-15 段改写：**呈现组件落 `packages/file-viewers/src/VersionDiffView.tsx` 而非 `hostedApp/publish/VersionDiff.tsx`**——两个 SPA 不共享 `src/`，原落点让「同一呈现」只能靠两份拷贝成立；platform 下的同名文件保留但只做选版与取数。顺带：两端 `tailwind.config` 的 `content` 补扫 `packages/file-viewers/src`（此前共享组件的 class 只在「恰好本应用别处也用了」时才生成） | PRD-1 收尾切片 f055-review-frontend |
