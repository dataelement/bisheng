...
interface DialogContentProps
  ...
  close?: boolean
  /** Merged into the overlay (e.g. above drawers at `z-[70]`). Default overlay is `z-[100]`. */
...
const DialogContent = React.forwardRef<
  ...
  DialogContentProps
>(({ className, children, close = true, overlayClassName, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      {close && <DialogPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground">
        <X className="h-4 w-4" />
