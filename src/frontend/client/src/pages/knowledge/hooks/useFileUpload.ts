...
async function uploadFilesSequential(
    ...
    for (let i = 0; i < files.length; i++) {
        ...
        try {
            ...
            const statusCode: number | undefined = uploadErr.statusCode ?? uploadErr.response?.data?.status_code;
...
export function useFileUpload({
    ...
    }, [activeSpace, duplicateFiles, currentPage, isStorageBlocked, loadFiles, refreshQuota, showToast]);
