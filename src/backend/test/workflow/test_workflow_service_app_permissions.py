...
def _load_workflow_service_module():
    ...
    try:
        ...
        tag_module.TagDao = SimpleNamespace(
            ...
            asearch_tags=AsyncMock(return_value=[]),
            aget_resources_by_tags=AsyncMock(return_value=[]),
        )
        ...
        class _DummyApplicationPermissionService:
            @staticmethod
            async def get_app_permission_map_async(login_user, rows, permission_ids, *, context=None):
                return {}
            ...
            @staticmethod
            async def build_app_permission_context_async(login_user):
...
async def test_get_uncategorized_flows_filters_by_view_app_permission_id():
    ...
    with (
        ...
        patch.object(
            WorkFlowService,
            "_scan_visible_apps_page",
            new_callable=AsyncMock,
            return_value=(app_rows, {"wf-1": {"view_app", "share_app"}}),
        ) as mock_scan,
        patch.object(
            WorkFlowService,
            "get_logo_share_link",
        ...
        data = await WorkFlowService.get_uncategorized_flows(login_user, 1, 8, None)

    login_user.aget_merged_rebac_app_resource_ids.assert_not_awaited()
    login_user.get_merged_rebac_app_resource_ids.assert_not_called()
    assert mock_scan.await_args.kwargs["permission_id"] == "view_app"
    assert data[0]["logo"] == "logo-url"
    assert data[0]["can_share"] is True


@pytest.mark.asyncio
async def test_scan_visible_apps_page_refills_sparse_batches_and_reuses_context():
    workflow_module = _load_workflow_service_module()
    WorkFlowService = workflow_module.WorkFlowService
    workflow_module._APP_COMPAT_PAGE_SCAN_BATCH_SIZE = 2

    login_user = SimpleNamespace(user_id=7, is_admin=lambda: False)
    first = [
        {"id": "wf-4", "flow_type": FlowType.WORKFLOW.value, "update_time": 4},
        {"id": "wf-3", "flow_type": FlowType.WORKFLOW.value, "update_time": 3},
    ]
    second = [
        {"id": "wf-2", "flow_type": FlowType.WORKFLOW.value, "update_time": 2},
        {"id": "wf-1", "flow_type": FlowType.WORKFLOW.value, "update_time": 1},
    ]
    ...
    with (
        patch.object(
            ...
            "_app_type_ids_for_permission",
            new_callable=AsyncMock,
            return_value={FlowType.WORKFLOW.value: ["wf-1", "wf-2", "wf-3", "wf-4"]},
        ),
        patch.object(
            workflow_module.ApplicationPermissionService,
            "build_app_permission_context_async",
            new_callable=AsyncMock,
            return_value=permission_context,
        ) as build_context,
        patch.object(
            workflow_module.FlowDao,
            "aget_all_apps",
            new_callable=AsyncMock,
            side_effect=[(first, True), (second, False)],
        ) as get_apps,
        patch.object(
            workflow_module.ApplicationPermissionService,
            "get_app_permission_map_async",
            new_callable=AsyncMock,
            side_effect=[
                {"wf-4": set(), "wf-3": {"view_app"}},
                {"wf-2": {"view_app", "share_app"}, "wf-1": set()},
            ],
        ) as get_permissions,
    ):
        data, page_permissions = await WorkFlowService._scan_visible_apps_page(
            user=login_user,
            page=1,
            page_size=2,
            name=None,
    ...
    assert [item["id"] for item in data] == ["wf-3", "wf-2"]
    assert page_permissions["wf-2"] == {"view_app", "share_app"}
    assert get_apps.await_count == 2
    assert get_apps.await_args_list[1].kwargs["cursor"] == [3, "wf-3"]
    build_context.assert_awaited_once_with(login_user)
    assert all(call.kwargs["context"] is permission_context for call in get_permissions.await_args_list)


@pytest.mark.asyncio
async def test_scan_visible_ranked_apps_advances_composite_cursor_and_hides_internal_keys():
    workflow_module = _load_workflow_service_module()
    WorkFlowService = workflow_module.WorkFlowService
    workflow_module._APP_COMPAT_PAGE_SCAN_BATCH_SIZE = 2
    login_user = SimpleNamespace(user_id=7, is_admin=lambda: False)
    ...
    with (
        patch.object(
            ...
            "_app_type_ids_for_permission",
            new_callable=AsyncMock,
            return_value={FlowType.WORKFLOW.value: ["wf-2", "wf-3", "wf-4"]},
        ),
        patch.object(
            workflow_module.ApplicationPermissionService,
            "build_app_permission_context_async",
        ...
        data, _ = await WorkFlowService._scan_visible_apps_page(
            user=login_user,
            page=1,
            page_size=2,
            name=None,
    ...
    assert [item["id"] for item in data] == ["wf-3", "wf-2"]
    assert get_apps.await_args_list[1].kwargs["cursor"] == [1, 30, "wf-3"]
    assert all("_used_rank" not in item and "_sort_time" not in item for item in data)


@pytest.mark.asyncio
async def test_uncategorized_empty_tag_links_keeps_all_apps_eligible():
    workflow_module = _load_workflow_service_module()
    WorkFlowService = workflow_module.WorkFlowService
    login_user = SimpleNamespace(user_id=7, is_admin=lambda: False)
    app_rows = [
        {"id": "wf-1", "flow_type": FlowType.WORKFLOW.value, "logo": "/logo.png"},
    ]
    ...
    with (
        patch.object(
            ...
            "asearch_tags",
            new_callable=AsyncMock,
            return_value=[SimpleNamespace(id=60)],
            create=True,
        ),
        patch.object(
            workflow_module.TagDao,
        ...
        patch.object(
            ...
            "_scan_visible_apps_page",
            new_callable=AsyncMock,
            return_value=(app_rows, {"wf-1": {"view_app"}}),
        ) as scan,
        patch.object(
            WorkFlowService,
            "get_logo_share_link",
        ...
        data = await WorkFlowService.get_uncategorized_flows(login_user, 1, 20, None)

    assert [item["id"] for item in data] == ["wf-1"]
    assert scan.await_args.kwargs["id_list_not_in"] == []
