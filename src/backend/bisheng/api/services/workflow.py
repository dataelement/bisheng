...
class WorkFlowService(BaseService):
    ...
    @classmethod
    async def aget_frequently_used_flows_cursor(
        cls,
        user: UserPayload,
        user_link_type: str,
        cursor: str | None = None,
        ...
        """F040/F027 pseudo-cursor envelope for the "常用" (frequently-used) list.

        The candidate set is the user's own favourites link list (bounded
        per-user) sorted by the order they were added — a custom order that a
        keyset cursor over ``update_time`` can't express, so this reuses the
        F027 AD-15 pseudo-cursor (key=``[page_num]``, **no total**, INV-6),
        mirroring ``alist_mine_and_joined_cursor``. The page is sliced first and
        only then enriched, so per-page cost is bounded by ``page_size``.
        Equivalent to ``get_frequently_used_flows`` for the same page.
        """
        from bisheng.common.cursor import CursorDecodeError, decode_cursor, encode_cursor
        from bisheng.common.errcode.flow import AppInvalidCursorError
        from bisheng.common.schemas.api import PageInfiniteCursorData

        context = "frequently_used|sort=link_order"
        try:
        ...
        ret = UserLinkDao.get_user_link(
            user.user_id,
            [app_type.value for app_type in UserLinkType.app.value],
        )
        if not ret:
            return PageInfiniteCursorData(
                data=[],
        ...
        page_items = data[start : start + page_size + 1]
        has_more = len(page_items) > page_size
        page_items = page_items[:page_size]

        page_items = cls.add_extra_field(user, page_items)
        page_items = await cls.aenrich_apps_can_share(user, page_items)
