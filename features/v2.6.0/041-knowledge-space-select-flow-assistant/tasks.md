...
- [ ] **T001**: `accessScope` 字段 + 迁移（citation 链路，F029 协同）
...
- [ ] **T003**: 无副作用构造"配置者 UserPayload"辅助
...
- [ ] **T004**: 空间检索核心辅助（file_ids 解析 + F029 双层过滤 + 身份 + sync→async）
...
- [ ] **T005**: 工作流 rag / knowledge_retriever 节点接空间分支
...
- [ ] **T006**: 工作流 agent 节点接空间分支 + 新增 user_auth 读取
...
- [ ] **T007**: 助手应用接空间分支 + 新增 user_auth 读取
...
- [ ] **T008**: 4 个 citation 生成点按开关赋 `accessScope`
...
- [ ] **T009**: `CitationResolveService` 分级解析（filter + enrich 两处）
...
- [ ] **T010**: Platform 知识空间列表 API（client 口径）
...
- [ ] **T014**: 4 入口统一「用户知识库权限校验」开关 + tips 文案
