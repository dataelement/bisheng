...
- **开放 API 管理面开关（2026-09-10，改接 beta2 后）**：`config.yaml` 需 **`open_api.management_ui_enabled: true`** 才看得到 platform 系统管理里的「服务账号 / 个人令牌」tab（beta2 默认关），这是剧本步 1 的前置；`open_platform.enabled: true` 仍是 `app:manage` 位可签发的前置。两键都是进程级配置，改后全量重启（迁移方案 M14 / §4 步 4）。
