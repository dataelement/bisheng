...
- [x] **T029**: 发布说明 + docs 补章（1h）
  ...
  **同批复核订正（2026-09-16，切片内复核，四处失实已改）**：① §2 第 4 步原写「名称没命中时区分模型已下线 `26212` / 服务商已删除 `26213`」——`model_catalog._explain_miss()` 的注释明写它**不出 26213**（删服务商连同模型行一起删，与「从未存在」无从分辨），26213 只在目录缓存窗口内 `get_bisheng_llm` 发现行已消失时出现，已按实现改写并写明窗口来自 `open_api.model_catalog_ttl_seconds`（默认 30s、上限 60s）；② §1 原写「页面也从 `whoami.model_base_url` 取」——接入信息区实际读 `GET /api/v1/dev-toolkit/versions` 的 `model.base_url`（`AccessInfoPanel.tsx:88`），两者同出 `model_gateway_base_url()`，改成「只有一个生产者、两个字段」；③ §3 原写 `reasoning_content`「为空时不出现」——只有流式增量成立（`openai_codec.sse_line` 用 `exclude_none=True`），非流式响应体没有 `response_model_exclude_none`，为空时是 `null`；④ §4 码表补 `26043` 与各码的 `openai_code`，`26205` 单独成行（它是 `permission_error`，与共用底座那五个码的 `invalid_request_error` 不同）。同批把 `docs/README.md` 的 `api/` 一行从「filelib 纯检索、知识空间/知识库接口」补上 MCP 面与本页。
