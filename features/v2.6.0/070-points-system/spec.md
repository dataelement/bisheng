...
| ID | 角色 | 操作 | 预期结果 |
|----|------|------|---------|
| AC-08 | 用户 | 文件成功进入目标库（直传或发布审批通过） | 按目标库 `space.level` 匹配 G1/G2/G5/G6；幂等键含 `file_id+target_space_id`；个人库/收藏库不计；操作者为该 space 的 `creator`/`admin` 则豁免；积分入账用户 = 规则 `beneficiary` 解析结果（`uploader`→文档上传人，`publisher`→发布操作人），只发给一方 |
