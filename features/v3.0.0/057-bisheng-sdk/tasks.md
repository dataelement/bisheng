...
| 实现 | 🚧 进行中 | **42 / 45**（三条切片合流：`wt/f057-sdk-core` 包本体 Wave 0–3、`wt/f057-sdk-dist` 分发与文档 Wave 0 / 4 / 5 / 6、收尾切片 `wt/f057-tail`（T029 wheel 已在位并逐项复核 / T041 旅程测试文件默认跳过 / T043 跨 Feature 回写）与 `wt/f053-skillpack-model`（T035a 模型章两条回归断言改守 F051 已交付的契约）。仍未勾的是需要 114 真机或真实构建容器、以及需要人拍板的项：T032 / T040 / T042。）|
...
- [x] **T010**: HTTP 层测试（客户端池 / 超时 / trust_env / 信封解析顺序 / 码映射两级降级）〔2h〕
  ...
  **偏差记录（2026-09-17）**：`test_trust_env_false_by_default_and_env_override` 拆成三例（本地默认不读 / `BISHENG_SDK_TRUST_ENV=1` 打开 / 注入 `BISHENG_APP_EGRESS_TOKEN` 即打开），D11 同步修订。起因：F054 T077 把托管应用网改成 `--internal` 后，平台 API 只能经出站代理到达，原「一律不读代理变量」会让托管期 `retrieve` 与版本探测直连得到「网络不可达」（114 实测）。同批：`test_contract_alignment.py` 加出站凭据变量名对账；`test_compat.py` 的本地版本断言改读 `__version__`（原写死 `0.1.0`，一升版本就红）；SDK 版本 `0.1.0 → 0.1.1` 并重打安装件，`SDK_MIN_COMPATIBLE` 保持 `0.1.0`（0.1.0 在 internal 网里连版本探测都发不出去，抬下限无用）。
...
- [x] **T029**: 打包并提交 SDK wheel（Wave 4 末尾，串行）〔0.5h〕
  ...
  **完成证据**: 两条切片合流后由协调批执行，commit **`c5e33ea8b`**（`git log -1 -- dev_toolkit/artifacts/` 即此条）。**2026-09-16 由 `wt/f057-tail` 逐项复核（未重打）**：① wheel 在位、18 个条目、`bisheng_sdk/` 15 个模块 + `dist-info`（不是坑 12 那种空壳 wheel）；② `shasum -a 256` = `380f1d6122821a8d293a689ae878535c3e087c0520fe910833de255de3ae3800`，与 `manifest.json["sdk"].sha256` **逐字相等**；③ `manifest.json` 四个键 `cli` / `platform` / `_note` / `sdk` 齐全，`cli` 段 sha256 `635054e2…` 与同目录 CLI wheel 实测一致（T038 重打后未被 T029 覆写）；④ `artifact_service.read_snapshot()` 真实读出 `SdkArtifact(version='0.1.0', min_compatible='0.1.0', …)`，即 `/versions` 现在答的是**实体 `sdk` 段**而非 null；⑤ wheel 版本 `0.1.0` 与 `bisheng_sdk/__init__.py:27 __version__` 一致；⑥ `test/dev_toolkit` 全量 **118 passed**。
...
- [ ] **T040**: 评测跑分（模型判定）并回填〔1.5h〕
  ...
  **2026-09-16 `wt/f057-tail` 复核：仍未做，且不该由本会话代跑。** 说明白为什么，免得下一轮又停在这里：对照组的全部价值在于**生成方没读过这个包**，而本会话为了做 T041 / T043 已经读过样本文件（它把「读包后应有产出」逐条写在正文里）、读过 `example-sdk/main.py` 的三件套写法。同一个上下文里再生成一次「未读包」的答案，测的是本会话能不能装作不知道，不是包有没有用；自评自判还叠一层。**真要跑，需要的是这些**：
  ① 一个**每条样本、每个臂各起一个干净会话**的跑批器（子代理 / 独立进程各一次，禁止复用上下文）；② 对照臂的提示词 = 样本的「需求」一行 + 「写出这个应用的代码」，**不给** `platform-wiring/SKILL.md`、不给任何平台名字；实验臂 = 同一句需求 + 把 `src/backend/bisheng/dev_toolkit/skills/platform-wiring/SKILL.md` 原文放进上下文；③ 判定用 `test/dev_toolkit/fixtures/auth_silent_failure_samples.md` 里那三条判据（无登录 / 注册 / 会话签发代码 · 访问者来自平台注入 · 取不到身份时报错而非匿名兜底），**判据不许改**，判定者是另一个未参与生成的会话或人；④ 回填「样本 id / 两臂产出摘要 / 判定 / 日期 / 所用模型」到本文「评测记录」小节。
  **顺带记一处口径冲突，跑之前先拍板**：本任务与样本文件的达标线都写「**5/5**」，而 T039 实际交付了 **6 条**样本（S-01…S-06）。要么把达标线改成 6/6，要么把它写成「随机抽 5 条」——现状下 5/6 算不算过，两边文本给不出答案。
...
- [x] **T043**: 跨 Feature 回写登记（只追加文档条目，不改他人代码）〔1h〕
