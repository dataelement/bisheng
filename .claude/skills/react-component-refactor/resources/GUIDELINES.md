...
7. **[ ] Verify** — Run the dev server (`npm start`) to ensure compilation succeeds.
...
```
API Layer
    ↕  raw types              Client: ~/api/    Platform: @/controllers/API/
...
- **Path aliases**: Client uses `~/` (or `@/`), Platform uses `@/` only.
