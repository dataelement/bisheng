...
class ReviewTagDao(ReviewTag):
    ...
    async def get_tags_by_business(cls, business_type: TagBusinessTypeEnum, business_id: str, name: str = None) -> List[
        ReviewTag]:
        statement = select(ReviewTag).where(ReviewTag.business_type == business_type, ReviewTag.business_id == business_id)
        if name:
    ...
    async def aupdate_resource_tags(cls, tag_ids: List[int], resource_id: str, resource_type: ResourceTypeEnum,
        ...
        async with get_async_db_session() as session:
            ...
            statement = delete(ReviewTagLink).where(
                col(ReviewTagLink.resource_id) == resource_id,
                col(ReviewTagLink.resource_type) == resource_type.value
            )
            await session.exec(statement)
            # Insert new tag association data
            for tag_id in tag_ids:
                tag_link = ReviewTagLink(tag_id=tag_id, resource_id=resource_id, resource_type=resource_type.value,
    ...
    @classmethod
    async def add_tags(cls, review_tag_ids: List[int], resource_id: str, resource_type: ResourceTypeEnum,
                       user_id: int) -> bool:
        ...
        async with get_async_db_session() as session:
            ...
            for one in need_add_tags:
                session.add(
                    ReviewTagLink(tag_id=one, resource_id=resource_id, resource_type=resource_type.value, user_id=user_id, is_deleted=False))
            await session.commit()
            return True