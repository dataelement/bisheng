...
def _install_apps_endpoint_stubs() -> None:
    ...
    if "bisheng.database.models.flow" not in sys.modules:
        ...
        flow_module.FlowDao = SimpleNamespace(
            get_all_apps=lambda **kwargs: ([], 0), aget_all_apps=lambda **kwargs: ([], 0)
        )
...
async def test_get_recommended_apps_does_not_use_relation_visibility_helper():
    ...
    with (
        patch.object(module.WorkStationService, "get_config", return_value=config),
...
async def test_get_recommended_apps_filters_by_view_app_permission():
    ...
    with (
        ...
        patch.object(
            ...
            side_effect=lambda user, data: data,
            create=True,
        ),
    ):
...
async def test_get_used_apps_does_not_use_relation_visibility_helper():
    ...
    with (
        patch.object(
            module.MessageSessionDao, "get_user_used_apps", new_callable=AsyncMock, return_value=used_apps, create=True
        ),
