...
- **广场搜索已跨 tab（2026-08-19 落地，D10 的另一半）**：`explore.tsx` 原先把 `searchQuery` **连同当前 `tag_id` 一起**发（`api/apps.ts:441-443` 透传给后端过滤），于是未打标的托管应用**既不在默认 tab、也搜不到**——AC-08「可按名称搜索到」在配了首页标签的环境上不成立（114 恰好没配首页标签，`AgentNavigation.tsx:59` 的 `next[0]` 为空回落「未分类」，所以一直没暴露）。现改为**有关键词时不按 tab 收窄**，搜索横跨全部分类。**默认 tab 仍不动**（产品 2026-08-19 拍板）：它是工作流 / 助手的既有行为，改它属广场级行为变更；且应用中心首页 `AgentGrid.tsx:432-437` 恒定渲染「未分类」分区，未打标的托管应用在首页本来就直接可见，受影响的只有广场页那一个 tab。
