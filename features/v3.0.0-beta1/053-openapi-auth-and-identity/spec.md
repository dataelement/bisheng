...
- **AC-R12 免登录拒绝的呈现（2026-09-11）**：链接无效 `26101`、应用已下线 `26102` 共用 HTTP 404，免登录开关关闭或默认操作员不可用 `26103`、身份标识请求头 `26104` 共用 HTTP 403；响应体 `data` 为空对象。资源类型不符与助手已删除归入链接无效，两侧探测中任一侧为下线即判下线。两个 WebSocket 端点在拒绝时先 accept 再发送错误帧、最后以 1008 关闭，使浏览器能读到拒绝原因而非空 reason 的 1006。client 免登录页据业务码显示对应状态页，并在被拦截时不挂载对话组件、不发起 WebSocket；网络故障与 5xx 不拦页面。四个业务码三语文案同版交付。
