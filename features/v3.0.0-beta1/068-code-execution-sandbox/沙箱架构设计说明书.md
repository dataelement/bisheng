...
**版本**：v3.0.0-beta1 · 2026-09-20 · 错误码模块 **280**  
**状态**：Spec + Design 已 ★ 确认（2026-09-20）；tasks 29/29 已拆解并落地，业务 E2E 待 compose 实跑。
...
| 发现 | worker 领租约时 DNS 探测；runner **不向 Redis/API 自注册**。配置 **没有** k8s/compose 枚举，只换主机名模式 |
...
调用点落在现有装配层，**不新增对外 API**。灰度按工具切 `type`，不是按页面各接一套沙箱。

| # | 产品入口 | 今天的装配锚点 | 沙箱里有什么 |
|---|---|---|---|
| 1 | 灵思任务模式 | `workbench_impl._init_bisheng_code_tool` → `ToolExecutor` | 任务工作区全量（`uploads/` `output/` `scratch/` `skills/`）；sticky |
| 2 | 工作台日常会话 | `chat_service._prepare_tools`（`DAILY_CHAT`） | 通常空临时目录；用户原件不进 |
| 3 | 助手 | `assistant_agent.py`（`ASSISTANT`） | 同日常 |
| 4 | 工作流 Agent 节点 | `workflow/nodes/agent/agent.py:_init_tools` | 该次工具目录 |
| 5 | 工作流工具节点 | `workflow/nodes/tool/tool.py` | 同路径 4 |
| B | 工作流代码节点 | `CodeNode` → wrapper → 同一 `/exec` | 通常无业务文件 |

共同 dispatch：`ToolExecutor` 注入 MinIO → `load_tools._get_native_code_interpreter` 显式三分支 `local` / `container` / `e2b`。

**不是调用点**：灵思 `ls`/`read_file`/`grep`（MinIO 工作区）、知识库解析 LibreOffice、附件预览、技能包物化（物化后再被路径 1 copy-in）。
...
**何时整目录打 tar**：只在 `type=container` 的 `execute_code` 里发生（`local` / `e2b` 都不打 tar）。扫描是全目录；进 tar 的只有 md5 对不上的文件。新租约第一次 ≈ 全量；灵思 sticky 后续轮次只打变更；租约 404 / 副本重启后再 exec 才重新全量。日常会话 `dir_path` 常为空。容器模式 **没有** E2B 那条 5MB 自动推送天花板。
...
| `config.yaml` | **不**写 `deploy_mode: compose` | **不**写 `deploy_mode: k8s`。只改 `discover_host_pattern` + `discover_index_start`，或用 `endpoints` 钉死 |

两种编排下，**与 runner 的 HTTP 时序完全相同**；差别只是问哪套 DNS、主机名怎么写。应用看不见 docker / kube-apiserver，配「部署方案」枚举没有作用对象，还会和编排清单各写一份而漂移。
...
线上协议是 **三层**，不是一条 `/exec`（现网 SDK `e2b-code-interpreter==1.5.2`）：

| 层 | 做什么 | 毕昇调用 |
|---|---|---|
| 控制面 `api.e2b.app` | REST 建/杀 VM（`X-API-Key`） | `Sandbox(...)` / `kill()` |
| envd :49983 | 逐文件 `POST /files` + ConnectRPC 文件系统 | `files.write` / `read` / `list` |
| Jupyter :49999 | `POST /execute` NDJSON；kernel **跨轮保留变量** | `run_code` |

这和 F068 的「常驻副本上领租约 + 一次子进程 `/exec` + 目录 tar」不是同一类生命周期，所以禁止把云 API 适配成假 `/v1/sessions`。

自动 copy-in **5MB**（`SIZE_AUTOPUSH`）是 F035 防 worker 整文件读进内存后 silent bulk 出境的产品阈值，**不是** E2B 接口上限。设计里超限本应由 `required_files` 声明；现状工具 schema 只有 `python_code`，oversized 在装配期剔出 `file_list`，>5MB 等于进不去 E2B。本期不还。容器模式用 `max_copy_in_bytes` 点名跳过，不套这条 5MB。

本期不还 E2B 的已知债（技能包 copy-in 时机、产物只列一层目录、5MB 声明未接线等）。若以后要「正式支持」，对齐的是 `run_with_dir` 语义，不是再开一条业务路径。
...
- **没有** `deploy_mode` / `orchestrator` 字段。默认按 compose 主机名 `code-runner-{n}` 从 1 扫；K8s 改 `discover_host_pattern=code-runner-{n}.code-runner` 且 `discover_index_start=0`。`endpoints` 非空则跳过 DNS。
- 接入 URL / token **不进** 工具 `extra`。E2B 的 `api_key` / `domain` 留在工具 `config.e2b`，不写进 `sandbox_conf`。
...
4. **发现方式**：同意 worker DNS 发现，反对 runner 向 Redis 自注册；配置不写 k8s/compose 枚举，只换主机名模式。
...
| 11 | 默认一副本一会话 |
| 12 | 领租约时 DNS 发现，runner 不自注册；配置无编排类型枚举 |
...
| 五条路径 + 代码节点调用点、何时打 tar | design.md §4.1–§4.2 |
| HTTP tar copy-in / copy-out 步骤 | design.md §4.2 |
| E2B 三层协议、5MB 自动推送、后续接法 | design.md §4.8 · 决策 10 |
| 配置无 k8s/compose 枚举；compose / K8s 发现 | design.md §4.6 · 决策 12 |
