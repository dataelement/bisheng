...
**壳把规范写死的部分**：只有一档尺寸（数字 16px、圆点 6px），不随宿主放大；数字**原样显示、不折算**（没有 99+），位数多了胶囊自己变宽；等宽数字，9→10 不抖；挂角款自带 1px 页面底色描边，压在彩色图标或头像上也看得清；传 0 什么都不渲染（`showZero` 例外）；徽标不可点、无悬停、无焦点、出现消失不加转场——点击全归宿主。这些没有 prop。
...
| `dot` | `boolean` | `false` | 只说「有新的」。`count` 显示时它不生效 |
| `color` | `'danger' \| 'brand'` | 挂角 `danger` / 独立 `brand` | 按语义选，不按位置选：红底白字只表示「等你处理」 |
