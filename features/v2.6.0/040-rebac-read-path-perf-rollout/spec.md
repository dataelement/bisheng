...
> **2026-07-13 增补范围**：纳入此前 D5 遗留的工作台未分类列表，以及客户端应用广场默认在线列表。两者保持既有 `page/limit` 与列表响应契约，通过有界扫描消除普通用户的全量读取、全量鉴权和全量装饰。
...
>   - **F. 应用广场在线 / 未分类列表兼容优化**（后端）：`GET /chat/online` 默认排序与 `GET /workstation/app/uncategorized` 不再全量加载候选、全量计算普通用户权限后再切片；按数据库稳定序分批扫描、权限预过滤与精确复核，凑满请求页所需可见前缀后停止。两个端点保持现有 `page/limit` 参数和列表响应，现有 Platform / Client 调用方无需改造。
> - **本次明确排除**：
...
>   - **`/chat/online`、`/workstation/app/uncategorized` 暂不切换 cursor 响应**：两端已有页码及裸列表调用方，尤其 `/chat/online` 同时被应用广场、标签页、应用切换器和 Platform 页面消费。本次以兼容性为先，扫描量与“请求页深度”相关；后续若迁移 cursor，须单独完成调用方契约升级。
...
**故事 H（普通用户 · 应用广场）**：
作为 **只能访问租户内部分应用的普通用户**，
我希望 **打开应用广场的在线与未分类列表时，首屏耗时不再取决于租户应用总量**，
以便 **即使租户内存在大量无权应用，也不必等待全量权限评估完成**。
...
### 2.9 F · 应用广场在线 / 未分类列表兼容优化

- **AC-32** — THE SYSTEM SHALL 保持 `GET /chat/online` 与 `GET /workstation/app/uncategorized` 的既有请求和响应契约：继续接收 `page/limit`，成功响应继续返回应用列表，不要求现有前端切换 cursor 或 envelope。
- **AC-33** — WHEN 普通用户请求上述列表, THE SYSTEM SHALL 在数据库查询前使用权限系统给出的粗粒度可访问应用集合缩小候选范围，并对候选批次执行既有精确权限计算；粗过滤不得替代精确复核或放宽权限。
- **AC-34** — THE SYSTEM SHALL 对候选应用分批扫描，仅扫描到能够确定请求页所需的可见前缀或候选耗尽；首屏不得无条件加载租户内全部候选、全部会话或全部权限结果。
- **AC-35** — THE SYSTEM SHALL 仅对最终返回页执行用户、版本、标签、Logo 等响应装饰；不得对未进入返回页的全部候选执行逐项 Redis / MinIO 或附属数据查询。
- **AC-36** — WHEN `/chat/online` 使用默认排序, THE SYSTEM SHALL 保持“用户使用过的应用优先，按最后使用时间倒序；未使用应用随后按更新时间倒序”的既有顺序语义，并在同值时提供稳定顺序，避免跨批次重复或遗漏。
- **AC-37** — WHEN 系统中存在应用标签定义但没有任何应用标签关联, THE SYSTEM SHALL 将全部满足其它条件的应用视为未分类，而不是错误返回空列表。
- **AC-38** — THE SYSTEM SHALL 保持管理员短路及普通用户 `permission_id`、`share_app` 等现有权限语义；权限服务失败时沿用 fail-closed 行为，不把失败批次解释为无更多数据。
- **AC-39** — THE SYSTEM SHALL 使用 MySQL 与达梦均支持的 SQLAlchemy 表达式及现有 DM8 安全分页条件；本增补不新增数据库迁移、不使用行值比较、MySQL 专属函数或方言 SQL。
...
- **未分类标签集合为空（F）**：没有应用标签定义或没有任何标签关联时，排除集合为空，继续扫描全部满足其它条件的候选应用。
- **普通用户可见项稀疏（F）**：单批结果不足一页时继续取下一批并做精确复核，直到凑满请求页所需可见前缀或数据库候选耗尽；不得返回错误的半页。
- **默认排序时间相同（F）**：使用稳定的 ID 兜底序保证批间无重复、无遗漏；该兜底只消除旧实现未定义的同值顺序，不改变主要排序规则。
- **深页访问（F）**：兼容页码契约下允许从首批扫描到目标页所需可见前缀，成本可随页深增长，但首屏和浅页不得退化为租户全量扫描。
