...
from langchain_contrib import document_loaders as contribute_loader
from langchain_contrib import embeddings as contribute_embeddings
...
llm_type_to_cls_dict.update({
    llm_name: import_class(f'langchain_contrib.chat_models.{llm_name}' )
...
embedding_type_to_cls_dict.update({
    embedding_name: import_class(f'langchain_contrib.embeddings.{embedding_name}')
...
documentloaders_type_to_cls_dict.update({
    loader: import_class(f'langchain_contrib.document_loaders.{loader}')
...
CUSTOM_NODES = {**CUSTOM_AGENTS, **CUSTOM_CHAINS,
                **CUSTOM_EMBEDDING, **CUSTOM_INPUTOUTPUT}  # type: ignore
