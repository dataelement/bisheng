...
@pytest.mark.asyncio
async def test_permission_migration_script_passes_only_step():
    from bisheng.script.permission_rbac_to_rebac_migration import run_migration
    ...
    with patch(
        ...
        exit_code = await run_migration(only_step=4)

    migrator_cls.assert_called_once()
    assert migrator_cls.call_args.kwargs['only_step'] == 4
    fake_migrator.run.assert_awaited_once()
    fake_redis.aset.assert_not_awaited()
    assert exit_code == 0


@pytest.mark.asyncio
async def test_permission_migration_dry_run_does_not_require_fga():
    from bisheng.script.permission_rbac_to_rebac_migration import run_migration
    ...
    with patch(
        ...
        exit_code = await run_migration(dry_run=True)

    async_get_fga.assert_not_awaited()
    sync_get_fga.assert_not_called()
    fake_migrator.run.assert_awaited_once()
    assert exit_code == 0
...
@pytest.mark.asyncio
async def test_permission_migration_force_only_step_does_not_clear_checkpoint():
    from bisheng.script.permission_rbac_to_rebac_migration import run_migration
    ...
    with patch(
        ...
        exit_code = await run_migration(force=True, only_step=4)

    fake_migrator.clear_checkpoint.assert_not_called()
    fake_migrator.run.assert_awaited_once()
    assert exit_code == 0
