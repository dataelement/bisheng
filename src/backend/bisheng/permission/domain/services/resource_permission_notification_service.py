...
class ResourcePermissionNotificationService:
    ...
    async def _snapshot_access(
        ...
        ``checker`` is ``_can_manage``. Semantics are identical to calling it
        serially — same per-member OpenFGA checks (cache / legacy alias /
        fallback all preserved) — only the round-trips run in parallel, capped
        at ``_CHECK_CONCURRENCY``, so the snapshot no longer blocks
