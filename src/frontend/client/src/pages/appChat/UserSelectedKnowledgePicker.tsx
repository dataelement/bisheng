import {
    ...
    getWorkflowKnowledgeBasesApi,
    getWorkflowKnowledgeFilesApi,
    KnowledgeFile,
    KnowledgeSpace,
} from "~/api/knowledge";
...
function normalizeKnowledgeFile(raw: any, source: SourceItem): ScopeItem {
    ...
    return {
        ...
        isFolder,
        status: raw.status,
        fileLevelPath: raw.fileLevelPath ?? raw.file_level_path ?? "",
    };
}

function normalizeSpaceChild(raw: KnowledgeFile, source: SourceItem): ScopeItem {
    const isFolder = raw.type === FileType.FOLDER;
    return {
        source_type: source.source_type,
        source_id: source.source_id,
        source_name: source.source_name,
        ...
        isFolder,
        status: raw.status,
        successFileCount: Number(raw.visibleSuccessFileNum ?? raw.successFileNum ?? 0),
    };
}

function mapSpace(space: KnowledgeSpace): SourceItem {
    return {
        source_type: "space",
        source_id: toId(space.id),
        source_name: space.name,
        level: space.spaceLevel || space.spaceKind || "personal",
    };
}
...
export default function UserSelectedKnowledgePicker({
    ...
    useEffect(() => {
        ...
        getWorkflowKnowledgeBasesApi({ keyword, page_size: 80 })
            .then((items) => {
                if (!canceled) {
                    setKnowledgeList(items.map((item) => ({
        ...
        getGroupedSpacesApi({ order_by: "name" })
            .then((groups) => {
                if (canceled) return;
                const spaces = [
                    ...groups.publicSpaces,
    ...
    const loadKnowledgeChildren = (source: SourceItem) => {
        ...
        getWorkflowKnowledgeFilesApi({ knowledge_id: String(source.source_id), page_size: 1000 })
            .then((items) => {
                ...
                normalized.filter((item) => item.isFolder).forEach((folder) => {
                    const prefix = knowledgeFolderPrefix(folder);
                    nextStats[itemKey(folder)] = normalized.filter((item) =>
                        !item.isFolder
                        && isSuccessFile(item)
    ...
    const loadSpaceChildren = (source: SourceItem, parentId: number | null) => {
        ...
        getSpaceChildrenApi({
            space_id: String(source.source_id),
                        ...
                        setFolderStats((prev) => ({
                            ...
                            ...Object.fromEntries(stats.map((item) => [
                                itemKey({
                                    ...
                                    source_name: source.source_name,
                                    ref_type: "folder",
                                    id: Number(item.folderId),
                                    name: "",
                                }),
                                Number(item.visibleSuccessFileNum || item.successFileNum || 0),
                            ])),
                        }));
                    });
                }
    ...
    const renderScopeItems = (source: SourceItem, parentId: number | null, depth: number) => {
        ...
        if (loading) {
            return <div className="flex items-center gap-2 px-2 py-2 text-sm text-gray-400"><Loader2 className="h-4 w-4 animate-spin" />加载中...</div>;
        }
        if (!items.length) {
            return <div className="px-2 py-2 text-sm text-gray-400">暂无可选文件</div>;
        }
        return items.map((item) => {
            ...
            return (
                <div key={key}>
                    <div className={`flex items-center gap-2 rounded px-2 py-1.5 text-sm hover:bg-gray-100 ${selected ? "bg-primary/10 text-primary" : ""}`} style={{ paddingLeft: 10 + depth * 18 }}>
                        {item.isFolder ? (
                        ...
                        {item.isFolder && <span className="text-xs text-gray-400">{folderStats[key] ?? item.successFileCount ?? 0}</span>}
                    </div>
    ...
    const renderSource = (source: SourceItem) => {
        ...
        return (
            <div key={key} className="border-b last:border-b-0">
                <div className={`flex items-center gap-2 px-2 py-2 text-sm hover:bg-gray-100 ${selected ? "bg-primary/10 text-primary" : ""}`}>
    ...
    return (
        <div className="mb-2 rounded-lg border border-border bg-background p-2 shadow-sm">
            <div className="mb-2 flex items-center justify-between">
                <div>
                    <div className="text-sm font-medium">自选知识范围</div>
                    <div className="text-xs text-gray-400">整库限选 1 个，文件最多 20 个</div>
                </div>
                <div className={`text-xs ${overLimit ? "text-red-500" : "text-gray-400"}`}>已选范围：{selectedText}</div>
            </div>
            <div className="relative mb-2">
                <Search className="absolute left-2 top-2.5 h-4 w-4 text-gray-400" />
                <input className="h-9 w-full rounded-md border bg-background pl-8 pr-2 text-sm outline-none" value={keyword} disabled={disabled} placeholder="文件名搜索" onChange={(event) => setKeyword(event.target.value)} />
            </div>
            <div role="tablist" aria-label="知识来源类型" className="mb-2 grid grid-cols-2 rounded-md bg-gray-100 p-1">
                <button
                    type="button"
                    role="tab"
                    aria-selected={activeTab === "knowledge"}
                    className={`h-8 rounded text-sm font-medium ${activeTab === "knowledge" ? "bg-background text-primary shadow-sm" : "text-gray-500"}`}
                    disabled={disabled}
                ...
                <button
                    ...
                    className={`h-8 rounded text-sm font-medium ${activeTab === "space" ? "bg-background text-primary shadow-sm" : "text-gray-500"}`}
                    disabled={disabled}
            ...
            <div className="max-h-80 overflow-auto rounded-md border">
                {activeTab === "knowledge" ? (
                    <>
                        {sourceLoading.knowledge && <div className="flex items-center gap-2 p-2 text-sm text-gray-400"><Loader2 className="h-4 w-4 animate-spin" />加载中...</div>}
                        {!sourceLoading.knowledge && knowledgeList.map(renderSource)}
                        {!sourceLoading.knowledge && !knowledgeList.length && <div className="p-2 text-sm text-gray-400">暂无匹配的知识库</div>}
                    </>
                ) : (
                    <>
                        {sourceLoading.space && <div className="flex items-center gap-2 p-2 text-sm text-gray-400"><Loader2 className="h-4 w-4 animate-spin" />加载中...</div>}
                        {!sourceLoading.space && Object.keys(groupedSpaces).map((level) => (
                            <div key={level}>
                                <div className="px-2 py-1 text-xs text-gray-400">{SPACE_LEVEL_LABELS[level] || level}</div>
                                {groupedSpaces[level].map(renderSource)}
                            </div>
                        ))}
                        {!sourceLoading.space && !spaceList.length && <div className="p-2 text-sm text-gray-400">暂无匹配的知识空间</div>}
                    </>
                )}
            </div>
        </div>
