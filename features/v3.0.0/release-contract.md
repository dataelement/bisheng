...
|---------|--------------|------|
| **ApiCredential**（平台自签发凭据：服务账号密钥 `bs-sak-` / 个人访问令牌 `bs-pat-` / 会话派生凭据，同一底座同一张表） | **F049-openapi-auth-baseline** | 拥有凭据的生成、哈希存储、校验、权限位与过期、软撤销与批量撤销、最后使用时间写入。F050 只新增 `bs-pat-` 形态与委托相关属性的取值，不改底座语义；F055 的应用 token 按服务账号密钥形态由本底座签发 |
...
| INV-28 | **平台自签发凭据统一底座**：一切平台自签发凭据（服务账号密钥、个人访问令牌、应用 token、会话派生凭据）必须密码学随机生成、服务端仅存单向哈希（不用可逆加密）、明文仅签发时一次性展示、此后一切界面只见掩码（前缀 + 末四位）、撤销为软撤销且保留审计记录、撤销生效上界 5 秒（主动清缓存，不依赖 TTL 自然过期）。有效性判据唯一 = 未撤销且未过期，不得引入可独立漂移的状态枚举列 | ApiCredential | F049 |
...
| F050-identity-modes-and-personal-key | F049 | **伴生 PRD 的 P1 阶段** + PRD-1 DEV-01：R2 三种身份模式 + R3 受限委托六道准入 + R7 审计双归属 + 个人访问令牌 `bs-pat-` + 裸 `user_id` 参数收口；DEV-01 个人 key 与 API key 管理页个人 tab、「我的 API key」；GOV-07 开放能力层部署 flag 与入口隐藏。个人 key 与开放 API 个人访问令牌是否同一凭据实例 = PRD-1 §6 开放问题 3，本 spec 定夺 |
