...
- **目标**：让 license 到期从「Gateway 进程 `System.exit(0)` 硬停服」改为「降级模式（拦业务、放行登录）+ 管理后台常驻 Banner 提示」，给管理员留出感知与续期窗口。
...
- **原因**：硬退出同样导致「全线不可达、无从解释」，与本 feature 目标相悖。配置缺失视作无有效授权 → 进入降级、severity 记为 `expired`（有业务拦截意义）或 `unknown`（无 daysRemaining 时）；具体取 `expired`（无有效授权应拦业务）。
...
**降级错误体** —— 业务路由被拦时返回（沿用 Gateway 现有响应风格，HTTP 200 + 业务错误码，参照 `PathRateGlobalFilter` 限流返回）：
```json
{ "status_code": <license过期错误码>, "status_message": "软件授权已过期，请联系管理员续期" }
```
...
| `LicenseExpiredGlobalFilter.java`（新，GlobalFilter+Ordered） | expired 时拦业务、放行白名单 | 不做限流 / 鉴权 |
...
- GlobalFilter：`PathRateGlobalFilter`（order `LOWEST_PRECEDENCE-90`，按 path 命中后短路返回）—— `LicenseExpiredGlobalFilter` 仿此，order 取更高优先级（更早拦截）。
- Gateway 路由：`/api/v1/**`、`/api/v2/**` 默认代理后端；自处理路径需确保不落入代理（`/api/license/*` 不在 `/api/v1` 前缀下，天然由 Controller 接管）。

**降级白名单**（放行，其余 `/api/v1/**`、`/api/v2/**` 业务拦截）：登录 / 鉴权相关（`/api/oauth2/**`、`/api/getkey`、登录与用户信息路由）、`/api/license/status`、健康检查（actuator）、静态资源。白名单以前缀列表实现，必要时可配置化。
...
| 3 | 状态接口必须在降级过滤器白名单内 | 过期后 Banner 取不到状态，反而看不到过期提示 | `LicenseExpiredGlobalFilter` 白名单含 `/api/license/status` |
...
- **Gateway 单元测试**：daysRemaining/severity 映射（含 30/7/0 边界）；`LicenseExpiredGlobalFilter` 白名单放行 vs 业务拦截；配置异常进入降级而非退出。
- **手动验证**：构造一张已过期 trial license（或改系统时间 / mock `LicenseLoader`），启动 Gateway → 确认进程不退出、业务路由返错误体、登录与 `/api/license/status` 可访问；超管登录 platform 看到红色置顶 Banner；改成 20 天后到期看黄色 warning。
