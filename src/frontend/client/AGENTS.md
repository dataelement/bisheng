...
- **Frosted glass exception switch**: the root §4 ban stands; the one sanctioned exception (a full-screen overlay, at most one at a time) goes through `~/utils/frostedGlass` — `cn(FROSTED_GLASS_ENABLED && FROSTED_GLASS_CLASS)`. `backdrop-filter` is written ONLY in `style.css` under `.bs-frosted-glass`; never add it to a component. `VITE_FROSTED_GLASS=false` turns every instance off for 信创 deployments. Currently used by: knowledge-space drag-upload overlay (`pages/knowledge/index.tsx`).
