            ...
            [
                {
                    ...
                    "updated_by": 90,
                    "version": 3,
                    "model_configs": [{"model_id": model, "monthly_token_limit": 900}],
                }
...
def test_users_with_and_without_policy_are_listed_independently_of_dsh_sessions(model_access_db):
    with Session(model_access_db) as session, profile_scope(2):
        ...
        first = repository.users(UserDshProfileRepository.access_users(session, limit=1), limit=1)
        assert first["items"] == [
            {
                "user_id": 20,
        ...
        second = repository.users(UserDshProfileRepository.access_users(session, after_user_id=20), limit=20)
        assert [row["user_id"] for row in second["items"]] == [21, 26]
        assert second["items"][0]["models"] == [{"model_id": 7, "monthly_token_limit": 900}]
        assert not second["has_more"] and second["next_cursor"] is None
        assert (
            repository.users(UserDshProfileRepository.access_users(session, keyword="first-login"))["items"]
            == first["items"]
        )
        assert [
            row["user_id"]
            for row in repository.users(UserDshProfileRepository.access_users(session, keyword="%"))["items"]
        ] == [26]
    with Session(model_access_db) as session, profile_scope(3):
        assert [
            row["user_id"]
            for row in DshModelAccessRepository(session).users(UserDshProfileRepository.access_users(session))["items"]
        ] == [22]
...
async def test_model_users_service_authorizes_tenant_before_any_query():
    ...
    async def view(model_id, **kwargs):
        ...
        assert model_id == 7 and kwargs == {"after_user_id": 20, "limit": 10, "keyword": "test"}
        return {"tenant_id": 2}
...
async def test_model_users_route_uses_verified_actor_and_validates_query():
    ...
    async with httpx.AsyncClient(transport=httpx.ASGITransport(app=app), base_url="http://test") as client:
        ...
        service.model_users.assert_awaited_once_with(90, 7, tenant_id=2, cursor=None, limit=20, keyword="first")
