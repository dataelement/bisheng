...
def _builtin_config() -> BrandConfig:
    ...
    config.loading.icon = BrandAsset(
        url=DEFAULT_LOADING_ICON_URL, relative_path='', file_name=''
    )
...
class BrandService:
    ...
    async def list_asset_options(self, category: BrandAssetCategory) -> list[BrandAssetOption]:
        ...
        def sort_key(item: Any) -> datetime:
            return getattr(item, 'last_modified', None) or datetime.min.replace(tzinfo=timezone.utc)
    ...
    def _merge_with_defaults(self, incoming: BrandConfig | BrandConfigUpdate) -> BrandConfig:
        ...
        merged.loading.iconOptions = [
            BrandAsset.model_validate(option)
            for option in loading.get('iconOptions') or []
            if option
        ]
    ...
    async def _reset_saved_asset_if_deleted(
        ...
        if category == 'loadingIcon':
            ...
            if normalized.loading.icon and normalized.loading.icon.relative_path == object_name:
                ...
                changed = True
            next_options = [
                option for option in normalized.loading.iconOptions
                if option.relative_path != object_name
            ]
        ...
        setattr(
            normalized.assets,
            category,
            BrandAsset(url=default_asset.url, relative_path=default_asset.relative_path, file_name=default_asset.file_name),
        )
        normalized.URLLoadingIcon = self._resolve_loading_icon_url(normalized)
        await self.repository.upsert_value(
            BRAND_CONFIG_KEY,
