...
@pytest.mark.asyncio
async def test_filter_knowledge_ids_by_permission_async_ignores_knowledge_space_tuples():
    service = KnowledgePermissionService()
    login_user = SimpleNamespace(
        user_id=7,
    ...
    with (
        ...
        assert await service.filter_knowledge_ids_by_permission_async(login_user, [13], "view_kb") == []
        assert {call.kwargs["object"] for call in fake_fga.read_tuples.await_args_list} == {"knowledge_library:13"}
