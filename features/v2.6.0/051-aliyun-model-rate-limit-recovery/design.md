...
>
> 本版是 2026-08-28 的完整重写，覆盖此前 execution 聚合器、任务 429 特殊旁路等旧设计。
> 当前未提交代码和旧测试必须重新按本文核对，不能作为设计正确性的依据。
> tasks.md 和 release-contract.md 已按本版同步；当前未提交代码和旧测试仍必须逐项重新核验。

**关联**: [spec.md](./spec.md) · [tasks.md](./tasks.md)  
**版本**: v2.6.0  
**状态**: ✅ 最终设计已确认（2026-08-28）  
**最后更新**: 2026-08-28
...
5. 任务模式的内部自动重试、失败处理和用户 Retry 全部保持原样。
6. F051 任一附加能力失败时，原模型异常和业务链路仍按旧逻辑工作。
...
| 任务用户 Retry | 既有 <code>continueConversation(sessionVersionId, question)</code> | 复用成功调用观察以清 Redis 状态 | 新增 task recovery API、command、attempt 或 queue 字段 |
...
状态和 UI <code>error_type=rate_limit</code>，不负责控制任务执行。
...
- **日常 Agent**
  - 首次请求先保存用户消息。
  - 限流不保存失败回答。
  - 使用以原用户消息 ID 隔离的 Redis checkpoint，在模型节点存在安全 pending 状态时继续，避免重放已完成工具和外部副作用。
  - checkpoint 缺失、过期或图状态不可安全继续时返回恢复拒绝，不从头静默执行。
- **知识空间 file/folder 与订阅频道**
  - 首次请求先保存用户消息。
  - 检索和生成链路为只读时，从原 ChatMessage 读取问题，在同一页面位置重新调用。
  - 限流不保存失败回答；成功只写一条正常回答，不增加第二条用户消息。
- **任务模式**
  - 内部自动重试、最终失败处理、前端 Retry 全部沿用旧逻辑。
  - 不接入公共 recovery command，不新增 checkpoint 分支。
- **原因**：恢复方式必须服从入口既有业务副作用边界，不能为了“统一”强迫四入口共享同一种执行状态机。
- **何时重新考虑**：某入口新增可写工具或外部副作用时，应先在该入口建立自身幂等/checkpoint，再调整恢复方式。
...
### 4.2 任务模式 429 调用链
    ...
    Linsight middleware 调用 handler
      ...
      -> 用户点击 Retry
           -> 既有 continueConversation(sessionVersionId, question)
...
### 4.4 非任务入口的主动恢复
    ...
    当前页面收到 rate_limit SSE
      ...
      -> 用户点击 Retry 或确认换模
           ...
           -> 获取 5 秒短锁
           -> 日常：只从安全 checkpoint pending 节点继续
              RAG：按原问题只读重新调用
           -> SSE 结果携带当前 attempt_id
           -> 当前页面只接受仍在等待的 attempt
           -> 成功按旧格式写一条正常回答

恢复校验失败、短锁未获准、checkpoint 不可恢复均返回 12048/recovery_rejected；该结果结束当前 attempt，
...
| 3 | 任务有内部自动 retry、失败落档、用户 Retry 三层机制 | “沿用旧重试”被实现成只保留其中一层 | §2.3 和 §4.2 分阶段固定行为 |
...
任务模式不提供上述 API，继续使用既有 <code>continueConversation</code> 和 <code>continue_question</code>。
