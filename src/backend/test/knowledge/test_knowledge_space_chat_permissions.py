...
class TestKnowledgeSpaceChatPermissions:
    ...
    @pytest.mark.asyncio
    async def test_get_chat_folder_session_bypasses_only_session_lookup(self, chat_service):
        permission_service = SimpleNamespace(
            _is_read_permission_bypassed=MagicMock(return_value=True),
            _get_space_or_raise=AsyncMock(),
            _get_folder_for_action=AsyncMock(return_value=_make_file(file_id=22, file_type=FileType.DIR.value)),
        )
        ...
        assert result == []
        permission_service._get_space_or_raise.assert_awaited_once_with(1)
        permission_service._get_folder_for_action.assert_awaited_once_with(1, 22)
        mock_require_folder.assert_not_awaited()
