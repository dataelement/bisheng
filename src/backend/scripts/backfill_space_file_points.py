...
4. 忽略账号：匹配 user.user_name，命中忽略账号（如 admin）的文件跳过不发分。
5. 积分规则：突破单日 15 分 daily_cap 限制全额累加，按文件 ID 生成强幂等键防重跑。
6. 支持 --dry-run 演练预览模式。
"""
...
async def fetch_ignore_user_ids(
    session,
    tenant_id: int,
...
async def fetch_eligible_main_files(
    ...
    doc_ids = {int(f.reference_document_id) for f in all_files if getattr(f, "reference_document_id", None) is not None}
    ...
    if doc_ids:
        ...
        for doc_id, _prim_ver_id, phys_file_id, lifecycle_status in doc_res.all():
...
def group_files_by_payee(
    ...
    for f in files:
        ...
        # 检查是否属于忽略账号
        if payee_id in ignore_user_ids:
...
def print_report(summary: BackfillSummary, dry_run: bool) -> None:
    ...
    print(f" - 命中忽略账号跳过:   {summary.ignored_user_files}")
    print("-" * 60)
    if dry_run:
    ...
    for _uid, u in sorted(summary.users_affected.items(), key=lambda x: x[1]["file_count"], reverse=True):
        print(
            f"{u['user_id']:<10} "
            f"{u['user_name']:<20} "
            f"{u['file_count']:<8} "
...
async def run(args: argparse.Namespace) -> int:
    ...
    ignore_accounts = [acc.strip() for acc in (args.ignore_accounts or "").split(",") if acc.strip()]
    logger.info("开始执行目标库文件补发积分...")
    logger.info(
        "参数配置: 目标级别=%s, 单文件积分=%d, 忽略账号=%s, DryRun=%s, 租户ID=%d",
        target_level,
        args.score_per_file,
        ignore_accounts,
        args.dry_run,
    ...
    async with get_async_db_session() as session:
        ...
        # 2. 查忽略账号的 user_ids
        ignore_user_ids = await fetch_ignore_user_ids(session, args.tenant_id, ignore_accounts)
        logger.info("识别到忽略账号 ID 列表: %s", list(ignore_user_ids))

        # 3. 查有效主文件
        eligible_files = await fetch_eligible_main_files(session, args.tenant_id, space_ids, summary)
        logger.info(
            "扫描完成: 总文件数=%d, 排除目录=%d, 排除分享=%d, 排除历史版本=%d, 有效主文件数=%d",
            summary.total_files_scanned,
            summary.excluded_dirs,
            summary.excluded_shares,
            summary.excluded_history_versions,
            summary.eligible_main_files,
        )

        # 4. 确定受让人并按用户分组
        payee_files = group_files_by_payee(eligible_files, ignore_user_ids, summary)
        logger.info("受让人分组完成: 待发分用户数=%d, 忽略账号文件数=%d", len(payee_files), summary.ignored_user_files)
...
def main():
    parser = argparse.ArgumentParser(
        description="根据目标类型库下所有有效主文件给原始上传人批量增加指定积分",
