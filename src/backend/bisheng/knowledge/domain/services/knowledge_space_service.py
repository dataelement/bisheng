...
from bisheng.knowledge.domain.schemas.knowledge_space_schema import (
    ...
    ShougangPortalQaCategoryFileRefsReq,
...
@dataclass(frozen=True)
class FavoriteDeletePlan:
    expanded_file_ids: list[int]
...
class PortalDiscoveryResult:
    discoverable_space_ids: list[int]
    explicitly_visible_space_ids: list[int]
    explicitly_visible_file_ids: list[int]
    explicit_file_space_by_id: dict[int, int]
    grant_parent_space_ids: list[int]
...
class KnowledgeSpaceService(KnowledgeUtils):
    ...
    def __init__(self, request: Request, login_user: UserPayload):
        ...
        self._portal_discovery_result: PortalDiscoveryResult | None = None
        self._portal_explicit_file_ids: set[int] = set()
        self._portal_grant_parent_space_ids: set[int] = set()
        self._portal_space_kind_map: dict[int, str] = {}
        # 登录检索阶段不解析部门文件权限；这些文件在详情、收藏或下载时再校验。
        self._portal_unchecked_department_file_ids: set[int] = set()
    ...
    def _ensure_container_has_no_distribution_entries(
        ...
        if any(
            (
                ...
                or file.entry_type == KnowledgeFileEntryType.PROJECTION_TOMBSTONE.value
            )
    ...
    async def _enqueue_document_distribution_projection(
        ...
        *,
        ...
        try:
            ...
            logger.exception(
                "F059 projection enqueue failed after lifecycle transition: tenant_id=%s entry_ids=%s",
                tenant_id,
                entry_ids,
            )
    ...
    async def _handle_distribution_file_delete(
        ...
        try:
            ...
            if file_record.entry_type == KnowledgeFileEntryType.MANAGER.value:
                result = await self.document_distribution_service.delete_manager(
                    tenant_id=int(file_record.tenant_id),
                    document_id=int(file_record.reference_document_id),
    ...
    def _require_distribution_manager(
        ...
        if (
            ...
            or file_record.entry_type != KnowledgeFileEntryType.MANAGER.value
            or file_record.entry_status != KnowledgeFileEntryStatus.ACTIVE.value
        ):
    ...
    async def list_document_share_entries(
        ...
        *,
        ...
        entries = await self.document_distribution_service.file_repository.find_distribution_entries_by_document_id(
        ...
        shares = [entry for entry in entries if entry.entry_type == KnowledgeFileEntryType.SHARE.value]
        spaces = await KnowledgeDao.aget_list_by_ids(sorted({int(entry.knowledge_id) for entry in shares}))
        space_names = {int(space.id): str(space.name or "") for space in spaces}
        data = [
            {
                "entry_id": int(entry.id),
                "target_space_id": int(entry.knowledge_id),
                "target_space_name": space_names.get(int(entry.knowledge_id)),
                "allow_download": bool(entry.allow_download),
                "entry_status": str(entry.entry_status),
                "create_time": (entry.create_time.isoformat() if entry.create_time is not None else None),
            }
            for entry in shares
        ]
    ...
    async def revoke_document_share(
        ...
        *,
        ...
        try:
            result = await self.document_distribution_service.remove_share_entry(
    ...
    async def _resolve_document_entry(
        ...
        *,
        ...
        if required_capability and not bool(getattr(resolved.capabilities, required_capability, False)):
    ...
    async def _require_document_content_manager(
        ...
        if (
            ...
            return None
        if getattr(file_record, "entry_type", None) != KnowledgeFileEntryType.MANAGER.value:
            raise KnowledgeDocumentManagerRequiredError()

        resolved = await self._resolve_document_entry(file_record)
        if (
            resolved is None
            or resolved.entry_type != KnowledgeFileEntryType.MANAGER.value
    ...
    async def resolve_shougang_portal_download_content(
        ...
        *,
        ...
        _, content = await self.resolve_shougang_portal_download_reference(
            space_id=space_id,
            file_id=file_id,
        )
        return content
        ...
        *,
        ...
        if self.document_durable_reference_resolver is None:
            ...
            resolved = await self._resolve_document_entry(
                entry,
                required_capability=(None if external_share_grant else "can_download"),
            )
        else:
            try:
                resolved = await self.document_durable_reference_resolver.resolve(
                    tenant_id=int(self.login_user.tenant_id),
                ...
                raise SpaceFileNotFoundError() from exc
            entry = await KnowledgeFileDao.query_by_id(int(resolved.entry_file_id))
            if entry is None:
                raise SpaceFileNotFoundError()
            if not external_share_grant and not resolved.capabilities.can_download:
                raise KnowledgeDocumentDownloadDeniedError()
        ...
        content = (
            ...
            else await KnowledgeFileDao.query_by_id(int(resolved.content_file_id))
        )
        if content is None:
            raise SpaceFileNotFoundError()
    ...
    def _expand_file_sync_folder_visibility(
        ...
        for folder in selectable_folders:
            ...
            visible_ids.update(
                int(part) for part in str(folder.file_level_path or "").split("/") if part.isdigit() and int(part) > 0
            )
        ...
        *,
        ...
        async with get_async_db_session() as session:
            ...
            if parent_id is not None:
                ...
                if (
                    ...
                    or (access.visible_folder_ids is not None and parent_id not in access.visible_folder_ids)
                ):
            ...
            page_rows = list(rows[:page_size])
            child_paths = {
                f"{folder.file_level_path or ''}/{int(folder.id)}" for folder in page_rows if folder.id is not None
            }
        ...
        items = [
            FileSyncTargetFolderItem(
                ...
                selectable=(access.selectable_folder_ids is None or int(folder.id) in access.selectable_folder_ids),
    ...
    async def resolve_file_sync_target_displays(
        ...
        target_folder_ids = {folder_id for _knowledge_id, folder_id in targets.values() if folder_id is not None}
        async with get_async_db_session() as session:
            knowledge_repository = KnowledgeRepositoryImpl(session)
            file_repository = KnowledgeFileRepositoryImpl(session)
        ...
        folder_map = {int(folder.id): folder for folder in [*target_folders, *ancestors] if folder.id is not None}
        ...
        for token_id, (knowledge_id, folder_id) in targets.items():
            ...
            stale = (
                ...
                or any(
                    record is None or int(getattr(record, "knowledge_id", 0) or 0) != knowledge_id
    ...
    async def _create_space_scope(
        ...
        *,
        ...
        portal_discovery_enabled = self._portal_discovery_default_for_scope(
            level=level,
            is_clinic=level == KnowledgeSpaceLevelEnum.TEAM_KS,
        )
        if self.knowledge_space_scope_repo is not None:
            return await self.knowledge_space_scope_repo.save(
                KnowledgeSpaceScope(
                    tenant_id=int(self.login_user.tenant_id),
                    space_id=space_id,
                    level=level,
    ...
    @staticmethod
    def _portal_discovery_default_for_scope(
        *,
        level: KnowledgeSpaceLevelEnum,
        is_clinic: bool,
    ) -> bool:
    ...
    def build_created_space_info(
        ...
        *,
        ...
        result.is_clinic = result.space_level == KnowledgeSpaceLevelEnum.TEAM_KS
        result.portal_discovery_enabled = (
            self._portal_discovery_default_for_scope(
                level=result.space_level,
                is_clinic=result.is_clinic,
            )
    ...
    @staticmethod
    def _portal_discovery_kind(
        scope: KnowledgeSpaceScope | None,
        binding: Any | None,
    ) -> str | None:
        if scope is None:
            return None
    ...
    async def resolve_portal_discovery(
        self,
        *,
        scope: str,
        ...
        valid_department_ids = {
            ...
            if getattr(department, "status", "active") == "active"
            and int(getattr(department, "is_deleted", 0) or 0) == 0
            and int(getattr(department, "tenant_id", tenant_id) or tenant_id) == tenant_id
        }

        space_kind_by_id: dict[int, str] = {}
        configurable_scope_rows: list[tuple[str, int, bool]] = []
        discoverable_space_ids: set[int] = set()
        valid_binding_by_space_id: dict[int, Any] = {}
        scope_by_space_id = {int(item.space_id): item for item in scopes}
        for item in scopes:
            ...
            kind = self._portal_discovery_kind(item, binding)
            if kind is None:
                if item.level == KnowledgeSpaceLevelEnum.TEAM:
                    kind = "team"
                elif item.level == KnowledgeSpaceLevelEnum.PERSONAL:
                    kind = "personal"
                else:
                    logger.warning(
                        "portal discovery ignored invalid scope: tenant_id={} space_id={}",
                        tenant_id,
                        space_id,
                    )
                    continue
            space_kind_by_id[space_id] = kind
        ...
        if scope == "portal_configured":
            ...
            if is_global_admin:
                ...
                closed_public_readable_ids = sorted(
                    space_id
                    for space_id in readable_space_ids - explicit_space_ids
                    if space_kind_by_id.get(space_id) == "public"
                    and not bool(scope_by_space_id[space_id].portal_discovery_enabled)
                )
                if closed_public_readable_ids:
                    explicit_space_ids.update(
                        int(space_id)
                        for space_id in await FineGrainedPermissionService.filter_object_ids_by_explicit_binding_async(
                            self.login_user,
                            "knowledge_space",
                            closed_public_readable_ids,
                        )
                    )
                explicit_space_ids.update(
                    space_id
                    ...
                    if space_kind_by_id.get(space_id) != "public"
                    or bool(scope_by_space_id[space_id].portal_discovery_enabled)
                )

            access_service = self.department_file_view_access_service
            grant_repository = getattr(access_service, "grant_repository", None)
            if grant_repository is not None:
                grants = await grant_repository.list_active_by_user(
                    tenant_id=tenant_id,
                    user_id=int(self.login_user.user_id),
                )
                candidate_grants = []
                for grant in grants:
                    parent_space_id = int(grant.space_id)
                ...
                if candidate_grants and access_service is not None:
                    ...
                    for grant in candidate_grants:
                        ...
                        if (
                            file is None
                            or int(getattr(file, "knowledge_id", 0) or 0) != parent_space_id
                            or getattr(decision, "status", None) != "allowed"
                        ):
                            continue
                        explicit_file_ids.add(file_id)
        ...
        snapshot = self.knowledge_space_scope_repo.build_discovery_snapshot(
            ...
            explicit_file_ids=sorted(explicit_file_ids),
        )
        result = PortalDiscoveryResult(
            discoverable_space_ids=sorted(discoverable_space_ids),
            explicitly_visible_space_ids=sorted(explicit_space_ids),
            explicitly_visible_file_ids=sorted(explicit_file_ids),
            explicit_file_space_by_id=dict(sorted(explicit_file_space_by_id.items())),
            grant_parent_space_ids=sorted(grant_parent_space_ids),
            query_space_ids=query_space_ids,
    ...
    def _ensure_space_folder(folder: KnowledgeFile | None, space_id: int) -> KnowledgeFile:
        if (
            ...
            or getattr(folder, "deleted_at", None) is not None
        ):
            raise SpaceFolderNotFoundError()
        return folder
    ...
    def _ensure_space_file(
        ...
        *,
        ...
        if not file_record or file_record.knowledge_id != space_id:
            raise SpaceFileNotFoundError()
        if getattr(file_record, "deleted_at", None) is not None:
            raise SpaceFileNotFoundError()
    ...
    async def _list_space_child_resources(self, space_id: int) -> list[tuple[str, int]]:
        ...
        return [
            ("folder", int(file.id)) if file.file_type == FileType.DIR.value else ("knowledge_file", int(file.id))
            for file in files
        ]
    ...
    def _favorite_ref_meta(
        ...
        if canonical_document_id is not None:
            reference["canonical_document_id"] = int(canonical_document_id)
    ...
    async def _resolve_favorite_durable_target(
        ...
        *,
        ...
        return (
            ...
            (int(resolved.canonical_document_id) if resolved.canonical_document_id is not None else None),
        )
    ...
    async def _find_favorite_reference(
        ...
        for row in rows:
            ...
            stored_space_id = int(meta.get("source_space_id") or 0)
            stored_file_id = int(meta.get("source_file_id") or 0)
            if stored_space_id == int(source_space_id) and stored_file_id == int(source_file_id):
                return row
            if canonical_document_id is None or stored_space_id != int(source_space_id):
                continue
            stored_document_id = (
                int(meta["canonical_document_id"]) if meta.get("canonical_document_id") is not None else None
            )
            if stored_document_id is None and stored_file_id > 0:
                _, stored_document_id = await self._resolve_favorite_durable_target(
    ...
    async def create_shougang_portal_favorite(
        ...
        if not source_space or source_space.type != KnowledgeTypeEnum.SPACE.value:
            raise SpaceNotFoundError()
        source_file_id, canonical_document_id = await self._resolve_favorite_durable_target(
            source_space_id=req.source_space_id,
            source_file_id=req.source_file_id,
            require_view_permission=False,
        )
        source_file, _ = await self._get_authorized_shougang_portal_file(
    ...
    async def list_shougang_portal_favorites(
        ...
        for ref in rows:
            ...
            current_file_id, _ = await self._resolve_favorite_durable_target(
            ...
            current_file = None
            if current_file_id:
                current_file = await KnowledgeFileDao.query_by_id(current_file_id)
                alive = (
                    current_file is not None
                    and int(current_file.knowledge_id) == src_space
                    and getattr(current_file, "deleted_at", None) is None
                )
            data.append(
                ShougangPortalFavoriteFileItem(
                    favorite_file_id=int(ref.id),
                    source_space_id=src_space,
                    source_file_id=(current_file_id if alive else src_file),
                    title=Path((current_file.file_name if current_file is not None else ref.file_name) or "").stem,
                    file_name=str((current_file.file_name if current_file is not None else ref.file_name) or ""),
                    status="valid" if alive else "invalid",
                    invalid_reason=None if alive else "source_deleted",
    ...
    async def _get_department_file_share_access_decision(
        self,
        source_file: KnowledgeFile,
    ):
        access_service = self.department_file_view_access_service
        if access_service is None:
            scope = await KnowledgeSpaceScopeDao.aget_by_space_id(int(source_file.knowledge_id))
            if self._enum_value(getattr(scope, "level", "")) == KnowledgeSpaceLevelEnum.DEPARTMENT.value:
                raise RuntimeError("DepartmentFileViewAccessService 未注入")
            return None
        decision = await access_service.evaluate_file(
            login_user=self.login_user,
    ...
    async def get_shougang_portal_share_link_meta(
        ...
        share_token: str,
        *,
        for_anonymous_portal: bool = False,
    ) -> ShougangPortalShareLinkMetaResp:
        ...
        if for_anonymous_portal:
            guarded_marker = meta_data.get("department_file_view_guarded")
            guarded = guarded_marker is True
            if guarded_marker is None:
                source_file = await KnowledgeFileDao.query_by_id(int(meta_data.get("file_id") or 0))
                if source_file is not None:
                    guarded = (await self._get_department_file_share_access_decision(source_file)) is not None
            if guarded:
                raise KnowledgeDepartmentShareLoginRequiredError()
    ...
    async def verify_shougang_portal_share_link(
        ...
        if self.document_durable_reference_resolver is not None:
            ...
            try:
                durable_reference = await self.document_durable_reference_resolver.resolve(
                    tenant_id=int(self.login_user.tenant_id),
        ...
        if meta_data.get("department_file_view_guarded") is not False:
            current_entry_file_id = int(getattr(durable_reference, "entry_file_id", file_id))
            source_file = await KnowledgeFileDao.query_by_id(current_entry_file_id)
        ...
        if department_decision is not None:
            permissions = ShougangPortalSharePermissions(
                view=True,
                download=False,
                upload=False,
            )
        ...
        return ShougangPortalShareLinkAccessResp(
            ...
            space_id=space_id,
            file_id=file_id,
            entry_file_id=(int(durable_reference.entry_file_id) if durable_reference is not None else file_id),
            canonical_document_id=(
                int(durable_reference.canonical_document_id) if durable_reference is not None else None
            ),
            canonical_version_id=(
                int(durable_reference.canonical_version_id)
                if durable_reference is not None and durable_reference.canonical_version_id is not None
                else None
            ),
    ...
    async def require_shougang_portal_share_download(
        ...
        *,
        ...
        try:
            ...
            source_file = None
            if self.document_durable_reference_resolver is not None:
                source_file, _ = await self.resolve_shougang_portal_download_reference(
                    space_id=space_id,
                    file_id=file_id,
    ...
    async def search_shougang_portal_tags(
        ...
        *,
        ...
        if not spaces:
            return []
        tag_map = await TagLibraryTagService.collect_space_portal_tag_map([int(space.id) for space in spaces])
    ...
    async def count_shougang_portal_files(
        ...
        if req.query_type == "advanced":
            advanced_payload = req.model_dump(
                exclude={"query_type", "q", "filter_tag", "retrieval_profile"},
                exclude_unset=True,
            )
            ...
            return [
                {
                    ...
                    "space_level": (
                        ...
                        if discovery.space_kind_by_id.get(int(space.id)) == "clinic"
                        else discovery.space_kind_by_id.get(int(space.id), "personal")
                    ),
                    "is_clinic": discovery.space_kind_by_id.get(int(space.id)) == "clinic",
                    "grant_only": (
                        int(space.id) in grant_parent_ids
                        and int(space.id) not in discoverable_ids
                        and int(space.id) not in explicit_space_ids
                    ),
                }
                for space in spaces
            ]
        ...
        return [
            {
                ...
                "space_level": (
                    ...
                    if int(space.id) in department_space_ids
                    else KnowledgeSpaceLevelEnum.PUBLIC.value
                ),
            }
            for space in spaces
        ]
    ...
    async def _get_shougang_portal_home_sections(self, req: ShougangPortalHomeReq) -> dict:
        ...
        space_ids = [int(space.id) for space in spaces]
        tag_map = await TagLibraryTagService.collect_space_portal_tag_map(space_ids)
    ...
    async def _resolve_shougang_portal_advanced_search_spaces(
        ...
        elif req.public_only:
            spaces = await self._get_shougang_portal_public_search_spaces(req.space_ids)
    ...
    async def _load_qa_category_space_metadata(self, space_ids: list[int]) -> list[tuple[Knowledge, str | None]]:
        async with get_async_db_session() as session:
            repository = KnowledgeRepositoryImpl(session)
            rows = await repository.find_qa_spaces_by_ids(space_ids)
            found = {int(space.id) for space, _ in rows}
            missing = sorted(set(space_ids) - found)
            # 关联表的租户条件可能漏掉无范围记录的旧库；仅补查缺失项，不将其视为公共库。
            for start in range(0, len(missing), 500):
                for space in await repository.find_by_ids(missing[start:start + 500]):
                    if space.type == KnowledgeTypeEnum.SPACE.value and space.state != KnowledgeState.DELETING.value:
                        rows.append((space, None))
            return rows

    async def _resolve_qa_category_spaces(
        self, space_ids: list[int], discovery_scope: str,
    ) -> tuple[dict[int, Knowledge], set[int]]:
        if not space_ids:
            return {}, set()
        rows = await self._load_qa_category_space_metadata(list(dict.fromkeys(space_ids)))
        requested = set(space_ids)
        public = {int(space.id): space for space, level in rows
                  if int(space.id) in requested and level == KnowledgeSpaceLevelEnum.PUBLIC.value}
        if discovery_scope == "public":
            return public, set(public)
        private_ids = [int(space.id) for space, _ in rows
                       if int(space.id) in requested and int(space.id) not in public]
        # 先实时鉴权，避免旧可见性解析器吞掉上游异常。
        readable_ids = [sid for sid in private_ids if await self._can_read_qa_category_space(sid)]
        private_spaces = await self._get_shougang_portal_request_spaces(
            requested_space_ids=readable_ids, space_level=None, discovery_scope=discovery_scope,
        ) if readable_ids else []
        allowed_ids = set(readable_ids)
        return {**public, **{int(space.id): space for space in private_spaces
                             if int(space.id) in allowed_ids}}, set(public)

    async def _filter_qa_category_visible_files(
        self, files: list[KnowledgeFile], spaces: dict[int, Knowledge], public_ids: set[int],
    ) -> list[KnowledgeFile]:
        by_space: dict[int, list[KnowledgeFile]] = {}
        for file in files:
            if int(file.knowledge_id) in spaces:
                by_space.setdefault(int(file.knowledge_id), []).append(file)
        visible: list[KnowledgeFile] = []
        public_permissions = default_permission_ids_for_relation("viewer")
        for space_id, items in by_space.items():
            if space_id in public_ids:
                # 公共库默认读取不依赖用户的 FGA 关系。
                for file in items:
                    self._entry_permission_ids_by_file[int(file.id)] = set(public_permissions)
                    self._portal_file_download_map[int(file.id)] = "download_file" in public_permissions
                visible.extend(items)
            else:
                visible.extend(await self._filter_visible_child_items(items, space_id=space_id))
        return visible
    ...
    async def _load_qa_category_files(
        ...
        if not req.space_ids:
            return [], {}
        scoped = bool(req.document_type or req.file_subcategory_code)
        if scoped:
            if self.knowledge_file_repo is None:
                raise RuntimeError("Knowledge file repository is not initialized")
            files = await self.knowledge_file_repo.list_qa_category_candidates(
                space_ids=req.space_ids, document_type=req.document_type,
                file_subcategory_code=req.file_subcategory_code, before_id=None, limit=None,
            )
            ...
            requested = set(req.space_ids)
            files = [file for file in files if int(file.knowledge_id) in requested]
            matched_ids = list(dict.fromkeys(int(file.knowledge_id) for file in files))
            spaces, public_ids = await self._resolve_qa_category_spaces(matched_ids, req.discovery_scope)
        else:
            # 保留无分类条件的旧统计查询契约。
            spaces, public_ids = await self._resolve_qa_category_spaces(req.space_ids, req.discovery_scope)
            if not spaces:
                return [], {}
            files = await KnowledgeFileDao.aget_file_by_space_filters(
                knowledge_ids=list(spaces), status=[KnowledgeFileStatus.SUCCESS.value],
                order_by="update_time", order_sort="desc",
            )
        if not spaces:
            return [], {}
        excluded_ids: set[int] = set()
        if self.version_repo is not None:
            excluded_ids.update(await self.version_repo.find_non_primary_file_ids_by_knowledge_ids(list(spaces)) or [])
        if not scoped:
        ...
        candidates = [file for file in files
                      if int(file.id) not in excluded_ids
                      and int(file.file_type) == FileType.FILE.value
                      and int(file.status) == KnowledgeFileStatus.SUCCESS.value]
        visible = await self._filter_qa_category_visible_files(candidates, spaces, public_ids)
        return visible, {sid: str(space.name or sid) for sid, space in spaces.items()}

    async def _scan_qa_category_page(
        self, req: ShougangPortalQaCategoryFilesReq | ShougangPortalQaCategoryFileRefsReq,
    ) -> tuple[list[KnowledgeFile], dict[int, Knowledge], bool, str | None]:
        if not req.space_ids:
            return [], {}, False, None
        if self.knowledge_file_repo is None:
            raise RuntimeError("Knowledge file repository is not initialized")
        checked_spaces: set[int] = set()
        spaces: dict[int, Knowledge] = {}
        ...
        requested = set(req.space_ids)
        exhausted = False
        # 权限拒绝较多时限制单次扫描量，由游标继续扫描。
        for _ in range(5):
            candidates = await self.knowledge_file_repo.list_qa_category_candidates(
                space_ids=req.space_ids, document_type=req.document_type,
                file_subcategory_code=req.file_subcategory_code, before_id=before_id, limit=batch_size,
            )
            if not candidates:
                exhausted = True
                break
            ...
            matched = [file for file in matched if int(file.knowledge_id) in requested]
            new_ids = list(dict.fromkeys(int(file.knowledge_id) for file in matched
                                         if int(file.knowledge_id) not in checked_spaces))
            if new_ids:
                new_spaces, new_public = await self._resolve_qa_category_spaces(new_ids, req.discovery_scope)
                checked_spaces.update(new_ids)
                spaces.update(new_spaces)
                public_ids.update(new_public)
                if new_spaces and self.version_repo is not None:
                    excluded_ids.update(
                        await self.version_repo.find_non_primary_file_ids_by_knowledge_ids(list(new_spaces)) or [])
            current = [file for file in matched if int(file.id) not in excluded_ids]
            allowed = await self._filter_qa_category_visible_files(current, spaces, public_ids)
        ...
        has_more = len(visible) > req.page_size or not exhausted
        cursor = str(page[-1].id if len(visible) > req.page_size else before_id) if has_more else None
        return page, spaces, has_more, cursor

    async def _get_qa_category_page(self, req: ShougangPortalQaCategoryFilesReq) -> dict:
        page, spaces, has_more, cursor = await self._scan_qa_category_page(req)
        result = {"counts": {}, "data": [], "has_more": has_more, "next_cursor": cursor}
        for item in await self._handle_file_folder_extra_info(page) if page else []:
            space_id = int(item["knowledge_id"])
            item["knowledge_name"] = str(spaces[space_id].name or space_id)
    ...
    async def get_shougang_portal_qa_category_file_refs(self, req: ShougangPortalQaCategoryFileRefsReq) -> dict:
        page, _, has_more, cursor = await self._scan_qa_category_page(req)
        refs = []
        for file in page:
            is_department, content_access = self._get_portal_file_access_state(int(file.id))
            if not is_department or content_access == "allowed":
                refs.append({"space_id": int(file.knowledge_id), "file_id": int(file.id)})
        return {"data": refs, "has_more": has_more, "next_cursor": cursor}
    ...
    async def _get_shougang_portal_qa_space(
        self,
        *,
        space_id: int,
        discovery_scope: str,
        ...
        if not spaces:
            raise SpacePermissionDeniedError()
        department_space_ids = await self._get_valid_department_space_ids({int(space_id)})
        return spaces[0], int(space_id) in department_space_ids

    async def _filter_shougang_portal_qa_authorized_files(
        self,
        *,
        files: list[KnowledgeFile],
        is_department_space: bool,
    ) -> tuple[list[KnowledgeFile], dict[int, Any]]:
        valid_files = [
            file
            for file in files
            if int(getattr(file, "file_type", -1)) == FileType.FILE.value
            and int(getattr(file, "status", -1)) == KnowledgeFileStatus.SUCCESS.value
        ]
        if not valid_files or not is_department_space:
            return valid_files, {}
        ...
        authorized = [
            file
            for file in valid_files
            if getattr(decisions.get(int(file.id)), "status", None) == DepartmentFileAccessStatus.ALLOWED
        ]
        return authorized, decisions

    async def _load_shougang_portal_qa_folder_counts(
        self,
        *,
        space_id: int,
        ...
        descendant_groups = await asyncio.gather(
            *(
                SpaceFileDao.get_children_by_prefix(
                    space_id,
        ...
        for descendants in descendant_groups:
            for file in descendants or []:
                if int(getattr(file, "file_type", -1)) == FileType.FILE.value:
                    all_files_by_id[int(file.id)] = file
        authorized_files, _ = await self._filter_shougang_portal_qa_authorized_files(
            files=list(all_files_by_id.values()),
            is_department_space=is_department_space,
        )
        authorized_ids = {int(file.id) for file in authorized_files}

        counts: dict[int, tuple[int, int]] = {}
        for folder, descendants in zip(folders, descendant_groups, strict=True):
            candidate_files = [
                file
                for file in descendants or []
                if int(getattr(file, "file_type", -1)) == FileType.FILE.value
            ...
            counts[int(folder.id)] = (
                len(candidate_files),
                sum(1 for file in candidate_files if int(file.id) in authorized_ids),
            )
        return counts
    ...
    async def list_shougang_portal_qa_children(
        ...
        *,
        ...
        if parent_id is not None:
            ...
            if not self._is_qa_scope_folder(parent, space_id):
                raise SpaceFolderNotFoundError()

        safe_page_size = min(max(int(page_size or 10), 1), 100)
        context = f"shougang_portal_qa_children|scope={discovery_scope}|space={space_id}|parent={parent_id or 0}"
        try:
            decoded = decode_cursor(
                cursor,
                expected_key_len=child_order_cursor_key_len("file_type"),
                expected_context=context,
            )
        except CursorDecodeError as exc:
            raise KnowledgeInvalidCursorError(exception=exc)

        raw_items = await SpaceFileDao.async_list_children(
        ...
        has_more = len(raw_items) > safe_page_size
        page_items = list(raw_items[:safe_page_size])
        page_files = [item for item in page_items if int(item.file_type) == FileType.FILE.value]
        _, decisions = await self._filter_shougang_portal_qa_authorized_files(
            files=page_files,
            is_department_space=is_department_space,
        )
        page_folders = [item for item in page_items if int(item.file_type) == FileType.DIR.value]
        folder_counts = await self._load_shougang_portal_qa_folder_counts(
            space_id=space_id,
        ...
        authorized_files = [
            ...
            if (
                decisions.get(int(item.id)) is None
                or str(decisions[int(item.id)].status) == DepartmentFileAccessStatus.ALLOWED
            )
        ]
        ...
        path_items = [
            {
                ...
                "file_level_path": str(item.file_level_path or ""),
                "file_name": str(item.file_name or ""),
            }
            for item in page_items
        ]
        folder_path_map, _ = await self._resolve_shougang_portal_source_paths(path_items)

        data: list[dict] = []
        for item in page_items:
            item_id = int(item.id)
            is_file = int(item.file_type) == FileType.FILE.value
            decision = decisions.get(item_id)
            content_access = str(decision.status) if decision is not None else "allowed"
            selectable = (
                content_access == DepartmentFileAccessStatus.ALLOWED
                if is_file
                else folder_counts.get(item_id, (0, 0))[1] > 0
            )
            candidate_count, authorized_count = folder_counts.get(
                item_id,
                (0, 0),
            )
            response_item = {
                ...
                "parent_id": parent_id,
                "file_name": str(item.file_name or ""),
                "file_type": int(item.file_type),
                "status": int(item.status or 0),
                "folder_path": folder_path_map.get(item_id, ""),
                "file_ext": self._get_file_ext(str(item.file_name or "")) if is_file else "",
                "selectable": selectable,
                "disabled_reason": ("" if selectable else SHOUGANG_PORTAL_QA_DISABLED_REASON),
            ...
            if is_file and content_access == DepartmentFileAccessStatus.ALLOWED:
                enriched = authorized_metadata_by_id.get(item_id, {})
                for field in (
                    "tags",
                    "summary",
                    "file_size",
                    ...
                    if field in enriched:
                        response_item[field] = enriched[field]
            data.append(response_item)

        next_cursor = None
        if has_more and page_items:
            next_cursor = encode_cursor(
                build_child_order_cursor_key(page_items[-1], "file_type"),
                context=context,
            )
        return {
            "data": data,
    ...
    async def get_shougang_portal_qa_folder_stats(
        ...
        *,
        ...
        folder_map = {int(folder.id): folder for folder in folders if self._is_qa_scope_folder(folder, space_id)}
        if set(folder_map) != set(unique_folder_ids):
            raise SpaceFolderNotFoundError()
        ordered_folders = [folder_map[folder_id] for folder_id in unique_folder_ids]
        counts = await self._load_shougang_portal_qa_folder_counts(
            space_id=space_id,
        ...
        *,
        ...
        if mode == "files":
            file_ids = self._dedupe_ids(
                [
                    self._scope_ref_int(ref, "file_id")
                    for ref in file_refs or []
                    if self._scope_ref_int(ref, "file_id") > 0
                ]
            )
            file_map = {int(file.id): file for file in await KnowledgeFileDao.aget_file_by_ids(file_ids)}
            for ref in file_refs or []:
                ...
                if self.document_durable_reference_resolver is not None and file_id > 0 and space_id > 0:
                    ...
                    try:
                        durable = await self.document_durable_reference_resolver.resolve(
                            tenant_id=int(self.login_user.tenant_id),
                            requested_space_id=space_id,
                            durable_file_id=file_id,
            ...
            folder_ids = self._dedupe_ids(
                [
                    self._scope_ref_int(ref, "folder_id")
                    for ref in folder_refs or []
                    if self._scope_ref_int(ref, "folder_id") > 0
                ]
            )
            folder_map = {int(folder.id): folder for folder in await KnowledgeFileDao.aget_file_by_ids(folder_ids)}
            for ref in folder_refs or []:
                space_id = self._scope_ref_int(ref, "knowledge_space_id")
    ...
    async def get_shougang_portal_file_preview(
        ...
        *,
        ...
        content_file = file
        if resolved is not None and int(resolved.content_file_id) != int(file.id):
            content_file = await KnowledgeFileDao.query_by_id(int(resolved.content_file_id))
            ...
            detail.update(
                {
                    ...
                    "manager_file_id": (
                        int(resolved.manager_file_id) if resolved.capabilities.can_edit_content else None
                    ),
                    ...
                    "capabilities": (resolved.capabilities.model_dump()),
                    "can_download": (resolved.capabilities.can_download),
                }
            )
        return detail
        ...
        *,
        ...
        for hit in hits:
            ...
            if chunk_text.startswith("{<file_title>"):
                ...
                chunk_text = chunk_text.split("</paragraph_content>")[0]
            else:
        ...
        raw_total = ((result or {}).get("hits") or {}).get("total", 0)
    ...
    async def _get_authorized_shougang_portal_file(
        ...
        *,
        ...
        if (
            file is None
            or int(file.knowledge_id) != int(space_id)
            or int(file.file_type) != FileType.FILE.value
            or int(file.status) != KnowledgeFileStatus.SUCCESS.value
        ):
            return None, []
    ...
    async def _search_shougang_portal_files_impl(self, req: ShougangPortalFileSearchReq) -> dict:
        ...
        elif is_public_latest_selected:
    ...
    async def _list_shougang_portal_files_without_keyword(
        ...
        *,
        ...
        cursor_context = self._shougang_portal_file_cursor_context(req, space_ids)
        try:
            batch_cursor = decode_cursor(
                req.cursor,
                expected_key_len=2,
        ...
        folder_path_map, source_path_map = await self._resolve_shougang_portal_source_paths(enriched_items)
        page_items: list[ShougangPortalFileItemResp] = []
        for item in enriched_items:
            space_id = int(item.get("knowledge_id") or 0)
            item["knowledge_name"] = space_name_map.get(space_id, str(space_id))
            item_id = int(item.get("id") or 0)
    ...
    async def _search_portal_metadata_files(
        ...
        """仅可发现空间复用文件名查询，不进入正文召回。"""
        metadata_files: list[KnowledgeFile] = []
        if space_ids:
            metadata_files = await KnowledgeFileDao.aget_file_by_space_filters_cursor(
                knowledge_ids=space_ids,
                file_name=keyword,
                status=[KnowledgeFileStatus.SUCCESS.value],
                file_ids=tag_file_ids,
        ...
        *,
        ...
        if self._is_shougang_portal_updated_at_sort(sort):
            return await self._collect_visible_shougang_portal_updated_at_candidates(
                ...
                sort=sort,
                defer_department_access=defer_department_access,
            )
    ...
    def _build_shougang_portal_search_chunk(
        ...
        *,
        ...
        canonical_metadata = user_metadata if isinstance(user_metadata, dict) else {}

        def _metadata_int(key: str) -> int:
            return self._coerce_shougang_portal_int(metadata.get(key) or canonical_metadata.get(key))
        ...
        return PortalSearchChunk(
            file_id=file_id,
            knowledge_id=knowledge_id,
            canonical_document_id=_metadata_int("canonical_document_id"),
    ...
    async def _filter_and_dedupe_portal_search_chunks(
        ...
        *,
        ...
        if not chunks:
            return []
        file_ids = sorted({int(chunk.file_id) for chunk in chunks if int(chunk.file_id) > 0})
        ...
        candidate_files = [
            ...
            and (
                ...
                or (
                    ...
                    and file.projection_status == KnowledgeFileProjectionStatus.READY.value
                    and file.applied_content_generation >= file.desired_content_generation
                    and file.applied_entry_generation >= file.desired_entry_generation
                )
            )
        ]
        visible_files = await self._filter_shougang_portal_search_files(
        ...
        document_ids = sorted(
            {int(file.reference_document_id) for file in visible_files if file.reference_document_id is not None}
        )
        expected_version_by_document: dict[int, int] = {}
        if document_ids and self.doc_repo is not None and self.version_repo is not None:
        ...
        for chunk in chunks:
            ...
            if file.reference_document_id is None:
                canonical_document_id = int(chunk.canonical_document_id) or int(file.id)
                canonical_version_id = int(chunk.canonical_version_id) or int(file.id)
            else:
                canonical_document_id = int(file.reference_document_id)
                canonical_version_id = int(chunk.canonical_version_id)
                if (
                    int(chunk.canonical_document_id) != canonical_document_id
                    or canonical_version_id <= 0
                    or int(chunk.content_generation) != int(file.desired_content_generation)
                    or int(chunk.entry_generation) != int(file.desired_entry_generation)
                ):
                    continue
                expected_version_id = expected_version_by_document.get(canonical_document_id)
                if expected_version_id is not None and canonical_version_id != expected_version_id:
                    continue
            validated.append(
                (
                    chunk,
        ...
        space_priority = {int(space.id): index for index, space in enumerate(spaces)}
        entry_priority = {
        ...
        for chunk in chunks:
            ...
            candidate = candidates.setdefault(
                ...
                PortalFileCandidate(
                    file_id=file_id,
                    knowledge_id=knowledge_id,
                    canonical_document_id=int(getattr(chunk, "canonical_document_id", 0) or 0),
                    canonical_version_id=int(getattr(chunk, "canonical_version_id", 0) or 0),
                ),
            )
            if not candidate.knowledge_id and knowledge_id:
                candidate.knowledge_id = knowledge_id
            if not candidate.canonical_document_id:
                candidate.canonical_document_id = int(getattr(chunk, "canonical_document_id", 0) or 0)
            if not candidate.canonical_version_id:
                candidate.canonical_version_id = int(getattr(chunk, "canonical_version_id", 0) or 0)
    ...
    def _sort_shougang_portal_semantic_candidates(
        ...
        *,
        ...
        for candidate in candidates:
            candidate.final_score = self._get_shougang_portal_candidate_final_score(candidate)
        return sorted(
            candidates,
            key=lambda candidate: (
    ...
    def _get_shougang_portal_candidate_final_score(
        ...
        rerank_score = (
            float(candidate.rerank_score) if candidate.rerank_score is not None else float(candidate.fusion_score)
        )
        bounded_rerank = max(min(rerank_score, 10.0), -10.0)
        return round(
            float(candidate.title_match_tier) * 1_000.0 + float(candidate.title_match_score) * 100.0 + bounded_rerank,
            12,
        )
    ...
    def _get_shougang_portal_semantic_cursor_key(
        ...
        final_score = 0.0 if self._is_shougang_portal_updated_at_sort(sort) else float(candidate.final_score)
        return (
            final_score,
            self._get_shougang_portal_file_update_timestamp(file_map.get(candidate.file_id)),
            self._get_shougang_portal_candidate_canonical_id(
    ...
    def _paginate_shougang_portal_semantic_candidates(
        ...
        *,
        ...
        ascending = (
            self._is_shougang_portal_updated_at_sort(req.sort) and self._shougang_portal_order_sort(req.sort) == "asc"
        )
        remaining = [
            candidate
            for candidate in candidates
    ...
    @classmethod
    def _shougang_portal_advanced_file_cursor_context(
        cls,
        req: ShougangPortalAdvancedFileSearchReq,
        space_ids: list[int],
    ) -> str:
        payload = {
            "sort": req.sort or "updated_at_desc",
            "space_ids": sorted({int(space_id) for space_id in space_ids if int(space_id) > 0}),
            "tag": str(req.tag or ""),
            "file_ext": str(req.file_ext or "").strip().lower().lstrip("."),
            "document_type": cls._normalize_shougang_document_type_code(req.document_type),
            "file_subcategory_code": cls._normalize_shougang_file_subcategory_code(req.file_subcategory_code),
            "business_domain_code": cls._normalize_shougang_portal_business_domain_code(req.business_domain_code),
            "all_keywords": str(req.all_keywords or ""),
            "exact_phrase": str(req.exact_phrase or ""),
            "any_keywords": str(req.any_keywords or ""),
            "exclude_keywords": str(req.exclude_keywords or ""),
            "search_field": req.search_field,
            "updated_from": req.updated_from.isoformat() if req.updated_from else "",
            "updated_to": req.updated_to.isoformat() if req.updated_to else "",
        }
        signature = hashlib.sha256(
            json.dumps(
                payload,
        ...
        return f"shougang_portal_advanced_files|{signature}"
    ...
    async def _get_shougang_portal_request_spaces(
        self,
        *,
        ...
        if discovery_scope == "legacy":
            return await self._get_shougang_portal_visible_search_spaces(
                requested_space_ids,
                space_level,
            )
        if discovery_scope in {"portal_public", "portal_configured", "portal_enabled"}:
            ...
            ordered_ids = (
                ...
                if requested_ids
                else list(discovery.query_space_ids)
            )
            if space_level is not None:
                expected_kinds = {
                    KnowledgeSpaceLevelEnum.PUBLIC: {"public"},
                    KnowledgeSpaceLevelEnum.DEPARTMENT: {"department"},
                    KnowledgeSpaceLevelEnum.TEAM: {"team", "clinic"},
                    KnowledgeSpaceLevelEnum.TEAM_KS: {"clinic"},
        ...
        public_space_ids = await KnowledgeSpaceScopeDao.aget_space_ids_by_level(KnowledgeSpaceLevelEnum.PUBLIC)
        department_space_ids: set[int] = set()
        if discovery_scope == "public_and_department":
            if requested_space_ids:
                candidate_department_ids = {int(space_id) for space_id in requested_space_ids if int(space_id) > 0}
            else:
                candidate_department_ids = {
                    int(binding.space_id) for binding in await DepartmentKnowledgeSpaceDao.aget_all()
                }
            department_space_ids = await self._get_valid_department_space_ids(candidate_department_ids)

        allowed_ids = {int(space_id) for space_id in public_space_ids if int(space_id) > 0} | department_space_ids
        requested_ids = list(dict.fromkeys(int(space_id) for space_id in requested_space_ids if int(space_id) > 0))
        legacy_space_map: dict[int, Knowledge] = {}
        if requested_ids and discovery_scope == "public_and_department":
            legacy_candidate_ids = [space_id for space_id in requested_ids if space_id not in allowed_ids]
            if legacy_candidate_ids:
                legacy_spaces = await self._get_shougang_portal_visible_search_spaces(
                    legacy_candidate_ids,
                    space_level,
                )
                legacy_space_map = {int(space.id): space for space in legacy_spaces if space.id is not None}
        if requested_ids:
            ordered_ids = [
                space_id for space_id in requested_ids if space_id in allowed_ids or space_id in legacy_space_map
            ]
        else:
            ordered_ids = list(
                dict.fromkeys(
                    [
                        *[int(space_id) for space_id in public_space_ids if int(space_id) > 0],
                        *sorted(department_space_ids),
                    ]
                )
            )
        if space_level == KnowledgeSpaceLevelEnum.PUBLIC:
            ordered_ids = [space_id for space_id in ordered_ids if space_id not in department_space_ids]
        elif space_level == KnowledgeSpaceLevelEnum.DEPARTMENT:
            ordered_ids = [space_id for space_id in ordered_ids if space_id in department_space_ids]
        elif space_level is not None:
            ordered_ids = [space_id for space_id in ordered_ids if space_id in legacy_space_map]
        if not ordered_ids:
            return []
        ...
        space_map.update(legacy_space_map)
        public_can_download = "download_file" in default_permission_ids_for_relation("viewer")
        for space_id in ordered_ids:
            if space_id in allowed_ids:
                self._portal_space_download_map[space_id] = (
                    public_can_download if space_id not in department_space_ids else False
                )
        return [space_map[space_id] for space_id in ordered_ids if space_id in space_map]
    ...
    async def _get_valid_department_space_ids(
        ...
        if not space_ids:
            return set()
        ordered_space_ids = sorted(int(space_id) for space_id in space_ids if int(space_id) > 0)
        scopes = await KnowledgeSpaceScopeDao.aget_map_by_space_ids(ordered_space_ids)
        bindings = await DepartmentKnowledgeSpaceDao.aget_by_space_ids(ordered_space_ids)
        binding_map = {int(binding.space_id): binding for binding in bindings}
        department_ids = {int(binding.department_id) for binding in bindings}
        departments = await DepartmentDao.aget_by_ids(sorted(department_ids))
        department_map = {int(department.id): department for department in departments}
        valid_space_ids: set[int] = set()
        for space_id in ordered_space_ids:
            scope = scopes.get(space_id)
    ...
    async def list_valid_department_space_options(
        ...
        *,
        ...
        candidate_ids = {int(binding.space_id) for binding in bindings if int(getattr(binding, "space_id", 0) or 0) > 0}
        ...
        department_ids = sorted({int(binding.department_id) for binding in binding_map.values()})
    ...
    async def _get_shougang_portal_tag_file_ids(self, space_ids: list[int], tag_name: str | None) -> list[int] | None:
        ...
        tag_ids: list[int] = []
        for space_id in space_ids:
            tag_ids.extend(await TagLibraryTagService.resolve_tag_ids_by_name_for_space(space_id, tag_name))
    ...
    async def _get_shougang_portal_tag_ids_for_spaces(
        self,
        space_ids: list[int],
        tag_name: str | None,
    ) -> list[int] | None:
        ...
        for space_id in space_ids:
            tag_ids.extend(
                await TagLibraryTagService.resolve_tag_ids_by_name_for_space(
                    space_id,
                    tag_name,
                )
            )
        return list(dict.fromkeys(int(tag_id) for tag_id in tag_ids if int(tag_id) > 0))
        ...
        *,
        ...
        for space_id, items in grouped_files.items():
            if space_id in getattr(self, "_portal_grant_parent_space_ids", set()):
                discovery = getattr(self, "_portal_discovery_result", None)
                has_full_space_access = bool(
                    discovery
                    and (
                        space_id in discovery.discoverable_space_ids
                        or space_id in discovery.explicitly_visible_space_ids
                    )
                )
    ...
    async def _filter_shougang_portal_search_files(
        self,
        files: list[KnowledgeFile],
        *,
        spaces: list[Knowledge] | None = None,
        ...
        """Filter search candidates while deferring department access until interaction."""
        if not files:
            return []
        if not defer_department_access:
            return await self._filter_shougang_portal_visible_files(
                files,
                spaces=spaces,
            )

        space_ids = {int(file.knowledge_id) for file in files}
        public_space_ids = await self._get_shougang_portal_public_space_ids(
            list(space_ids),
            spaces=spaces,
        )
        ...
        guarded_files: list[KnowledgeFile] = []
        for file in files:
            space_id = int(file.knowledge_id)
            if space_id in public_space_ids:
                public_files.append(file)
            elif space_id in guarded_space_ids:
                discovery = getattr(self, "_portal_discovery_result", None)
                grant_only = bool(
                    space_id in getattr(self, "_portal_grant_parent_space_ids", set())
                    and discovery
                    and space_id not in discovery.discoverable_space_ids
                    and space_id not in discovery.explicitly_visible_space_ids
                )
                if not grant_only or int(file.id) in getattr(
                    self,
                    "_portal_explicit_file_ids",
                    set(),
                ):
                    department_files.append(file)
            else:
                guarded_files.append(file)
        ...
        if guarded_files:
            accepted_ids.update(
                int(file.id)
                for file in await self._filter_shougang_portal_visible_files(
                    guarded_files,
                    spaces=spaces,
                )
            )
        return [file for file in files if int(file.id) in accepted_ids]
    ...
    async def _get_shougang_portal_tag_ids(self, space_id: int, tag_name: str | None) -> list[int] | None:
        if not tag_name:
            return None
        return await TagLibraryTagService.resolve_tag_ids_by_name_for_space(space_id, tag_name)
    ...
    def _is_shougang_portal_file_item(
        ...
        if (
            normalized_file_subcategory_code
            and self._get_shougang_file_subcategory_code(item) != normalized_file_subcategory_code
        ):
            return False
        return True

    def _get_portal_file_access_state(self, file_id: int) -> tuple[bool, str]:
        access_decision = self._portal_file_access_decision_map.get(file_id)
        access_unchecked = file_id in getattr(
            self,
            "_portal_unchecked_department_file_ids",
            set(),
        )
        is_department_file = access_decision is not None or access_unchecked
        content_access = (
            str(access_decision.status)
            if access_decision is not None
            else "check_required"
            if access_unchecked
            else "allowed"
        )
        return is_department_file, content_access

    def _map_shougang_portal_file_item(self, space_id: int, item: dict) -> ShougangPortalFileItemResp:
        file_name = str(item.get("file_name") or "")
        file_id = int(item.get("id") or 0)
        capability_payload = item.get("capabilities") if isinstance(item.get("capabilities"), dict) else {}
        access_decision = self._portal_file_access_decision_map.get(file_id)
        is_department_file, content_access = self._get_portal_file_access_state(file_id)
        content_allowed = content_access == "allowed"
        discovery = getattr(self, "_portal_discovery_result", None)
        ...
        is_clinic = space_kind == "clinic"
        resolved_space_level = (
            ...
            else KnowledgeSpaceLevelEnum.DEPARTMENT
            if is_department_file
            else None
        )
        tag_infos = [
            {
        ...
        return ShougangPortalFileItemResp(
            ...
            file_size=(str(item.get("file_size") or "") if not safe_metadata_only else ""),
            file_encoding=(
                str(
            ...
            source_path=(str(item.get("source_path") or "") if not safe_metadata_only else ""),
            can_download=bool(
            ...
            access_source=(
                ...
                if access_decision is not None and access_decision.source is not None
                else None
            ),
            is_department_file=is_department_file,
            space_level=resolved_space_level,
            is_clinic=is_clinic,
            ...
            canonical_version_id=(
                int(item["canonical_version_id"]) if item.get("canonical_version_id") is not None else None
            ),
            manager_file_id=(
                int(item["manager_file_id"])
                if item.get("manager_file_id") is not None and capability_payload.get("can_edit_content", False)
                else None
            ),
            manager_space_id=(int(item["manager_space_id"]) if item.get("manager_space_id") is not None else None),
            desired_content_generation=int(item.get("desired_content_generation") or 0),
            applied_content_generation=int(item.get("applied_content_generation") or 0),
            desired_entry_generation=int(item.get("desired_entry_generation") or 0),
            applied_entry_generation=int(item.get("applied_entry_generation") or 0),
    ...
    def _get_shougang_business_domain_code(cls, item: Any) -> str:
        _, business_domain_code = cls._parse_shougang_file_encoding_codes(item)
    ...
    def _get_shougang_file_encoding(item: Any) -> str:
        ...
        return str(getattr(item, "file_encoding", "") or "")
    ...
    async def delete_space(self, space_id: int, *, force: bool = False, migrate_free_space: bool = True) -> None:
    ...
    async def _prepare_department_rebind_for_update(
        self,
        *,
        space: Knowledge,
        space_id: int,
        department_id: int,
        old_department: Any,
        target_department: Any,
    ) -> "DepartmentSpaceRebindPlan":
        old_admin_rows = await DepartmentService.aget_admins(
            old_department.dept_id,
            self.login_user,
        )
        new_admin_rows = (
            old_admin_rows
            if int(old_department.id) == int(target_department.id)
            else await DepartmentService.aget_admins(
        ...
        if plan.is_noop:
            return plan
        if self.department_file_view_lifecycle_service is None:
        ...
        try:
            await self.department_file_view_lifecycle_service.prepare_department_rebind(
                ...
                operator_id=int(self.login_user.user_id),
                operator_name=str(self.login_user.user_name or ""),
            )
    ...
    async def _rollback_prepared_department_rebind(
        self,
        plan: "DepartmentSpaceRebindPlan",
    ) -> None:
        await self.department_space_binding_repo.rollback_prepared_rebind()
    ...
    async def update_knowledge_space(
        ...
        if department_id is not None:
            ...
            if is_department_space and not self.login_user.is_admin():
        ...
        if portal_discovery_enabled is not None and discovery_scope is not None:
            ...
            try:
                ...
                await self._write_portal_discovery_audit(
                    space=space,
                    old_value=old_value,
                    new_value=bool(portal_discovery_enabled),
                    result="failed",
            ...
            await self._write_portal_discovery_audit(
                space=space,
                old_value=old_value,
                new_value=bool(updated_scope.portal_discovery_enabled),
                result="success",
            )
        else:
            space = await KnowledgeDao.async_update_space(space)
        if department_id is not None:
            if prepared_portal_rebind_plan is None:
                ...
                try:
                    ...
                    await self._rollback_prepared_department_rebind(plan)
                    raise
        ...
        *,
        ...
        try:
            await AuditLogDao.ainsert_v2(
                tenant_id=int(getattr(space, "tenant_id", None) or self.login_user.tenant_id),
                operator_id=int(self.login_user.user_id),
                operator_tenant_id=int(self.login_user.tenant_id),
                operator_name=str(self.login_user.user_name or ""),
                action="knowledge_space.portal_discovery.update",
                target_type="knowledge_space",
                target_id=str(space.id),
                object_name=str(space.name or ""),
                metadata=metadata,
                ip_address=get_request_ip(self.request),
            )
        except Exception:
            _logger.exception(
                "Portal discovery audit write failed: space_id=%s result=%s request_id=%s",
                space.id,
                result,
                request_id,
            )
    ...
    async def _notify_favorite_source_changed(
        ...
        *,
        ...
        """在业务变更成功后尽力投递一条收藏字段变化事件。"""
        normalized_before = self._normalize_favorite_change_value(before_value)
        normalized_after = self._normalize_favorite_change_value(after_value)
        ...
        try:
            ...
            _logger.exception(
                "failed to enqueue favorite change: file_id=%s action_code=%s",
                source_file_id,
                action_code,
            )

    @staticmethod
    def _normalize_favorite_change_value(value: Any) -> Any:
    ...
    async def _resolve_shougang_portal_source_metadata(self, items: list[dict], *, search_context=None) -> dict[int, dict]:
        ...
        Share entries use share_source_file_id because their own knowledge_id and
        file_level_path describe the receiver placement, not the sharing source.
        ...
        The result uses separate source_* fields so callers never overwrite the
        receiver entry's knowledge_id/file_level_path.
        """
        resolved_metadata: dict[int, dict] = {}
        unresolved_items: list[dict] = []
        for item in items:
            item_id = int(item.get("id") or 0)
            source_space_id = int(item.get("source_space_id") or 0)
            source_space_name = str(item.get("source_space_name") or "")
        ...
        share_source_file_by_item: dict[int, int] = {}
        publish_source_file_by_item: dict[int, int] = {}
        explicit_source_file_ids: set[int] = set()
        ...
        for item in unresolved_items:
            ...
            share_source_file_id = (
                item.get("share_source_file_id")
                if str(item.get("entry_type") or "") == KnowledgeFileEntryType.SHARE.value
                else None
            )
            if share_source_file_id:
                share_source_file_by_item[item_id] = int(share_source_file_id)
                explicit_source_file_ids.add(int(share_source_file_id))
                continue
            ...
            if source_file_id:
                publish_source_file_by_item[item_id] = int(source_file_id)
        ...
        shared_source_by_item: dict[int, tuple[int, str, str]] = {}
        for item_id, source_file_id in share_source_file_by_item.items():
            source_file = source_file_map.get(source_file_id)
            if source_file is None:
                continue
            shared_source_by_item[item_id] = (
        ...
        for item_id, source_file_id in publish_source_file_by_item.items():
            source_file = source_file_map.get(source_file_id)
            if source_file is None:
                continue
        ...
        all_item_ids = (
            set(current_source_by_item)
            | set(shared_source_by_item)
            | set(published_source_by_item)
            | set(document_source_by_item)
        )
        for item_id in all_item_ids:
            shared_source = shared_source_by_item.get(item_id)
            ...
            if shared_source:
                source_record_by_item[item_id] = shared_source
            elif published_source and published_source[1]:
        ...
        for item_id, (knowledge_id, file_level_path, file_name) in source_record_by_item.items():
            ...
            source_folder_path = "/".join([space_name, *folder_segments])
            if folder_segments:
                source_tail_segments = [*folder_segments, *([file_name] if file_name else [])]
                source_path = f"{space_name}>{'/'.join(source_tail_segments)}"
            else:
                source_path = space_name
            resolved_metadata[item_id] = {
                "source_space_id": knowledge_id,
                "source_space_name": space_name,
                "source_folder_path": source_folder_path,
                "source_path": source_path,
            }
            if item_id in shared_source_by_item:
    ...
    async def _load_filtered_folder_stat_counts(
        ...
        *,
        ...
        recycled = await KnowledgeRecycleService.list_recycled_file_ids(int(space.id))
        if recycled:
    ...
    async def _load_document_distribution_info(
        ...
        *,
        ...
        file_items = [item for item in files if item.file_type == FileType.FILE.value]
        document_ids = sorted(
        ...
        for item in file_items:
            ...
            capabilities = KnowledgeDocumentEntryResolver._capabilities(
                ...
                set(permission_ids),
                allow_download=(
                    bool(item.allow_download) if entry_type == KnowledgeFileEntryType.SHARE.value else True
                ),
            )
            ...
            canonical_version_id = (
                int(primary_version.id) if primary_version is not None else getattr(item, "_canonical_version_id", None)
            )
            manager_file_id = int(primary_version.knowledge_file_id) if primary_version is not None else int(item.id)
            ...
            projection_ready = (
                ...
                and item.applied_content_generation >= item.desired_content_generation
                and item.applied_entry_generation >= item.desired_entry_generation
                if is_distribution
                else True
            )
            item_info = {
                ...
                "entry_status": (item.entry_status or KnowledgeFileEntryStatus.ACTIVE.value),
                "canonical_document_id": canonical_document_id,
                ...
                "applied_entry_generation": (int(item.applied_entry_generation) if is_distribution else 0),
                "projection_status": (
                    item.projection_status if is_distribution else KnowledgeFileProjectionStatus.READY.value
                ),
        ...
        *,
        ...
        if not permission_context.get("can_view_all_statuses", False):
            visible = self._hide_restricted_status_items(
                visible,
                owner_user_id=self.login_user.user_id,
            )
        return visible
    ...
    async def list_space_children(
        ...
        recycled = await KnowledgeRecycleService.list_recycled_file_ids(space_id)
        if recycled:
            exclude_file_ids = list(dict.fromkeys([*(exclude_file_ids or []), *recycled])) or None
    ...
    def _is_qa_scope_file(file_record: Any, space_id: int) -> bool:
        ...
        distribution_ready = document_id is None or (
            ...
            and getattr(file_record, "projection_status", None) == KnowledgeFileProjectionStatus.READY.value
    ...
    async def delete_folder(self, space_id: int, folder_id: int):
        ...
        recycle = KnowledgeRecycleService(self.login_user)
    ...
    async def move_file_folder(
        ...
        if resolved is not None:
            ...
            file_record.projection_status = KnowledgeFileProjectionStatus.PENDING.value
    ...
    async def rename_file(self, file_id: int, new_name: str) -> KnowledgeFile:
        ...
        if resolved is None:
            ...
            if self.document_distribution_service is None:
                raise KnowledgeDocumentStateConflictError(msg="文档分发生命周期服务不可用")
            try:
                updated_file = await self.document_distribution_service.rename_manager_document(
                    tenant_id=int(file_record.tenant_id),
                    document_id=int(file_record.reference_document_id),
                    manager_file_id=int(file_record.id),
                    new_name=new_name,
        ...
        if (old_name or "") != (updated_file.file_name or ""):
            await self._notify_favorite_source_changed(
                ...
                before_value=old_name or "",
                after_value=updated_file.file_name or "",
            )
        return updated_file
    ...
    async def update_file_encoding(
        ...
        if business_domain_changed:
            await self._notify_favorite_source_changed(
                ...
                source_file_id=file_id,
                file_name=updated_file.file_name,
                action_code=FAVORITE_SOURCE_BUSINESS_DOMAIN_UPDATED,
                before_value=BUSINESS_DOMAIN_OPTIONS.get(
                    old_business_domain_code,
                    old_business_domain_code or "",
                ),
                after_value=BUSINESS_DOMAIN_OPTIONS.get(
                    new_business_domain_code,
                    new_business_domain_code or "",
                ),
            )
        if subcategory_changed:
            _, subcategory_labels = FileClassificationLabelService.get_label_lookup_for_tenant(
            ...
            await self._notify_favorite_source_changed(
                ...
                source_file_id=file_id,
                file_name=updated_file.file_name,
                action_code=FAVORITE_SOURCE_SUBCATEGORY_UPDATED,
                before_value=subcategory_labels.get(
                    old_subcategory_code,
                    old_subcategory_code or "",
                ),
                after_value=subcategory_labels.get(
                    normalized_file_subcategory_code,
                    normalized_file_subcategory_code or "",
                ),
            )
    ...
    async def _plan_cascade_version_links_on_delete(
        ...
        """只读计算版本级联影响集，供删除前冻结收藏接收人。"""
        if not self.version_repo or not self.doc_repo or not file_ids:
            return FavoriteDeletePlan(
                expanded_file_ids=self._dedupe_ids(file_ids),
                version_ids=[],
                document_ids=[],
            )
        ...
        return FavoriteDeletePlan(
            expanded_file_ids=sorted(expanded),
            version_ids=self._dedupe_ids(versions_to_delete),
            document_ids=self._dedupe_ids(documents_to_delete),
        )

    async def _apply_cascade_version_delete_plan(
        self,
        plan: FavoriteDeletePlan,
    ) -> None:
        if not self.version_repo or not self.doc_repo:
            return
        for vid in plan.version_ids:
            await self.version_repo.delete(vid)
        for did in plan.document_ids:
    ...
    async def _cascade_version_links_on_delete(self, file_ids: list[int]) -> list[int]:
        """兼容旧调用：计算并立即执行版本级联清理。"""
        plan = await self._plan_cascade_version_links_on_delete(file_ids)
        await self._apply_cascade_version_delete_plan(plan)
        return plan.expanded_file_ids

    async def _prepare_favorite_delete_events(
        self,
        file_ids: list[int],
        *,
        source_files: list[KnowledgeFile] | None = None,
    ) -> list[FavoriteChangeEvent]:
        """在任何删除 mutation 前生成权限安全的接收人快照。"""
        ...
        try:
            ...
            _logger.exception(
                "failed to snapshot favorite delete recipients: file_ids=%s",
                normalized_ids,
            )
            return []
        ...
        for source_file in source_files:
            ...
            events.append(
                FavoriteChangeEvent(
                    ...
                    before_value=source_file.file_name or "",
                    after_value=None,
                    actor_user_id=int(self.login_user.user_id),
                    actor_user_name=getattr(self.login_user, "user_name", None),
    ...
    async def _delete_file_rows_with_department_grants(
        self,
        *,
        space_id: int,
        tenant_id: int,
        ...
        if not normalized_ids:
            return
        lifecycle_service = self.department_file_view_lifecycle_service
        if lifecycle_service is None:
            # 兼容仅用于旧单元测试/脚本的无 DI 构造路径；生产 API 依赖会始终注入
            # 生命周期服务。该回退路径不改变历史调用行为。
            await KnowledgeFileDao.adelete_batch(normalized_ids)
            return
        try:
            ...
            await lifecycle_service.session.commit()
        except Exception:
            await lifecycle_service.session.rollback()
            raise
    ...
    async def delete_file(self, file_id: int):
        ...
        delete_plan = await self._plan_cascade_version_links_on_delete([file_id])
        favorite_delete_events = await self._prepare_favorite_delete_events(
            delete_plan.expanded_file_ids,
            source_files=[file_record],
        )
        await self._apply_cascade_version_delete_plan(delete_plan)
        expanded_ids = delete_plan.expanded_file_ids
    ...
    async def get_file_download(self, file_id: int, *, space_id: int | None = None) -> dict:
        ...
        if resolved is None:
            ...
            if resolved.entry_type in {
                ...
                raise KnowledgeDocumentDownloadDeniedError(msg="分发入口只能通过水印 PDF 下载")
            content_file = (
                file_record
        ...
        original_url, preview_url = KnowledgeService.get_file_share_url(file=content_file)
    ...
    async def _find_pending_review_tag_in_bound_libraries(
        self,
        space_id: int,
        tag_name: str,
    ) -> ReviewTag | None:
        ...
        *,
        ...
        async with get_async_db_session() as session:
            if space_id is not None:
                ...
                if legacy_space_pending:
                    return legacy_space_pending
    ...
    async def _find_space_tag_by_name(self, space_id: int, tag_name: str) -> Tag | ReviewTag | None:
        ...
        bound_library_tag = await self._find_bound_library_tag_by_name(space_id, normalized)
        if bound_library_tag:
            return bound_library_tag

        tenant_library_tag = await self._find_tenant_library_tag_by_name(normalized)
        if tenant_library_tag:
            return tenant_library_tag

        # Legacy approved tags scoped to the knowledge space.
        ...
        pending_in_libraries = await self._find_pending_review_tag_in_bound_libraries(space_id, normalized)
        if pending_in_libraries:
            return pending_in_libraries

        # Legacy pending tags scoped to the knowledge space.
    ...
    async def update_file_tags(self, space_id: int, file_id: int, tag_ids: list[int], review_tag_ids: list[int]):
        ...
        await self._notify_favorite_source_changed(
            ...
            before_value=before_tags,
            after_value=after_tags,
        )
        if resolved is not None:
            ...
            if file.file_type == FileType.FILE.value and file.status in retryable_status:
                ...
                for item in all_failed_files:
                    if item.status in retryable_status and item.file_type == FileType.FILE.value:
                        resolved = await self._require_document_content_manager(item)
                        if resolved is None:
                            await self._require_resource_permission(
                                "can_edit",
                                "knowledge_file",
    ...
    async def batch_delete(self, knowledge_id: int, file_ids: list[int], folder_ids: list[int]):
        ...
        folder_records: list[KnowledgeFile] = []
        ...
        snapshot_file_ids = self._dedupe_ids(
            [
                *delete_plan.expanded_file_ids,
                *(int(file.id) for file in distribution_files),
            ]
        )
        favorite_delete_events = await self._prepare_favorite_delete_events(
            snapshot_file_ids,
        ...
        if unique_folder_ids:
            ...
            try:
                await ChannelKnowledgeSyncDao.adelete_by_folder_ids(unique_folder_ids)
            except Exception as e:
                _logger.warning(
        ...
        for folder in folder_records:
            await self.update_folder_update_time(folder.file_level_path)
            await self._notify_folder_deleted(
                folder_name=str(folder.file_name or folder.id),
                space_id=knowledge_id,
            )
        enqueue_favorite_change_events(favorite_delete_events)
