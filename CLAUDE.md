...
**v2.5 核心改造**：权限体系从 RBAC 迁移到 ReBAC（OpenFGA）+ 多租户重构为 Tenant 树形架构（仅私有化部署，Root + 0~N Child 两层拓扑，两层管理员模型）。改造上下文见「v2.5 改造上下文」章节。
...
**Nginx 反向代理**: 8860 → 3001（Platform 前端）
...
**错误码**：5 位编码 `MMMEE`（模块 3 位 + 错误 2 位），定义在 `common/errcode/`。三种输出：`return_resp()`（HTTP）、`to_sse_event()`（SSE）、`websocket_close_message()`（WS）。模块编码：100=server, 101=finetune, 104=assistant, 105=flow, 106=user, 108=llm, 109=knowledge, 110=linsight, 120=workstation, 130=chat/channel, 140=message, 150=tool, 160=dataset, 170=telemetry, 180=knowledge_space, 190=permission, 191=tenant_resolver (F012), 192=tenant_fga (F013), 193=tenant_sync (F014/F015), 194=tenant_quota (F016), 195=tenant_sharing (F017), 196=resource_owner_transfer (F018), 197=admin_scope (F019), 198=llm_tenant (F020), 200=tenant, 210=department, 220=tenant_tree (F011), 230=user_group, 240=org_sync
...
**前端集成**：`src/frontend/platform/src/controllers/API/pro.ts` 调用 6 个 Gateway 自有 API（SSO URL、LDAP 登录、敏感词、用户组）。
...
**资源级授权主体（2026-04-21 收窄）**：`user:X` / `department:X#member` / `user_group:X#member`（+ viewer 层 `tenant:X#shared_to#member` 用于 Root→Child 显式共享）。**Tenant 不进资源授权** —— 仅作配额/存储/归属边界。
...
- Tenant admin **不**沿 parent 继承（两层管理员模型）；**Root Tenant 不写 `tenant:1#admin` 元组**，Root 管理由 `system:global#super_admin` 承担
- Tenant 在 FGA 的 3 个用途：`admin`（Child Admin 短路，② 级）/ `member`（归属反查，不进资源授权）/ `shared_to`（显式共享）
- 资源 DSL manager/editor/viewer **不含** `tenant#member`（2026-04-21 收窄：避免"同 Tenant 成员默认可编辑/读所有资源"越权）
...
### 多租户引入（2026-04-18 Tenant 树形 / 04-20 仅私有化收窄 / 04-21 Round 2 Review）

**为什么**：面向大型集团企业（中粮、首钢），需要统一 SSO + 一棵部门树 + 子公司逻辑隔离 + 配额管控。原"扁平多租户"假设"每个租户=独立组织"与此冲突，重写为 Tenant 树形架构。

**适用范围（2026-04-20 收窄）**：**仅私有化部署**，一个 bisheng 实例只服务一个集团。SaaS 多客户共存能力**不支持**；多法人客户需独立部署 bisheng 实例。

**核心概念（Tenant 树）**：
- **Root Tenant**（本实例唯一，`tenant_id=1`）：代表集团总部。由迁移脚本**自动创建**，**不可删除/禁用**（INV-T11）
- **Child Tenant**（挂在 Root 下）：集团子公司/事业部，通过"挂载到部门树子节点"表达；全局超管通过 `POST /api/v1/departments/{dept_id}/mount-tenant` 手工标记
- **叶子 Tenant**：用户实际归属的最深层 Tenant；无 Child 挂载路径的用户归 Root
- **MVP 锁 2 层**（Root + 0~N Child），3+ 级集团拍平到同层 Child；多层嵌套 → v2.6+（附录 G.4）
- **数据模型精简（2026-04-20）**：tenant 表仅新增 `parent_tenant_id` + `share_default_to_children`；废弃 `tenant_path / level / tenant_kind`（2 层下通过 `parent_tenant_id IS NULL` 直接派生）
- **Tenant.status 枚举**：`active / disabled / archived / orphaned`（orphaned 为挂载部门被 SSO 删除时的状态）

**用户归属**：
- **唯一叶子**（废弃 `user_tenant` 多对多）；登录时沿主部门反向找最近挂载点，派生叶子 Tenant 写入 JWT
- JWT 含 `tenant_id`（叶子）+ `token_version`（`user` 表新字段，主部门变更时 +1 使旧 JWT 失效）
- 主部门变更 → 归属切换；**数据留原处不动**，通过 `POST /api/v1/tenants/{id}/resources/transfer-owner` 做所有者交接（to_user 叶子必须 ∈ `{tenant_id, tenant_id 的 Root}`，不支持 Root→Child 下沉路径——下沉走 `POST /api/v1/tenants/{child_id}/resources/migrate-from-root`）
- 配置 `user_tenant_sync.enforce_transfer_before_relocate=true` 可在主部门同步前阻断
- 兼职/离职走同一机制；`POST /api/v1/user/switch-tenant` 返 410 Gone

**管理员模型（两层）**：
- **全局超管**：`system:global#super_admin`；作用域含整棵树（Root + 所有 Child）
- **Child Admin**：`tenant:{child_id}#admin`；仅管本 Child，不跨 Child、不管 Root 配额、不挂载/解绑 Tenant
- 已废弃"Root Admin"独立角色；`POST /tenants/1/admins` 拒绝（错误码 19204）

**隔离方式**：逻辑隔离 —— 共享数据库 + `tenant_id` + **SQL IN 列表**（2 层下 = `{leaf, Root=1}`）自动注入过滤。23 张业务表添加 `tenant_id`。严格计数场景用 `strict_tenant_filter()` 上下文精确匹配。

**集团共享资源（Root → Child 分发）**：
- Root 创建的资源通过 UI 开关写入 `{resource}#viewer → tenant:{root}#shared_to#member`，对所有 Child 成员只读
- 新挂 Child 时自动写 `tenant:{root}#shared_to → tenant:{child}`；解绑 Child 时自动撤销
- Child 用户读 Root 共享产生的衍生数据（对话、token、调用日志）归属 **Child 叶子 Tenant**（INV-T13），不计入 Root 用量；写入由 F017 §5.4 `ChatMessageService` / `LLMTokenTracker` 用 `get_current_tenant_id()` 实现

**配额**：`有效配额 = min(叶子剩余, Root 剩余, 角色配额)`；**Root 用量 = Root 自身 + Σ 所有 Child 累计**（集团硬盖，INV-T9）。MVP 仅基础 CRUD，转移/计费/分摊 → v2.6+。

**孤儿 Tenant**：挂载部门被 SSO 删除时 Tenant 进 `orphaned` 状态；触发统一由 F011 `DepartmentDeletionHandler.on_deleted(dept_id, source)` 处理（写 audit_log + 站内/邮件告警），F014/F015 均调用此 handler。

**部门同步（10 万人关键）**：
- `POST /api/v1/internal/sso/login-sync`（Gateway SSO 登录时）
- `POST /api/v1/departments/sync`（Gateway 批量推送，HMAC）
...
### v2.5 Review 决策迭代（2026-04-18 → 04-20 → 04-21）

**2026-04-18 Round 1（9 项决策）** — PRD Review 后 Tenant 树落地决策：

1. **树深度**：MVP 锁 2 层；3+ 级客户拍平到同层 Child
2. **Root Admin 权限**：默认可见全 Root 子树（合规由客户自管，§1.5 合规责任声明）
3. **用户归属**：归属跟主部门切，资源留原处；新增所有者交接 API
4. **挂载决策权**：全局超管单人操作 + 强制操作日志
5. **取消挂载**：bisheng 仅一次性处理接口；留存/告警由客户外部承接
6. **配额商业化**：MVP 仅基础 CRUD；转移/计费/分摊 → v2.6+（附录 G）
7. **规模化**：仅审资"已有客户"级别压测；SSO 换型仅 relink 文档化
8. **两层管理员**：全局超管 + Child Admin（Root Admin 合并）；`tenant#admin` 不继承
9. **Feature 实施**：F011/F012/F013 spec 强制含依赖链 + 23 表改造清单 + checklist

**2026-04-20 Round 2 收窄**：
- **仅私有化部署**，删除所有 SaaS 多客户 / 跨 Root 运维场景
- **Root Tenant 自动创建且不可删/禁**（INV-T11；`POST /tenants` 返 410，`DELETE /tenants/1`、`PUT /tenants/1/status` 返 403 + 22008）
- **数据模型精简**：tenant 表仅 `parent_tenant_id` + `share_default_to_children`；废弃 `tenant_path / level / tenant_kind`
- **新增**：`audit_log` 表 / `user.token_version` 字段 / `Tenant.status` 加 `orphaned` 枚举 / Gateway 实时 vs Celery 校对 ts 冲突规则（INV-T12）
- **Root 用量聚合**：`Root 自身 + Σ 所有 Child`（集团硬盖，INV-T9）
- **共享资源衍生数据归叶子 Tenant**（INV-T13）

**2026-04-21 Round 3 修复**（深度 Review 后的收窄 + 对齐）：
- **DSL 彻底收窄**：资源 manager/editor/viewer 移除 `tenant#member`，viewer 仅保留 `tenant#shared_to#member`；FGA 删除 `tenant#parent` 关系
- **F014 补 `/departments/sync`** 批量部门同步端点
- **F011 新建 `DepartmentDeletionHandler`** 集中处理孤儿 Tenant 触发
- **F018 删除 Root → Child 交接路径**（to_user 叶子必须 ∈ `{tenant_id, Root}`；资源下沉改走 F011 `/tenants/{child_id}/resources/migrate-from-root`）
- **F017 新增 §5.4 衍生数据写入层**（`ChatMessageService` / `LLMTokenTracker` 用 `get_current_tenant_id()` 赋值 tenant_id）
- config key `user.enforce_transfer_before_relocate` → `user_tenant_sync.enforce_transfer_before_relocate`
- F011/F012 补废弃 API AC（410 Gone）+ Root admin API 保护 AC + audit_log action 清单
...
| **多租户需求文档（Tenant 树，含 2026-04-20/21 收窄）** | `docs/PRD/2.5 权限管理体系改造 PRD/2.5 多租户需求文档.md` |
| 多租户管理 PRD（已合入多租户需求文档） | `docs/PRD/2.5 权限管理体系改造 PRD/2.5 多租户管理 PRD.md` |
| ReBAC 技术方案（§11 Tenant 树技术增量 + §11.9 2026-04-20/21 详细实现） | `docs/PRD/2.5 权限管理体系改造 PRD/2.5 技术方案.md` |
| 技术方案 Review（§7 三轮迭代说明） | `docs/PRD/2.5 权限管理体系改造 PRD/2.5 技术方案 Review.md` |
| 版本升级迁移方案（§3.6 Tenant 树 DDL + audit_log + token_version） | `docs/PRD/2.5 权限管理体系改造 PRD/2.5 版本升级迁移方案.md` |
| v2.5.1 Feature 索引 + release-contract | `features/v2.5.1/README.md`、`features/v2.5.1/release-contract.md` |
| v2.5.1 Feature spec（F011~F018） | `features/v2.5.1/{011-tenant-tree-model ~ 018-resource-owner-transfer}/spec.md` |
