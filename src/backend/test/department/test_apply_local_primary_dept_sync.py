...
def patches(monkeypatch):
    ...
    monkeypatch.setattr(
        ds_module.DepartmentChangeHandler,
        "execute_async",
        dept_execute,
    )
    admin_grant_delete = AsyncMock(name="DepartmentAdminGrantDao.adelete")
    monkeypatch.setattr(
        ds_module.DepartmentAdminGrantDao,
        "adelete",
        admin_grant_delete,
    )
    session_calls = {
...
def test_same_primary_dept_is_noop(patches):
    ...
    asyncio.run(DepartmentService._apply_local_primary_department_change(100, 5))
...
def test_no_old_primary_inserts_and_syncs(patches):
    ...
    asyncio.run(DepartmentService._apply_local_primary_department_change(101, 12))

    # New UserDepartment row inserted (no delete — there was nothing to remove).
    assert patches.session_calls["delete"] == []
    inserted = [
        obj for obj in patches.session_calls["add"] if hasattr(obj, "department_id") and obj.department_id == 12
    ]
...
def test_cross_tenant_swap_dispatches_fga_and_syncs(patches):
    ...
    asyncio.run(DepartmentService._apply_local_primary_department_change(102, 20))
    ...
    inserted = [
        obj for obj in patches.session_calls["add"] if hasattr(obj, "department_id") and obj.department_id == 20
    ]
    assert len(inserted) == 1
    assert inserted[0].is_primary == 1
    assert patches.session_calls["commit"] == 1
...
def test_relocate_blocked_propagates(patches):
    ...
    with pytest.raises(TenantRelocateBlockedError):
        asyncio.run(DepartmentService._apply_local_primary_department_change(103, 20))
