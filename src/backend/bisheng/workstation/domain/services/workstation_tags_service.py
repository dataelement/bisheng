...
class WorkStationTagsService(BaseService):
    ...
    async def delete_review_tag(self, tag_name: str, business_type: TagBusinessTypeEnum, resource_type: TagResourceTypeEnum, tenant_id: int):
        review_tag_list = await self.review_tags_repository.get_review_tag_list_by_tag_name(tag_name, resource_type, tenant_id)
        if review_tag_list:
            for review_tag in review_tag_list:
                await self.review_tags_repository.delete_review_tag_id(review_tag.id, business_type.value, resource_type, tenant_id)
    ...
    async def approve_or_reject_review_tag(self, data: ApproveOrRejectRequest, tenant_id: int):
        ...
        if data and data.status == ApproveOrRejectEnum.APPROVE:
            existed_tag_list = await self.approve_tag_to_move_operation(data.tag_name, data.resource_type, tenant_id)
            ...
            await self.review_tags_repository.reject_review_tag(data.tag_name, data.reject_reason, data.resource_type, tenant_id)
            await self.session.commit()
        else:
    ...
    async def approve_tag_to_move_operation(self, tag_name: str, resource_type: TagResourceTypeEnum, tenant_id: int):
        review_tag_list = await self.review_tags_repository.get_review_tag_list_by_tag_name(tag_name, resource_type, tenant_id)
        existed_tag_list = []
    ...
    async def update_tag_library_by_name(self, original_tag_name: str, tag_name: str, resource_type: TagResourceTypeEnum, tenant_id: int):
        ...
        if resource_type == TagResourceTypeEnum.SYSTEM_TAG or resource_type == TagResourceTypeEnum.AI_AUTO_TAG:
            ...
            await self.review_tags_repository.update_tag_library_by_manual_tag(original_tag_name, tag_name, resource_type, tenant_id)
        await self.session.commit()
    ...
    async def list_tag_library_by_name(self, keyword: str, tenant_id: int):
        ...
        if tags_list and len(tags_list) > 0:
            for tag in tags_list:
                tag_obj = await self.review_tags_repository.get_tag_info_by_tag(tag, result_dict.get(tag, TagResourceTypeEnum.AI_AUTO_TAG), tenant_id)
                if tag_obj:
    ...
    async def list_review_tag_by_page(self, page: int, page_size: int, tenant_id: int):
        ...
        if group_tag_list and len(group_tag_list) > 0:
            for group_tag in group_tag_list:
                tag_obj = await self.review_tags_repository.get_review_tag_resource_info_by_tag(group_tag["name"], group_tag["resource_type"], tenant_id)
                if tag_obj:
                    result_list.append(tag_obj)
    ...
    async def list_all_tags_library_by_page(self, keyword: str, page: int, page_size: int, tenant_id: int):
        ...
        if tag_list and len(tag_list) > 0:
            for tag in tag_list:
                tag_obj = await self.review_tags_repository.get_tag_info_by_tag(tag["name"], tag["resource_type"], tenant_id)
                if tag_obj:
                    result_list.append(tag_obj)
        # 查找没有的数据
    ...
    async def list_tag_library_by_keyword(self, keyword: str, tenant_id: int):
        ...
        if tags_list and len(tags_list) > 0:
            for tag in tags_list:
                tag_obj = await self.review_tags_repository.get_not_exist_tag_info_by_tag(tag, result_dict.get(tag, TagResourceTypeEnum.AI_AUTO_TAG), tenant_id)
                if tag_obj:
                    result_list.append(tag_obj)
        return result_list
