...
export default function ColFilterUser({ onFilter }) {
    const { t } = useTranslation()
    const [open, setOpen] = useState(false);
    const [value, setValue] = useState([]);

    const { options, setOptions, reload, loadMore, search } = useUsersOptions()
    ...
    return <Popover open={open} onOpenChange={(bln) => setOpen(bln)}>
        ...
        <PopoverContent>
            <div>
                <SearchInput placeholder={'用户名称'} className="w-[240px]" onChange={(e) => searchDb(e.target.value)}></SearchInput>
...
const useUsersOptions = () => {
    ...
    const loadApps = () => {
        ...
        getUsersApi({ page, pageSize: 400, name: keywordRef.current }).then((res: any) => {
            const newOptions = res.data.map(el => ({
                label: el.user_name,
                value: el.user_id
            }))
