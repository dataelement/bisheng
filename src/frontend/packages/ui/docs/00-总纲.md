...
| State 状态页 | 🟨 规范 v1 | 无统一壳，各业务页自拼「插画 + 一行字 + 按钮」（插画本身已收敛：7 张组件 + 120/80 两档，见插画规范附录 A 台账） | 规范 **v1 已建**（2026-08-14：空状态与结果反馈合成一份；插画尺寸只看容器三档、文字排版只看场景两套，两轴独立；一主一次按钮；区域内居中且顶距封顶 200px）；实现待开工——`<State />` 声明式组件（`size` × `variant`），再照插画台账逐处迁移 | [组件-State状态页.md](组件-State状态页.md) |
