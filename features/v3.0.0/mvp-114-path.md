...
- **114 实测环境事实（2026-08-17 只读预检，实现与部署直接按此）**：
  ...
  - **8 核 / 31G 内存，已用 18G、available ≈ 12G**——容量准入阈值必须按**实际 available** 配（不是总量）；轻量档 0.5C/1G 可同时跑十余个演示应用（2026-09-09 实测：五个在线应用合计仅用 0.05% CPU 与 98 MiB，真正的瓶颈是 CPU 配额闸而非机器本身），性能档 4C/8G 只够一个且会挤压平台本体（RT-08「平台必须正常」在这台机器上是真约束）。
...
- **开放 API 管理面开关（2026-09-15 订正）**：beta2 自 `75455c572` 起 `open_api.management_ui_enabled` / `pat_enabled` 默认即开，剧本步 1 不再需要额外打开；存量 `config.yaml` 仍建议显式写 `management_ui_enabled: true`，以免默认值再变。`open_platform.enabled: true` 仍是 `app:manage` 位可签发的前置。两键都是进程级配置，改后全量重启（迁移方案 M14 / §4 步 5）。
...
| 2026-08-19 · **收口批（尾巴 + 批判 + 租户口径）** | 一批合并三件事：**① tasks.md 里登记的 20 条尾巴任务**；**② 一轮完整性批判发现的、清单之外的 8 条真缺陷**；**③ 单租户部署下「租户」概念透出的统一治理**。租户口径已由产品拍板并全批遵守：标准 docker 装机 `multi_tenant.enabled` **缺省 false = 单租户**（仓内开发用的 `config.yaml` 写的是 true，所以开发时永远看不到问题），此时 Root 禁止授予租户管理员（19204）、`tenant_admin` 恒解析为平台超管；**能拿到开关的地方加守卫隐藏（沿用 `appConfig.multiTenantEnabled &&` 既有范式），拿不到开关的地方（client SPA / app-proxy / CLI）改中性表述「平台管理员」，不为文案另拉开关传递链路**；多租户下仍保留「租户管理员」这个精确称呼。**三个真缺陷**：**① compose 形态三修**——`APP_PROXY_*` 环境变量名全错且缺两个、`bisheng-apps` 网络无人创建（rtm 只读不建）、`HostConfig.Binds` 用了容器坐标系而非宿主坐标系；三者**同属一个模式：systemd 形态恰好正确，所以掩盖了 compose 形态**，且全部通过了 `docker compose config`（"验了能解析、没验能不能跑"）。已补 `docker/verify-app-runtime-compose.sh` 静态契约守卫（17 断言，反向自测能把三个缺陷各揪出来）+ `.github/workflows/app-runtime-quality.yml`。**② 广场默认 tab**；**③ app-proxy 的 `Set-Cookie` 被 `dict(headers)` 压平**——多 Cookie 只活下来一条，登录 + CSRF 双 Cookie 的应用（Django / Flask-WTF 最典型）会静默登录失败，而两侧日志都干净。**文档收口**：F054 spec AC-28/29/30 与 PRD-1 兜底页表去租户化（PRD-1 升 v2.1 + 附录 A 留痕）；F055 T042/T043/T051/T050 与 F054 T073 勾选、F054 T074 标 🟡 部分完成；`approval-module` skill 按 `tenant_admin` 语义变更 / 新场景 / `cancel_instance_by_business` / 18118 守卫 / seed 迁移全面同步。⚠️ **批判给出的判据（本批未达成，是下一步的唯一验收口径）**：**「真正的完整判据是：换一台没配过的机器、用 docker-compose、配上首页标签、用非管理员账号，把剧本 1→6 重跑一遍」**——114 是 systemd + 已配好的环境，在 114 跑通不能替代这一条 |
| 2026-09-10 · **open_api 底座改接 beta2** | 合并 `origin/feat/3.0.0-beta2`（da2be5697）+ beta1 tip（59b27d1c1）到 `feat/3.0-vibe/openapi-beta2-base`（HEAD e590ac60a）；`open_api/**` 整体取 beta2 F053、vibe F049 实现归档；应用工场适配进行中（`app_publish` deploy 管线改端点 `open_api_scope("app:manage")` marker + `router_rpc` 的 `verify_open_api_access`、`secret_scanner` 改 `SERVICE_ACCOUNT_KEY_PREFIX` + `bs_pat` 规则、`entry_authz` 去 `user_type`、测试桩改 beta2 `OpenApiPrincipal`、platform / client 前端收口、FGA 模型并集升 `f048-v4`）。**114 不能原地升级**：两侧 `service_account` / `api_credential` 同名不同构，beta2 迁移的 `table_exists` 守卫会跳过建表、启动即崩，须按 [beta2-openapi-base-migration.md §4](./beta2-openapi-base-migration.md) 停服 → 备份 → DROP 两张 vibe 表 → `alembic upgrade head` → 发布 FGA v4 → 重建服务账号与密钥；本轮不部署 114 |
| 2026-09-15 · **二次同步发版线** | 合入 `origin/feat/3.0.0-beta2` 03106e053（merge `8496897b0`）：OpenFGA 模型升 `f048-v6`（本分支与 beta2 各自的 v5 合并，含 contextual 部门成员 + `app`），托管应用入口补单点登录校验，alembic 收单头 `merge_app_factory_beta2_heads`；**114 已按迁移方案 §4 重建并验收**：FGA v6 发布、11 个服务账号与 9 把密钥按原 id / 原哈希恢复（持有人无感、无需重新登录），剧本 CLI deploy → 审批 → 上线 → 同租户非管理员经部门子树授权访问 → logs → 下线删除全链路通过（详见迁移方案 §6）。遗留：form-survey 有一张 `execute_failed` 审批实例挡住迭代发布；~~托管应用删除不清 FGA 投影、不写删除审计~~（F054 既有缺陷，2026-09-16 已修 `2053db896` 并部署 114） |
| 2026-09-16 · **模型版本改用后缀命名** | `MODEL_VERSION` 由 `f048-v6` 改名 `f048-v5-app`：本分支不再占用发版线模型编号，checksum 不变、114 无需重新发布；`app` 进发版线模型暂缓到应用工场正式合入（详见迁移方案 §6） |
