...
class ApprovalQueryRepository:
    ...
    @classmethod
    async def count_pending_tasks_by_approver(cls, tenant_id: int, approver_user_id: int) -> int:
        """Count the approver's pending tasks with SQL COUNT (never loads the rows)."""
        statement = (
            select(func.count())
