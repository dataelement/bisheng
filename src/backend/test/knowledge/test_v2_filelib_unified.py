...
def _patch_identity(monkeypatch):
    """Open API compatibility operators return a fake user (no DB)."""

    async def _fake_default():
        return _FakeUser()

    def _fake_default_sync():
        return _FakeUser()

    monkeypatch.setattr(filelib, "get_open_api_operator_async", _fake_default)
    monkeypatch.setattr(filelib, "get_open_api_operator", _fake_default_sync)
