...
| `POST /v1/intents/preview` | `{instance_id, upstream, phase}`（入参 `{session_id, app_id, version_id, image_ref, tier, port, env, health, timeout}`）；容量不足 → `capacity_exhausted`、探活不过 → `probe_failed`（两种都已把容器拆掉） | **F055 审批期预览拉起（T053）** |
