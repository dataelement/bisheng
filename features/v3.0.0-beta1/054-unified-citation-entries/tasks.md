...
| 实现 | ✅ 已完成 | 14 / 14。基线重跑 15 项绿、相关套件 180 项绿、client typecheck/lint/check-i18n/build 绿；起真实后端（连 116 中间件）验过匿名收紧、原因区分、频道文章全链路。**浏览器 UI 交互待部署后由用户验收**。偏差处理见 `docs/SDD-Guide.md` §3-§4 |
...
- 验收方式：无法在本机驱动浏览器 UI，改为「起真实后端 + HTTP 打接口 + 进程内跑真实 Service」验证行为，前端只做类型分派校验与构建。UI 交互（悬浮卡、点击跳转、置灰观感）留给部署后人工确认。
