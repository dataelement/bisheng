...
| 2026-09-15 · **二次同步发版线** | 合入 `origin/feat/3.0.0-beta2` 03106e053（merge `8496897b0`）：OpenFGA 模型升 `f048-v6`（本分支与 beta2 各自的 v5 合并，含 contextual 部门成员 + `app`），托管应用入口补单点登录校验，alembic 收单头 `merge_app_factory_beta2_heads`；**114 已按迁移方案 §4 重建并验收**：FGA v6 发布、11 个服务账号与 9 把密钥按原样重建，剧本 CLI deploy → 审批 → 上线 → 同租户非管理员经部门子树授权访问 → logs → 下线删除全链路通过（详见迁移方案 §6）。遗留：form-survey 有一张 `execute_failed` 审批实例挡住迭代发布；托管应用删除不清 FGA 投影、不写删除审计（F054 既有缺陷） |
