...
def test_build_inspection_standard_xlsx_bytes_writes_template_headers_and_data():
    ...
    assert standard_sheet.cell(row=2, column=10).value == "1"
    ...
    assert item_sheet.cell(row=2, column=2).value == "1"


def test_build_inspection_standard_xlsx_bytes_writes_numeric_fields_as_text():
    content = build_inspection_standard_xlsx_bytes(
        check_standards=[
            _sample_standard(CHECK_PERIOD=2),
        ],
    ...
    assert standard_sheet.cell(row=2, column=10).value == "2"
    assert item_sheet.cell(row=2, column=6).value == "4"
    assert item_sheet.cell(row=2, column=12).value == "27.5"
    assert item_sheet.cell(row=2, column=13).value == "18"
