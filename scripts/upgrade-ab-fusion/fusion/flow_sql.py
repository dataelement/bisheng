    ...
    *,
    ...
    for fl in flows:
        ...
        tenant = (maps.get("tenant") or {}).get(
            str(fl.get("tenant_id") or "1"), a_tenant_default
        )
        ...
        logo_new, logo_jobs = rewrite_stored_value(
            fl.get("logo"), dst, ref=src, kind="logo"
        )
        logo_sql = (
            sql_json(logo_new)
    ...
    *,
    ...
    for a in assistants:
        ...
        if NAME_SUFFIX not in name:
            ...
            pass
        logo_new, logo_jobs = rewrite_stored_value(
            a.get("logo"), dst, ref=src, kind="logo"
        )
        logo_sql = (
            sql_json(logo_new)
            if isinstance(logo_new, (dict, list))
            else sql_str(logo_new or "")
        )
        lines.append(
    ...
    for link in links:
        ...
        if not asst:
            continue
        tenant = (maps.get("tenant") or {}).get(
            str(link.get("tenant_id") or "1"), a_tenant_default
        )
        ...
        fid_s = (
            (maps.get("flow") or {}).get(str(fid), None)
