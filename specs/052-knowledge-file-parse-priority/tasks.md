...
## 阅读摘要
- 本文档用于指导 Agent 按任务实现角色等级、文件快照、统一投递、Redis 单队列优先消费和排队位置近似展示。
- 每个任务必须保持在声明的边界内；不得拆分队列、增加解析并发或实现公平调度。
- task metadata 字段保持英文固定格式，便于 Agent 稳定读取。
- 推荐执行顺序：公共契约与角色解析 → 文件快照 → 统一投递 → 全入口集成 → Worker 配置 → 排队索引与生命周期 → API/前端 → 发布验证。
...
- [ ] T010 实现 Cluster-safe Redis 排队索引与位置计算
  - 定义逻辑 queue ticket、processing attempt、stage/state schema 和 Repository interface；Redis implementation 使用统一 `{knowledge_parse_queue}` hash tag、全局 sequence、三级 waiting ZSET、attempt member 的 processing/processing lease ZSET、ticket/attempt Hash、ticket→attempt ZSET 和 file→active tickets ZSET，禁止可被后写覆盖的单值 current pointer。
...
- [ ] T011 将 queue ticket 生命周期接入投递与 Worker
  - 统一投递服务生成 `queue_ticket_id` 并复用为 Celery `task_id`；最佳努力创建 publishing ticket，消息显式携带 ticket/stage/priority header，发布后条件转 queued，发布失败最佳努力清理。
  - 新增三个同步任务共享的 processing lease guard：每次 Worker delivery 生成全新不可预测 `processing_attempt_id`，入口 begin/rebuild logical ticket 后创建独立 attempt，任务期间用同步 Redis connection 只为自身 attempt 周期续期，成功、失败、撤销和正常结束时停止自身心跳并 fenced 清理自身；只有无其他有效 attempt 时才清理逻辑 ticket，初始化/续期失败只记日志，不改变任务业务结果。
  - 标题→正式解析、显式重试、重新解析和并发重复请求均为每个真实消息创建独立 ticket/sequence；同一文件的旧任务只能清理自身。相同 Celery `task_id` 每次重新领取时复用逻辑 ticket 但创建新 attempt，旧版本无 ticket 消息安全降级；不得修改全局 `visibility_timeout` 或把 Redis 旁路变为业务去重锁。
  - 增加生命周期集成测试，覆盖发布方与 Worker 竞态、publishing→processing 快路径、标题换票、同文件并发双 ticket、attempt 心跳续期、Worker 强退/心跳停止、租约有界过期、visibility timeout 在原执行完成前重投递同 task ID、两个 attempt 同时计数、原 attempt 先结束与新 attempt 先结束两种顺序、旧 attempt 过期、旧消息无 ticket 和索引失败不中断解析。
  - Done when: 三个白名单任务以逻辑 ticket 表达真实消息，以独立 attempt 表达每次实际执行；Worker 强退后 active count 在租约窗口内收敛，同 task ID 重叠 delivery 互不续期/清理且最后一个有效 attempt 消失前保持 processing，任一旧 ticket 无法删除同文件其他 ticket，旁路故障不改变 Celery 发布和文件状态机。
  - _Requirements: REQ-004, REQ-006, REQ-007_
  - _Acceptance: AC-REQ-004-01, AC-REQ-004-02, AC-REQ-004-04, AC-REQ-006-04, AC-REQ-007-03, AC-REQ-007-04, AC-REQ-007-07, AC-REQ-007-09, AC-REQ-007-10, AC-REQ-007-12_
  - _Verification: V-AC-REQ-004-01, V-AC-REQ-004-04, V-AC-REQ-006-04, V-AC-REQ-007-03, V-AC-REQ-007-07, V-AC-REQ-007-09, V-AC-REQ-007-10, V-AC-REQ-007-12; EG-004, EG-006, EG-007_
  - _Depends: T007, T010_
  - _Boundary: unified parse dispatcher, three knowledge task lifecycle paths, and lifecycle tests_

- [ ] T012 增加安全批量位置 API 和文件上传进度展示
  ...
  - Platform controller 增加有类型查询；`FileUploadStep4` 在现有 5 秒状态轮询内批量查询未结束文件，禁止同轮重复并发；`ProgressItem` 展示阶段、“前方约 N 个等待任务”和独立运行数，终态停止，unavailable 退化为普通排队文案且不高频 toast。
...
- [ ] T009 完成发布文档与跨边界验收证据
  - 更新 knowledge 架构与部署文档：角色/文件等级语义、`0/3/9` 映射、prefetch 1、逻辑 ticket/逐 delivery attempt 索引、attempt lease/心跳/fencing、文件级权限、近似口径、并发不变、全局 visibility timeout 不变、组合入口限制、Worker→producer→frontend 部署顺序、无 purge 和回滚步骤。
  - 在非生产 Redis 以独立 knowledge Worker `-c 1` 发布交错的低/中/高等待消息，记录高→中→低、同级 FIFO、位置公式、同文件多 ticket、阶段换票、Worker 强退后租约收敛、同 task ID 在原执行未完成时重投递、两个 attempt 同时计数与两种结束顺序、索引故障降级证据；注明已执行任务/预取任务不可抢占，旁路不防止业务重复执行。
  ...
  - Done when: EG-001～EG-007 均有可追踪证据，真实 Redis 顺序、逻辑 ticket/attempt 生命周期、attempt lease 有界收敛、重叠 delivery 隔离、文件级权限与双库 migration 门禁满足，发布/回滚不包含全局 visibility timeout 变更、broker purge、broker key 删除或消息迁移。
  - _Requirements: REQ-001, REQ-002, REQ-003, REQ-004, REQ-005, REQ-006, REQ-007_
  - _Acceptance: AC-REQ-001-01, AC-REQ-001-02, AC-REQ-001-05, AC-REQ-001-06, AC-REQ-002-01, AC-REQ-002-06, AC-REQ-003-01, AC-REQ-003-04, AC-REQ-003-05, AC-REQ-004-01, AC-REQ-004-02, AC-REQ-004-03, AC-REQ-004-04, AC-REQ-005-01, AC-REQ-005-02, AC-REQ-005-03, AC-REQ-005-04, AC-REQ-005-05, AC-REQ-005-06, AC-REQ-006-01, AC-REQ-006-02, AC-REQ-006-03, AC-REQ-006-04, AC-REQ-006-05, AC-REQ-007-01, AC-REQ-007-02, AC-REQ-007-03, AC-REQ-007-04, AC-REQ-007-05, AC-REQ-007-06, AC-REQ-007-07, AC-REQ-007-08, AC-REQ-007-09, AC-REQ-007-10, AC-REQ-007-11, AC-REQ-007-12, AC-REQ-007-13_
  - _Verification: V-AC-REQ-001-01, V-AC-REQ-001-02, V-AC-REQ-001-05, V-AC-REQ-001-06, V-AC-REQ-002-01, V-AC-REQ-002-06, V-AC-REQ-003-01, V-AC-REQ-003-04, V-AC-REQ-003-05, V-AC-REQ-004-01, V-AC-REQ-004-03, V-AC-REQ-004-04, V-AC-REQ-005-01, V-AC-REQ-005-02, V-AC-REQ-005-03, V-AC-REQ-005-05, V-AC-REQ-006-01, V-AC-REQ-006-02, V-AC-REQ-006-03, V-AC-REQ-006-04, V-AC-REQ-007-01, V-AC-REQ-007-02, V-AC-REQ-007-03, V-AC-REQ-007-06, V-AC-REQ-007-07, V-AC-REQ-007-09, V-AC-REQ-007-10, V-AC-REQ-007-11, V-AC-REQ-007-12, V-AC-REQ-007-13; EG-001, EG-002, EG-003, EG-004, EG-005, EG-006, EG-007_
  - _Depends: T003, T007, T008, T012_
...
| REQ-004 | AC-REQ-004-01 | T006, T007, T008, T009 | V-AC-REQ-004-01 |
| REQ-004 | AC-REQ-004-02 | T007, T009 | V-AC-REQ-004-01 |
| REQ-004 | AC-REQ-004-03 | T007, T009 | V-AC-REQ-004-03 |
| REQ-004 | AC-REQ-004-04 | T006, T007, T009 | V-AC-REQ-004-04 |
| REQ-004 | AC-REQ-004-05 | T001, T006 | V-AC-REQ-004-05 |
...
| REQ-005 | AC-REQ-005-05, AC-REQ-005-06 | T008, T009 | V-AC-REQ-005-05 |
...
| EG-004 | 投递与全入口继承 | Dispatch/task contract、source guard、受影响 knowledge 回归 |
...
| EG-007 | 逻辑 ticket、processing attempt/lease、位置 API 与 UI | Redis lifecycle/rank、多 ticket 并发、Worker 强退、同 task ID 重叠 delivery/两种结束顺序、文件级权限/部门审批、API 参数边界、Platform/Client 组件测试、非生产 Worker 冒烟 |
