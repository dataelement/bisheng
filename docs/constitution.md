...
| 26x–27x (open / public API · license) | 260 open_api · 261 public_endpoints · **262 model_face (F051)** · **263 mcp_face (F052)** · 270 commercial_license |
...
- **262 is assigned** to the OpenAI-compatible model protocol face (`/api/v2/model/v1`; F051, `common/errcode/model_face.py`). Do not reuse it. Sub-bands: `26200–26209` protocol face / endpoint / request shape (26201 endpoint not offered · 26202 Anthropic path · 26203 malformed request · 26204 access token refused · 26205 delegation-style identity header refused) · `26210–26229` callable range and name resolution (26211 not found · 26212 offline · 26213 revoked · 26214 ambiguous bare name · 26215 capability undeclared · 26216 catalog undecidable, fail-closed 503 · 26217 provider daily limit, 429) · `26230–26249` upstream and streaming (26231 upstream failure · 26232 upstream rejection, transport status taken from the provider · 26233 upstream rate limit · 26234 stream interrupted, only inside an SSE event) · `26250–26259` records and ledger, reserved. `ModelFaceError` subclasses `OpenApiAuthError` purely to reuse the v2 handler's `http_status` branch — they are not authentication errors. Each code also carries `openai_type` / `openai_code`, the OpenAI-shaped rendering the face returns under its own path prefix; copy for each code must land in `packages/locales/src/api_errors/*.json` (all three languages) in the same change, and codes here are declared `Code: int = NNNNN` so `check-i18n` actually sees them.
