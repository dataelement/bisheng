...
| F051-aliyun-model-rate-limit-recovery | F035、现有工作台日常/知识空间 AI/订阅频道 AI 会话 | 只处理阿里百炼明确临时限流；新增 Redis 模型繁忙投影、默认 Celery 有界模型探测与日常/知识/频道当前页面主动恢复，任务模式完整沿用 F035 既有自动重试、失败处理和用户 Retry；原请求读取既有业务记录，页面内维护三次提示，后端不新增执行表或严格次数状态；真实限流复用 12046/11090，新增 12048 表达非任务恢复拒绝；后台探测必须遵守 INV-9，模型恢复不得自动重发用户会话 |
