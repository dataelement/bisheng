...
async def test_apply_menu_access_request_uses_gate_when_mode_enabled():
    ...
    with (
        ...
        patch(
            "bisheng.approval.domain.services.approval_center_service.LoginUser.compute_menu_approval_mode_for_menu",
            new_callable=AsyncMock,
            return_value=True,
        ),
        patch(
    ...
    req = (
        mock_request.await_args.kwargs
        if mock_request.await_args and mock_request.await_args.kwargs
        else mock_request.await_args.args[0]
    )
...
@pytest.mark.asyncio
async def test_apply_admin_menu_rejected_when_only_workbench_scope_enabled():
    """An admin-area menu can't be applied for when only the workbench scope is on."""
    ApprovalCenterService = importlib.reload(approval_center_service_module).ApprovalCenterService
    login_user = SimpleNamespace(user_id=7, user_name="alice", tenant_id=1)
    db_user = SimpleNamespace(user_id=7, tenant_id=1)
    ...
    with (
        ...
        patch(
            # workbench scope on, admin scope off → the resolver returns False for 'knowledge'.
            "bisheng.approval.domain.services.approval_center_service.LoginUser.compute_menu_approval_mode_for_menu",
            new_callable=AsyncMock,
            return_value=False,
        ),
        ...
        with pytest.raises(ApprovalMenuApplyDisabledError):
            await ApprovalCenterService.apply_menu_access_request(
                login_user=login_user,
                menu_key="knowledge",
                menu_name="知识管理",
                reason="need access",
...
async def test_revoke_menu_grant_uses_instance_payload_menu_key():
    ...
    with (
        ...
        patch(
            "bisheng.approval.domain.services.approval_center_service.ApprovalInstanceRepository.create_action_log",
            new_callable=AsyncMock,
        ) as mock_action_log,
        patch(
    ...
    mock_action_log.assert_awaited_once()
    logged_action = mock_action_log.await_args.args[0]
    assert logged_action.action == "revoke_grant"
    assert logged_action.detail == {"reason": "manual revoke", "menu_key": "knowledge"}
    assert logged_action.operator_user_name == "admin"
