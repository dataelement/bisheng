...
**成功响应**：HTTP 状态码为 `200`，响应体中的业务状态码 `status_code` 也为 `200`。
...
| `separator_rule` | array[string] | 否 | 每项仅允许 `before` 或 `after`；不传时使用默认分隔规则。 |
| `retain_images` | integer | 否 | 保留文档图片，仅允许 `0`（关闭）或 `1`（开启），默认 `1`。 |
| `force_ocr` | integer | 否 | 强制 OCR，仅允许 `0` 或 `1`，默认 `0`。 |
| `enable_formula` | integer | 否 | 公式识别，仅允许 `0` 或 `1`，默认 `1`。 |
| `filter_page_header_footer` | integer | 否 | 过滤页眉页脚，仅允许 `0` 或 `1`，默认 `0`。 |

上述参数通过表单传入，数组使用同名字段重复传值。身份及权限校验通过后，非法解析参数返回 HTTP `400`、业务 `status_code=400`，`status_message` 中包含错误字段位置；不会进入本地文件缓存、远程文件下载或文件处理流程，也不会将非法值保存到 `split_rule`。
