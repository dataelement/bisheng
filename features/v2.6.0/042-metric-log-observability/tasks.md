...
- T011 偏差：E+ skipped 只在 forwarder **过白名单后**的收件人解析 skip 打，`feature_disabled`/`not_in_whitelist` 不打（高频刷屏）→ design §5 坑 12 + §6.1。另 `eplus_notify` 加 `reason`(skipped) / `err_code`(异常) 字段。纯实现细节。
