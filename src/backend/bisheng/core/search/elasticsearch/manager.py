...
async def get_es_connection() -> AsyncElasticsearch:
    ...
    try:
        ...
        logger.warning("EsConnManager not found in app_context. Registering a new instance.")
        try:
            from bisheng.common.services.config_service import settings

            app_context.register_context(
                EsConnManager(
                    es_hosts=settings.get_telemetry_conf().elasticsearch_url, **settings.get_telemetry_conf().ssl_verify
                )
            )
            return (await app_context.async_get_instance(EsConnManager.name)).es_connection
        except Exception as e:
            logger.error(f"Failed to register EsConnManager: {e}")
            raise KeyError("EsConnManager not found in app_context and failed to register a new instance.") from e
...
async def get_statistics_es_connection() -> AsyncElasticsearch:
    ...
    try:
        ...
        logger.warning("Statistics EsConnManager not found in app_context. Registering a new instance.")
        try:
            from bisheng.common.services.config_service import settings

            app_context.register_context(
                EsConnManager(
                    es_hosts=settings.get_telemetry_conf().elasticsearch_url,
            ...
            raise KeyError(
                "Statistics EsConnManager not found in app_context and failed to register a new instance."
            ) from e
