...
- **hideCancel（告知态）**：在 `default` 基础上隐藏取消按钮，只留一个主按钮——用户只有一条出路的失败提示。
...
function Demos() {
  ...
  const rows = [
    ...
    [
      '告知态 hideCancel — 用户只有一条出路的失败提示',
      () =>
        confirm({
          title: '公众号文章链接未识别',
          description: '暂时无法读取该链接，请确认链接完整、页面可以正常访问。',
          confirmText: '返回修改',
          hideCancel: true,
        }),
    ],
...
| 取消按钮 | 白底描边 `hover:bg-[#f7f8fa]`，`focus-visible` 焦点环 | 默认文案「暂不」；`hideCancel` 时不渲染 |

## 使用规则

- 危险操作（删除等）用 `variant: 'destructive'`；可逆但需确认的用 `default`。**不要自拼确认弹窗**。
- 只有一个出路的「告知型」提示（如链接解析失败，用户只能返回修改）用 `hideCancel: true`：隐藏取消按钮，只留主按钮；此时 `confirm()` 的返回值无意义，调用方 `void` 掉即可。
