...
def _rows_from_map(path: Path, src_key: str, kind: str) -> list[dict]:
    ...
    for row in load_csv(path):
        ...
        dst = (row.get("a_model_id") or row.get("a_tool_id") or row.get("a_id") or "").strip()
        if action in {"bind", "create"} or dst:
            continue
        out.append(
            {
                "kind": kind,
                ...
                "note": row.get("note") or row.get("reason") or "未映射",
            }
        )
    return out
...
def _rows_from_manual(path: Path, src_key: str, kind: str) -> list[dict]:
    ...
    for row in load_table(path):
        out.append(
            {
                "kind": kind,
                "b_id": row.get(src_key) or row.get("b_model_id") or row.get("b_tool_id") or "",
                "action": "manual",
...
def collect_gaps(map_dir: Path, propose_dir: Path | None = None) -> list[dict]:
    ...
    rows.extend(_rows_from_manual(src / "model-map.conflicts.csv", "b_model_id", "model"))
