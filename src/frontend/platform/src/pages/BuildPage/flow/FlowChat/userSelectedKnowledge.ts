...
export const USER_SELECTED_KNOWLEDGE_NODE_TYPES = [
...
export function validateRuntimeKnowledgeSelection(selection?: RuntimeKnowledgeSelection | null): string {
    ...
    if (selection.mode === "source") {
        if (!selection.whole_source?.source_id || selection.whole_source.source_type !== "space") {
            return "请选择知识空间。";
        }
        if (selection.items?.length) {
            return "完整知识来源不能与文件或文件夹范围同时选择。";
        }
        return "";
    }
    if (selection.mode !== "items" || !selection.items?.length) {
        return "请选择知识空间。";
    }
