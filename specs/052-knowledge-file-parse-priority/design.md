...
## 阅读摘要
- 本文档说明：在不拆分 `knowledge_celery`、不增加 Worker 总并发的前提下，把角色配置解析为文件级不可变快照，通过 Redis transport 完成三级调度，并以应用侧 Redis 索引向用户提供当前排队位置的近似快照。
- 设计重点：角色等级解析、文件首次入队原子固化、统一投递服务、标题提取/正式解析/重试显式继承、专用 knowledge Worker 的 Redis priority 与预取配置、逻辑 ticket 生命周期、每次 Worker delivery 独立 processing attempt、attempt 租约收敛和文件级安全批量查询。
...
- 现有架构 Existing architecture:
  ...
  - `knowledge_celery` 通过任务白名单只承载标题提取、首次解析和解析重试三个任务；标题提取完成后会再次发布正式解析任务。
  - 三个解析任务均使用 `acks_late=True` 且没有任务级硬时限；当前 Redis broker 配置未覆盖 `visibility_timeout`，Kombu 5.5.4 默认值为 3600 秒，因此大文件解析可能在原 delivery 尚未结束时被恢复并由另一 Worker 再次领取。
...
- 统一所有三类解析任务的投递入口，显式设置队列、Celery priority 与可观测 header。
- 让独立 knowledge Worker 按 Redis priority steps 消费并使用预取 1，同时保持原并发及其他队列不变。
- 为每个真实消息建立独立、可过期 queue ticket；同一文件的并发或阶段交接消息可同时存在，按优先级和同级序号计算当前 `ahead_waiting_count`，索引故障时不影响解析主链路。
- 为每次 Worker delivery 建立独立 processing attempt 和短周期可续期租约，使 Worker 强制退出后的幽灵活动记录在有界时间内退出 `active_count`，并隔离同一 task ID 的重叠执行。
- 通过同时校验知识库与现有文件级有效可见性的批量接口和现有 5 秒轮询，在 Platform 文件上传解析进度区展示近似排队数量、阶段和运行任务数；首钢门户上传记录使用不含阶段/运行数的简化排队文案，并在已有数据刷新时原位合并字段。
- 提供滚动升级、历史数据、存量 broker 消息和应用回滚的兼容路径。

### 非目标 Non-Goals
- 不设计公平调度、租户配额、等待老化或中低优保障时间。
- 不修改文件解析算法、文件状态机、任务参数或结果存储。
...
| 解析投递 | 新增统一服务并替换三任务的直接发布点 | 修改任务名称、参数、状态机、callback、preview key、`acks_late` | 新增第四类解析任务或消息 schema 变化 |
...
| REQ-004 | AC-REQ-004-01～05 | 统一解析投递服务、所有生产入口替换、后继任务重读快照、任务级中优默认值 | 发布参数 contract test + AST/source guard + 受影响回归 |
...
| REQ-007 | AC-REQ-007-01～13 | 应用侧 Redis 逻辑 ticket 索引、逐 delivery processing attempt/租约、多 ticket 归并、文件级有效权限、批量位置 API、Platform 上传进度和首钢门户上传记录展示及安全降级 | Redis/worker lifecycle 集成测试 + 同 task ID 重叠 delivery/并发多 ticket 测试 + API 权限测试 + Platform/Client 前端组件测试 |

## 架构设计 Architecture
- Pattern: `角色配置 → 有效等级解析 → 文件级不可变快照 → 统一消息投递 + 逻辑 ticket → Redis 单队列优先消费 → 每次 Worker delivery 建立 attempt 并回写生命周期 → 安全位置查询`。
- Rationale: 文件快照同时解决角色变更、标题后继、重试和多入口一致性；集中投递避免任一 `.delay()` 绕过 priority；旁路 ticket 索引提供可查询位置且不把 UI 可观测性变成解析依赖；单队列保持现有容量模型。
- Preserved existing patterns: 继续使用角色 `quota_config`、knowledge Repository、Celery 三任务白名单、tenant header 注入和独立 Worker 进程。
...
- 三个白名单任务的任务级默认 priority 设为中优 `3`，作为遗漏显式等级时的安全默认；统一投递仍必须显式传值。
...
- `...:{knowledge_parse_queue}:ticket:{ticket_id}`：逻辑消息 Hash，保存 `tenant_id`、`knowledge_id`、`file_id`、`stage`、内部 priority、sequence、逻辑 state 和时间；`queue_ticket_id` 继续复用 Celery `task_id`，但不作为单次执行身份。
...
- `...:{knowledge_parse_queue}:attempt:{attempt_id}`：单次 delivery Hash，保存所关联的 `queue_ticket_id`、`tenant_id`、`knowledge_id`、`file_id`、`stage`、开始时间、最近心跳和租约截止时间。
- `...:{knowledge_parse_queue}:file:{tenant_id}:{file_id}:tickets`：该文件所有活动 ticket 的 ZSET，member 为 ticket、score 为 sequence；不得使用会被后写覆盖的单值 current pointer。单个 ticket 清理只 `ZREM` 自身，集合为空时才删除 key。
...
1. 投递服务本地生成 `queue_ticket_id`，同时将其用作显式 Celery `task_id`，并确定 `stage=title|parse|retry`。
...
3. `apply_async` 除现有参数外显式携带 `knowledge_parse_ticket_id`、`knowledge_parse_stage` 和内部 priority header。
...
10. 标题任务发布正式解析、显式重试、重新解析或并发重复请求时，每个真实 broker 消息都创建独立逻辑 ticket 和 sequence；Redis 旁路不承担消息去重或发布锁职责。
...
- 查询前先清理过期 attempt 租约，再用数据库文件状态和文件活动 ticket ZSET 做一致性检查。每个文件先过滤已过期、归属不匹配或 metadata 缺失的 ticket/attempt：若存在有效 processing attempt，返回最早开始 attempt 所关联 ticket 的 stage；否则从 queued ticket 中按业务优先级、再按 sequence 选择实际调度最靠前者并计算位置。publishing-only、无可靠 ticket/attempt 或状态冲突时返回 unavailable，而不是返回 0。
...
- Response: 使用 `resp_200` 返回 `{items, active_count, approximate: true, as_of}`；item 为 `{file_id, state, stage, ahead_waiting_count}`，其中 state 为 `queued|processing|not_queued|unavailable`，只有 queued 可返回非空数量。
...
- Platform: `FileUploadStep4` 在现有状态轮询周期内批量请求尚未结束文件的位置，默认每 5 秒且禁止同轮并发；`ProgressItem` 显示阶段、约数和独立 active count，全部文件终态后停止。
...
| `src/backend/bisheng/knowledge/domain/services/knowledge_parse_dispatch_service.py` | create | 固化/读取快照、生成 ticket 并统一发布三类解析任务 | REQ-003, REQ-004, REQ-007 |
| `src/backend/bisheng/knowledge/domain/repositories/interfaces/knowledge_parse_queue_repository.py` | create | 声明逻辑 ticket 创建/状态 CAS、attempt lease/fenced 清理、多 ticket/attempt 查询与批量排名接口 | REQ-007 |
...
| `src/backend/bisheng/knowledge/domain/services/knowledge_parse_queue_service.py` | create | 组合文件状态、有效可见文件集合与多个 Redis ticket/attempt，计算安全位置响应 | REQ-007 |
| `src/backend/bisheng/knowledge/domain/schemas/knowledge_parse_queue_schema.py` | create | 定义逻辑 ticket、attempt、阶段/状态和批量位置响应 schema | REQ-007 |
...
| `src/backend/bisheng/worker/knowledge/{file_title_worker.py,file_worker.py}` | modify | 后继与重试显式按文件快照投递，设置任务默认等级，并用共享 guard 回写逐 delivery attempt 生命周期 | REQ-004, REQ-007 |
| `src/backend/scripts/enqueue_reparse_knowledge_space_files.py` | modify | 运维重解析脚本接入统一投递 | REQ-004 |
| `src/backend/bisheng/core/config/{celery_redis.py,celery_queues.py}` | modify | 声明兼容映射和三任务中优默认，保持白名单 | REQ-004, REQ-005 |
...
| `src/backend/test/knowledge/` | create/modify | 覆盖快照、投递、继承、排队排名/生命周期、权限与失败路径 | REQ-003, REQ-004, REQ-006, REQ-007 |
...
| `src/frontend/platform/src/components/bs-comp/knowledgeUploadComponent/{index.tsx,ProgressItem.tsx}` | modify | 扩展可选排队字段并展示阶段、约数和运行数量 | REQ-007 |
| `src/frontend/platform/src/test/parseQueuePosition.test.tsx` | create | 覆盖 queued/processing/unavailable/终态和动态约数文案 | REQ-007 |
| `src/frontend/client/src/api/knowledge.ts` | modify | 增加 Client 有类型的批量排队位置查询函数 | REQ-007 |
...
| `docs/architecture/{04-knowledge-rag.md,08-deployment.md}` | modify | 记录等级、排队索引语义、Worker 启动和发布/回滚约束 | REQ-005, REQ-006, REQ-007 |
...
### 统一解析投递服务
- Responsibility: 确保文件快照存在，把业务等级映射为消息 priority，最佳努力创建可观测 ticket，并投递指定白名单任务。
- Inputs: 文件或 `file_id`、投递类型（标题/解析/重试）、现有任务参数、可选当前操作者上下文。
- Outputs: Celery async result 或与现有调用方等价的发布结果。
- Dependencies: 文件 Repository、角色等级解析服务、排队索引 Repository、三个现有 Celery task。
- Error behavior: `apply_async` 显式设置 `queue="knowledge_celery"`、priority、stage 与 ticket header；排队索引失败记录后继续发布；broker 发布失败仍向上传播并最佳努力清理 ticket，不引入 outbox 或静默吞错。
...
- Responsibility: 在三个同步 Celery task 的每次 delivery 期间生成并维护独立 `processing_attempt_id` 的短周期租约，保证正常退出只停止自身心跳和幂等清理自身。
- Inputs: 逻辑 `queue_ticket_id`/ticket header、stage、priority、file ID，以及可配置 heartbeat/lease duration；guard 内部为本 delivery 生成 attempt identity。
...
- Outputs: Platform 输出 `标题提取/正式解析/重试排队中，前方约 N 个等待任务` 以及独立运行数；首钢门户 queued 状态只输出“排队中，前方约 N 个等待任务”，processing/unavailable 保留原文件状态文案。
...
2. 先部署可识别 ticket header、安全忽略缺失 ticket、支持逐 delivery attempt lease 和同 task ID 重叠执行隔离的 knowledge Worker 生命周期代码；独立入口同时增加 `--prefetch-multiplier=1`，原 `-c` 数值不变。
3. 再部署包含枚举兼容、快照、统一投递、排队索引和位置 API 的 Web/producer；新旧角色、文件空值和旧消息均可安全读取。
4. 后端全部具备新角色键与排队 API 后，再部署 Platform 角色配置和排队位置 UI。
5. 不运行 purge 或 Redis broker key 操作；等待旧消息自然消费，应用侧索引残留只通过自身 TTL 清理。
6. 在非生产 Redis 以 `-c 1` 发布交错的低、中、高任务，确认等待消息按高→中→低、同级 FIFO 取得，并对比位置 API 的近似公式、同文件多 ticket、attempt 心跳停止、同 task ID 重叠 delivery 两种结束顺序和阶段生命周期。
...
| AC-REQ-004-01～05 | high/V2 | 三任务参数；标题后继；全部入口；遗漏默认；旧行为保持 | service/task contract + source guard | EG-004 | 所有已知入口无直接绕过，受影响回归只运行一次并通过 |
...
| AC-REQ-007-01～13 | high/V3 | 三级排名公式；阶段换票；attempt lease/强退收敛；同 task ID 重叠 delivery；同文件多 ticket；文件级权限隔离；索引失败降级；Platform/Client UI 近似文案 | Redis/worker lifecycle integration + overlapping delivery/concurrent dispatch + API security + Platform/Client component | EG-007 | 真实 Redis 证明 attempt 租约有界收敛、同 task ID 两个 delivery 独立计数且两种结束顺序互不覆盖，文件级越权/审批路径拒绝，Platform/Client UI 状态均有证据；不扫描 broker 验证 |
...
- Context: 现有重试入口是数据库状态检查后再发布，无法与 Celery broker 组成原子去重；标题任务发布正式解析时也会短暂存在阶段交接重叠。旁路索引不得成为发布强依赖。
...
- Rationale: 能表达现有并发和阶段交接事实，不要求 Redis 可用才能发布，也不把本功能扩大为消息幂等/outbox 重构。
...
- Context: 三个解析任务均使用 `acks_late=True` 且没有任务级硬时限；Kombu Redis 默认 visibility timeout 可能在长任务结束前恢复同一消息，使相同 Celery `task_id` 的两个 delivery 重叠运行。若 processing 只以 task ID/ticket 为 member，先结束的 delivery 会删除仍在运行的另一个 delivery。
...
- Decision: 三个任务共享 processing lease guard，每个 delivery 的 attempt 运行时周期续期并在正常结束时只清理自身；心跳停止后由写入/查询在租约窗口内惰性剔除，其他 attempt 不受影响。
...
### Decision: Platform 展示当前阶段快照，首钢门户上传记录展示简化等待数
- Context: 标题提取与正式解析会真实地重新入队，后到高优任务也能插到中低优之前；文件大小和解析耗时差异很大。
- Options considered: 全文件链路固定名次；预计开始时间；当前阶段快照；只展示总队列长度。
- Decision: API 保留 `stage`、`ahead_waiting_count` 和独立 `active_count`；Platform queued 状态展示阶段、近似等待数及运行数，首钢门户 queued 状态只展示近似等待数，processing 使用原文件状态。两端都强制按 `approximate=true` 语义使用“约”。
- Rationale: 该含义能和实际严格优先/FIFO 规则对应，不对用户作无法兑现的稳定倒计时承诺。
- Consequences: 中低优数字可能增加；阶段切换后位置会重新计算，前端必须使用“约”和降级文案。首钢门户以更少信息换取更简洁的上传记录状态，后端契约和 Platform 展示不变。
