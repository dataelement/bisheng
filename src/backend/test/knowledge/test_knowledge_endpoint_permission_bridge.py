...
async def test_copy_knowledge_uses_permission_service_read_async_bridge():
    ...
    with (
        patch.object(module.UserPayload, "assert_effective_web_menu_contains", new_callable=AsyncMock),
        patch(
...
def test_get_filelist_file_ids_is_a_query_parameter():
    module = _load_endpoint_module()
    route = next(route for route in module.router.routes if route.path == "/knowledge/file_list/{knowledge_id}")

    assert "file_ids" in {field.name for field in route.dependant.query_params}
    assert "file_ids" not in {field.name for field in route.dependant.body_params}
...
def test_switch_model_uses_permission_service_write_sync_bridge():
    ...
    req_data = SimpleNamespace(
        knowledge_id=16, model_id=2, model_type="embedding", knowledge_name="new", description="desc"
    )
