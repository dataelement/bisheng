...
**当前冻结基线：`contract_version=0.3.0`。2026-09-09 原0.1.0冻结后，用户确认逐模型额度，并明确取消未知用量冻结及部门同步/筛选；0.3.0 更新成功响应中的 nullable usage 语义。** [client-api.md](./client-api.md) 及其 7 张时序图是本 Design 的规范性附件，不是可被服务端自行替换的示例。冻结不代表接口已上线，也不自动关闭 §8 的其他产品待决项。
...
当前修订 **0.3.0**：继承 0.2.0 的逐模型独立额度、usage 可选 model 参数及逐模型剩余量汇总；新增成功响应 nullable usage、缺少用量不冻结、移除部门筛选/同步。新版 config 返回 0.3.0，旧 JSON 仅保留用于差异审阅，不代表双版本运行时兼容。客户端接收确认与真实联调尚待完成，禁止将文档更新等同于已同步。

### 6.1 接口清单（26 个端点已接线，客户端契约 0.3.0 冻结）
