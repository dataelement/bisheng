...
def patch_tenant_dao():
    ...
    def _patch(parent_tenant_id):
        ...
        return patch("bisheng.database.models.tenant.TenantDao.aget_by_id", AsyncMock(return_value=fake_tenant))
...
async def test_l3_rejects_when_resource_tenant_not_visible(login_user_factory, patch_resolver):
    ...
    with patch_resolver(10), patch.object(PermissionService, "_is_shared_to", AsyncMock(return_value=False)):
...
@pytest.mark.asyncio
async def test_l3_allows_via_shared_to_when_not_visible(login_user_factory, patch_resolver, patch_tenant_dao):
...
async def test_list_accessible_ids_filters_objects_failing_tenant_gate(login_user_factory):
    ...
    with (
        ...
        patch.object(PermissionService, "_resource_ids_by_creator_user_ids", AsyncMock(return_value=[])),
        patch.object(PermissionService, "_resource_ids_child_tenant_admin_scope", AsyncMock(return_value=[])),
        patch.object(PermissionService, "_resource_tenant_map", AsyncMock(return_value={"wf-1": 9})),
        patch.object(PermissionService, "_is_shared_to", AsyncMock(return_value=False)),
    ):
...
@pytest.mark.asyncio
async def test_l4_skips_when_parent_tenant_id_null(login_user_factory, patch_resolver, patch_tenant_dao):
...
async def test_check_without_login_user_skips_tenant_gating():
    ...
    with (
        patch.object(PermissionService, "_get_fga", return_value=fake_fga),
        patch.object(PermissionService, "_get_resource_creator", AsyncMock(return_value=None)),
        patch.object(PermissionService, "_resolve_resource_tenant", AsyncMock(return_value=10)) as resolver,
    ):
        result = await PermissionService.check(
            user_id=100,
            relation="viewer",
...
async def test_resolver_dao_exception_returns_none():
    ...
    with patch("bisheng.database.models.flow.FlowDao.aget_flow_by_id", AsyncMock(side_effect=RuntimeError("db down"))):
...
async def test_resolver_returns_tenant_id_for_tenant_gated_resources():
    ...
    with (
        patch("bisheng.database.models.flow.FlowDao.aget_flow_by_id", AsyncMock(return_value=workflow)),
        patch("bisheng.database.models.assistant.AssistantDao.aget_one_assistant", AsyncMock(return_value=assistant)),
        patch("bisheng.knowledge.domain.models.knowledge.KnowledgeDao.aquery_by_id", AsyncMock(return_value=knowledge)),
    ):
...
async def test_resource_tenant_map_reads_tenant_ids_from_models():
    ...
    with (
        patch("bisheng.database.models.flow.FlowDao.aget_flow_by_ids", AsyncMock(return_value=[workflow])),
        patch(
            "bisheng.database.models.assistant.AssistantDao.aget_assistants_by_ids", AsyncMock(return_value=[assistant])
        ),
        patch(
