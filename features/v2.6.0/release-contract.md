...
| LinsightSkill（`linsight_skill` 表 + 对象存储 bundle 生命周期） | F035-linsight-task-mode | 租户自定义技能元数据（name/description/enabled/source，唯一约束 `(tenant_id,name)`）与 bundle 正文的创建/编辑/删除/启停（正文存对象存储，行上的 `content_hash` 是版本指针；v3.0 起，见变更记录）；内核 built-in skill 不入表、不经 API。同时拥有：自研 ReAct 内核与 SOP 动态生成链路的**下线**、`linsight_sop`→Skill 一次性迁移写行为；只读 `linsight_session_version`/`linsight_execute_task`（沿用既有模型，不改其归属） |
...
| 2026-08-14 | F035 存储形态修订：Skill bundle 正文由**节点本地磁盘**（`SKILLS_ROOT`）改为**对象存储内容寻址**（`linsight/skills/{tenant_id}/{name}/{content_hash}.zip`），`linsight_skill` 新增 `content_hash` 列、`object_path` 语义改为对象键；节点按内容哈希本地物化。原设计把多节点一致性降级为「运维须挂共享卷」的约定且从未落地，导致 A 机上传的技能在 B 机 worker 上静默不生效。同步新增架构宪法 C8（禁止本地文件系统承载跨进程共享状态） | F035 |
