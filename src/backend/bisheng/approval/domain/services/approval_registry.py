...
# Always allowed on every scenario (matches Platform ALWAYS_INCLUDED / identity routing).
UNIVERSAL_CONDITION_FIELDS = frozenset({"applicant_role"})
...
def _complete_preset(preset: ApprovalScenarioPreset) -> ApprovalScenarioPreset:
    if not preset.condition_field_options:
        preset.condition_field_options = [
            _CONDITION_FIELD_OPTIONS.get(field) or ApprovalPresetConditionField(field=field, label=field)
