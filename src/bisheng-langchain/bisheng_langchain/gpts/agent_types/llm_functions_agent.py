...
from bisheng_langchain.gpts.message_types import LiberalToolMessage, LiberalFunctionMessage


def get_openai_functions_agent_executor(
    tools: list[BaseTool],
...
def get_qwen_local_functions_agent_executor(
    ...
    if tools:
        llm_with_tools = llm.bind(functions=[format_tool_to_openai_tool(t)["function"] for t in tools])
    else:
        llm_with_tools = llm
    agent = _get_messages | llm_with_tools
    tool_executor = ToolExecutor(tools)
