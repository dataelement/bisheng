...
def upgrade() -> None:
    conn = op.get_bind()
    if not table_exists(conn, 'llm_token_log'):
        op.create_table(
            'llm_token_log',
        ...
        if not index_exists(conn, 'llm_token_log', 'idx_llm_token_log_tenant_created'):
            op.create_index(
                'idx_llm_token_log_tenant_created',
                'llm_token_log', ['tenant_id', 'created_at'],
            )
        if not index_exists(conn, 'llm_token_log', 'idx_llm_token_log_user_id'):
            op.create_index('idx_llm_token_log_user_id', 'llm_token_log', ['user_id'])
        if not index_exists(conn, 'llm_token_log', 'idx_llm_token_log_model_id'):
            op.create_index('idx_llm_token_log_model_id', 'llm_token_log', ['model_id'])
        if not index_exists(conn, 'llm_token_log', 'idx_llm_token_log_server_id'):
            op.create_index('idx_llm_token_log_server_id', 'llm_token_log', ['server_id'])

    if not table_exists(conn, 'llm_call_log'):
        op.create_table(
            'llm_call_log',
            sa.Column('id', sa.Integer, primary_key=True, autoincrement=True),
            sa.Column('tenant_id', sa.Integer, nullable=False, server_default='1',
        ...
        if not index_exists(conn, 'llm_call_log', 'idx_llm_call_log_tenant_created'):
            op.create_index(
                'idx_llm_call_log_tenant_created',
                'llm_call_log', ['tenant_id', 'created_at'],
            )
        if not index_exists(conn, 'llm_call_log', 'idx_llm_call_log_user_id'):
            op.create_index('idx_llm_call_log_user_id', 'llm_call_log', ['user_id'])
        if not index_exists(conn, 'llm_call_log', 'idx_llm_call_log_status'):
            op.create_index('idx_llm_call_log_status', 'llm_call_log', ['status'])

def downgrade() -> None:
    conn = op.get_bind()
    if table_exists(conn, 'llm_call_log'):
        if index_exists(conn, 'llm_call_log', 'idx_llm_call_log_status'):
            op.drop_index('idx_llm_call_log_status', 'llm_call_log')
        if index_exists(conn, 'llm_call_log', 'idx_llm_call_log_user_id'):
            op.drop_index('idx_llm_call_log_user_id', 'llm_call_log')
        if index_exists(conn, 'llm_call_log', 'idx_llm_call_log_tenant_created'):
            op.drop_index('idx_llm_call_log_tenant_created', 'llm_call_log')
        op.drop_table('llm_call_log')
    if table_exists(conn, 'llm_token_log'):
        if index_exists(conn, 'llm_token_log', 'idx_llm_token_log_server_id'):
            op.drop_index('idx_llm_token_log_server_id', 'llm_token_log')
        if index_exists(conn, 'llm_token_log', 'idx_llm_token_log_model_id'):
            op.drop_index('idx_llm_token_log_model_id', 'llm_token_log')
        if index_exists(conn, 'llm_token_log', 'idx_llm_token_log_user_id'):
            op.drop_index('idx_llm_token_log_user_id', 'llm_token_log')
        if index_exists(conn, 'llm_token_log', 'idx_llm_token_log_tenant_created'):
