    ...
    return (
        <div className="flex w-full min-w-0 flex-col gap-0.5">
            {shown.map((file) => {
                if (isViolationFile(file)) {
                    return (
                        <button
                            key={file.file_id}
                            type="button"
                            className="block w-full min-w-0 truncate text-left text-[#F53F3F] hover:underline"
                ...
                if (!url) {
                    return (
                        <span key={file.file_id} className="block w-full min-w-0 truncate" title={file.file_name}>
                            {file.file_name}
                        </span>
                    )
                }
                return (
                    <a
                        ...
                        className="block w-full min-w-0 truncate text-blue-600 hover:underline"
                    >
                        {file.file_name}
                    </a>
                )
