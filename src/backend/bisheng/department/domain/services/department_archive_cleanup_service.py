...
class DepartmentArchiveCleanupService:
    ...
    async def arun_for_archived_department(
        ...
        try:
            for user_id in cleanup.get("member_user_ids") or []:
                await DepartmentMembershipProjectionService.aremove_member(
                    user_id=int(user_id),
                    department_id=dept_id,
                )
            ops = DepartmentChangeHandler.on_purged(
                dept_id,
                [],
    ...
    async def _collect_and_delete_mysql_refs(cls, department_id: int) -> dict:
        async with get_async_db_session() as session:
            dept = (await session.exec(select(Department).where(Department.id == department_id))).first()
            if dept is None:
                return {
                    "parent_id": None,
                    "member_user_ids": [],
            ...
            scoped_role_ids = [
                int(row[0] if isinstance(row, tuple) else row)
                for row in (await session.exec(select(Role.id).where(Role.department_id == department_id))).all()
            ]

            if scoped_role_ids:
                await session.execute(delete(UserRole).where(UserRole.role_id.in_(scoped_role_ids)))
                await session.execute(delete(Role).where(Role.id.in_(scoped_role_ids)))
