...
| AC-36 | 管理员 | 在条件分支弹窗中选择条件字段"申请人身份" | 条件字段按当前场景的 `condition_fields` 过滤展示；条件值下拉动态展示该字段的可选枚举：系统管理员（`admin`）、租户管理员（`tenant_admin`）、部门管理员（`dept_admin`）、系统角色（当前租户已创建的角色列表，需调用角色列表接口）；条件匹配采用"包含即命中"语义 |
| AC-37 | 管理员 | 在条件分支弹窗中选择条件字段"申请菜单" | 条件值下拉展示当前租户可申请的菜单列表（来自系统菜单配置）；管理员不需要手动填写 menu_key 字符串 |
