...
1. **插画 = 空状态 / 状态反馈的图形层**，全部做成**内联 SVG React 组件**，统一放组件库 `@bisheng/ui`（`packages/ui/src/components/Illustration/`，从 `index.ts` 导出；client 侧 `~/components/illustrations` 是 re-export shim）。静态 PNG（`empty.png` / `ai-home.png` / `review.png`）已全量清零退役——**PNG 无法主题化，禁止回头再用**。
...
4. 空状态采用三段式结构：**插画 + 状态提示（+ 建议操作）**。插画只负责「发生了什么」，「接下来做什么」交给文案和按钮。组装规则见 [组件-State状态页.md](组件-State状态页.md)。
...
范本：`packages/ui/src/components/Illustration/EmptyStateIllustration.tsx`。
...
5. 放 `packages/ui/src/components/Illustration/`，在该目录 `index.ts` 追加导出（会经 `packages/ui/src/index.ts` 的 `export *` 自动出现在 `@bisheng/ui`），并在 client 的 `~/components/illustrations/index.ts` shim 里补一行 re-export；同时在画廊 `_gallery/sections/IllustrationSection.tsx` 的 `ILLUSTRATIONS` 清单登记（名称 + 场景一句话）。
...
**文案层级的间距与文字样式**（2026-07-15 定档，2026-08-14 移出）：见 [组件-State状态页.md](组件-State状态页.md) §4.1（紧凑排版）与 §4.2（结果反馈排版）。
...
- 调色板与灰稿：`packages/ui/src/styles/tokens.css`（`:root` / `.theme-green` / `.illus-grey` 三处 `--illus-*`），值定义在 `packages/ui/design-token.cjs` 的 `ILLUS`；client 的 `src/style.css` 保留一份等值副本（两处必须同步，见组件库 AGENTS.md）
- 组件目录：`src/frontend/packages/ui/src/components/Illustration/`（`index.ts` 统一导出，经 `@bisheng/ui` 对外）
- client 兼容层：`src/frontend/client/src/components/illustrations/index.ts`（re-export shim + 本地保留的 `KnowledgeSpaceIcon`）
...
| 2026-08-14 | 状态页建档，规则归位（元规范 §0.2 一条规则一个家）：§5.2 的文案层级间距表与主提示文字样式移到 [组件-State状态页.md](组件-State状态页.md) §4.1 / §4.2，本文原处改为指针；§0 关键结论第 4 条补一句指针。**插画自己的规则未增未减未改**（尺寸两档、调色板、灰稿、禁止项全部原样） | 待 committer 窗口提交 |
