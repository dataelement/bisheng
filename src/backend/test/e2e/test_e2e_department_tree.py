...
def _assert_error(resp: httpx.Response, code: int) -> dict:
    ...
    assert resp.status_code == 200
    body = resp.json()
    assert body["status_code"] == code, f"Expected {code}, got {body['status_code']}: {body.get('status_message')}"
    return body


class _FakeResp:
    """Minimal httpx.Response stand-in so callers can keep using
    ``resp.status_code`` / ``resp.json()`` unchanged."""
...
def _get_department_tree(client: httpx.Client, token: str):
    ...
    endpoint (one layer per call) so existing assertions keep working. Returns a
    response-like object exposing ``status_code`` + ``json()``."""
    headers = _auth(token)
    ...
    if body.get("status_code") != 200:
        # Propagate business errors (e.g. 21009 permission denied) verbatim.
    ...
    def _build(nodes):
        ...
        for n in nodes:
            ...
            if node.get("has_children"):
                child_body = _layer(node["id"]).json()
                node["children"] = _build(child_body.get("data") or [])
            else:
                node["children"] = []
            out.append(node)
        return out

    roots = _build(body.get("data") or [])
    return _FakeResp(200, {"status_code": 200, "status_message": "SUCCESS", "data": roots})
...
def root_dept_id(client, admin_token):
    ...
    resp = _get_department_tree(client, admin_token)
...
class TestTreeAndDetail:
    def test_ac03_get_tree(self, client, admin_token):
        ...
        tree_resp = _get_department_tree(client, admin_token)
    ...
    def test_ac04_get_department_detail(self, client, admin_token):
        ...
        # Create a dept
        tree = _assert_200(
            _get_department_tree(client, admin_token)
        )
        root_id = tree[0]["id"]
...
class TestPermissionDenied:
    def test_ac16_non_admin_denied(self, client, admin_token):
        ...
        resp = client.get(f"{API_BASE}/departments/children")
        # Without auth, FastAPI will return 401 or the endpoint logic
        # won't have user_role, resulting in permission error
        assert resp.status_code in (200, 401, 403, 422), f"Unexpected status: {resp.status_code}"
