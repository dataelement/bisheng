...
def _create_test_dept(client: httpx.Client, token: str, name: str) -> dict:
    ...
    tree = _get_department_tree(client, token)
    ...
    tree2 = _get_department_tree(client, token)
...
class TestTenantCodeAutogen:
    ...
    def test_case1_mount_auto_generates_t_dept_id(self, client, token, test_dept):
        ...
        data = body.get("data", body)
        expected = f"t{int_id}"
        assert data["tenant_code"] == expected, f"expected tenant_code={expected!r}, got {data!r}"
    ...
    def test_case2_unmount_then_remount_no_collision(self, client, token, test_dept):
        ...
        expected = f"t{int_id}"
        assert data["tenant_code"] == expected, f"remount expected tenant_code={expected!r}, got {data!r}"
    ...
    def test_case3_explicit_valid_code_preserved(self, client, token):
        ...
        try:
            ...
            assert data["tenant_code"] == explicit_code, f"expected explicit {explicit_code!r}, got {data!r}"
    ...
    def test_case4_invalid_code_rejected_422(self, client, token):
        ...
        try:
            ...
            assert resp.status_code == 422, f"expected 422 for invalid code, got {resp.status_code}: {resp.text}"
            print(f"  ✓ case4: invalid code rejected 422: {resp.json()}")
        finally:
            _delete_dept(
                client,
                token,
                dept.get("dept_id") or str(dept.get("id")),
            )
