...
async def test_knowledge_space_create_creates_exception_when_scenario_disabled(monkeypatch):
    ...
    approval_gate = SimpleNamespace(request_or_pass=AsyncMock(side_effect=ApprovalScenarioDisabledError()))
...
@pytest.mark.asyncio
async def test_file_publish_rejects_share_entry_before_permission_check():
    from bisheng.approval.domain.services.shougang_approval_service import ShougangApprovalService
    ...
    space_service = SimpleNamespace(_require_permission_id=AsyncMock())
    source_file = SimpleNamespace(
        id=100,
        knowledge_id=10,
        file_type=FileType.FILE.value,
    ...
    with pytest.raises(HTTPException, match="分享入口不能发布"):
        await ShougangApprovalService()._ensure_can_publish_file(
            source_file=source_file,
...
async def test_file_publish_target_space_validation_does_not_mutate_knowledge_model(monkeypatch):
    ...
    target_space = Knowledge(id=20, name="部门空间", type=KnowledgeTypeEnum.SPACE.value)
    monkeypatch.setattr(
        "bisheng.approval.domain.services.shougang_approval_service.KnowledgeDao.aquery_by_id",
...
def test_file_publish_matrix_allows_only_immediate_parent_pairs(source_level, target_level):
...
def test_file_publish_matrix_rejects_cross_level_same_level_and_downward_pairs(source_level, target_level):
    from bisheng.approval.domain.services import shougang_approval_handler as handler_mod
    from bisheng.approval.domain.services.shougang_approval_service import ShougangApprovalService
    from bisheng.knowledge.domain.models.knowledge_space_scope import KnowledgeSpaceLevelEnum
...
async def test_file_publish_target_spaces_follow_source_matrix(monkeypatch, source_level, expected_ids):
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(return_value=(
            SimpleNamespace(id=10, name="源空间"),
            SimpleNamespace(
                id=100,
                knowledge_id=10,
                file_type=FileType.FILE.value,
    ...
    level_by_id = {
        101: KnowledgeSpaceLevelEnum.PUBLIC,
        201: KnowledgeSpaceLevelEnum.DEPARTMENT,
        301: KnowledgeSpaceLevelEnum.TEAM,
        302: KnowledgeSpaceLevelEnum.TEAM_KS,
    }
    monkeypatch.setattr(
        ...
        AsyncMock(side_effect=lambda ids: {
            space_id: SimpleNamespace(level=level_by_id[space_id])
            for space_id in ids
        }),
    )
    monkeypatch.setattr(
        "bisheng.approval.domain.services.shougang_approval_service.KnowledgeDao.async_get_spaces_by_ids",
        AsyncMock(side_effect=lambda ids, **_: [
            space(space_id, None) for space_id in ids
        ]),
    )
...
async def test_file_publish_target_spaces_reject_public_source(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "_load_publish_source",
        AsyncMock(return_value=(
            SimpleNamespace(id=10, type=KnowledgeTypeEnum.SPACE.value, name="公共空间"),
    ...
    with pytest.raises(HTTPException, match="当前知识空间类型不支持发布文件"):
        await service.list_file_publish_target_spaces(
            source_space_id=10,
            source_file_id=100,
            space_service=space_service,
        )
...
async def test_file_publish_target_folders_return_only_minimal_metadata_without_target_view(monkeypatch):
    ...
    result = await service.list_file_publish_target_folders(
        source_space_id=10,
        source_file_id=100,
        target_space_id=20,
        parent_id=None,
            ...
            if await can_view_file(knowledge_file_id):
                out.append(
                    SimilarCandidateEntry(
                        ...
                        similarity=1.0,
                        primary_knowledge_file_id=knowledge_file_id,
                    )
                )
        return out
...
async def test_file_publish_document_search_filters_by_file_view_permission(monkeypatch):
    ...
    async def search_sources(target_space_id, keyword, source_file_id, *, can_view_file):
        assert target_space_id == 20
        assert keyword == "制度"
        assert source_file_id == 100
        out = []
        for document_id, file_id in [(1, 200), (2, 201)]:
            if await can_view_file(file_id):
                out.append(
...
async def test_file_publish_handler_reuses_existing_copied_file(monkeypatch):
    ...
    monkeypatch.setattr(
        "bisheng.approval.domain.services.shougang_approval_handler._copy_file_tags",
        AsyncMock(),
    )
...
async def test_file_publish_handler_stops_before_version_link_when_copy_fails(monkeypatch):
    ...
    monkeypatch.setattr(
        handler,
        "_space_level",
        AsyncMock(side_effect=[KnowledgeSpaceLevelEnum.TEAM, KnowledgeSpaceLevelEnum.DEPARTMENT]),
    )
    monkeypatch.setattr(
        handler,
        "_copy_file",
...
async def test_file_publish_handler_builds_document_for_target_file_before_link(monkeypatch):
    ...
    monkeypatch.setattr(handler_mod, "_link_file_as_version", link_file)
    monkeypatch.setattr(handler_mod, "_copy_file_tags", AsyncMock())
    monkeypatch.setattr(handler, "_copy_file", Mock())
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(
            side_effect=[
                ...
                SimpleNamespace(level=KnowledgeSpaceLevelEnum.DEPARTMENT),
            ]
        ),
    )
    monkeypatch.setattr(
...
async def test_new_file_publish_approval_rejects_inflight_cross_level_request(monkeypatch):
    ...
    with pytest.raises(
        ValueError,
        match="source and target space levels are not allowed for publish",
    ):
        await handler.on_approved(
            102,
            {
    ...
    distribution.assert_not_awaited()
...
async def test_copy_file_tags_copies_approved_and_pending_review_tags(monkeypatch):
    ...
    add_approved = AsyncMock()
    add_review = AsyncMock()
    monkeypatch.setattr(
        "bisheng.database.models.tag.TagDao.get_tags_by_resource_batch",
        lambda *args, **kwargs: {"100": [Tag(id=11, name="技术文档", resource_type="manual_tag")]},
    )
    monkeypatch.setattr(
        "bisheng.database.models.review_tags.ReviewTagDao.get_tags_by_resource_batch",
        lambda *args, **kwargs: {
            "100": [
                SimpleNamespace(id=21, name="新标签", resource_type="ai_auto_tag", review_status=0),
                SimpleNamespace(id=22, name="已拒绝", resource_type="ai_auto_tag", review_status=2),
            ]
        },
    )
    monkeypatch.setattr("bisheng.database.models.tag.TagDao.add_tags", add_approved)
    monkeypatch.setattr("bisheng.database.models.review_tags.ReviewTagDao.add_tags", add_review)

    await _copy_file_tags(source_file_id=100, target_file_id=188, user_id=11, tenant_id=1)

    add_approved.assert_awaited_once_with([11], "188", ResourceTypeEnum.SPACE_FILE, 11)
    add_review.assert_awaited_once_with(
        [21],
        "188",
...
@pytest.mark.asyncio
async def test_file_publish_handler_copies_tags_on_approved(monkeypatch):
    from bisheng.approval.domain.services import shougang_approval_handler as handler_mod
    from bisheng.approval.domain.services.shougang_approval_handler import KnowledgeSpaceFilePublishApprovalHandler
    from bisheng.knowledge.domain.models.knowledge import KnowledgeTypeEnum
    from bisheng.knowledge.domain.models.knowledge_file import KnowledgeFileStatus
    ...
    copy_tags.assert_awaited_once_with(
        source_file_id=100,
        target_file_id=188,
        user_id=11,
        tenant_id=1,
    )
    assert result == {"file_id": 188, "target_space_id": 20, "version": None}


@pytest.mark.asyncio
async def test_file_publish_handler_copies_tags_on_idempotent_retry(monkeypatch):
    from bisheng.approval.domain.services import shougang_approval_handler as handler_mod
    from bisheng.approval.domain.services.shougang_approval_handler import KnowledgeSpaceFilePublishApprovalHandler
...
def test_shougang_scenarios_registered_in_default_presets():
    ...
    assert [
        option.source_type for option in presets["knowledge_space_file_publish_request"].approver_source_options
    ] == [
        "direct_user",
        "department_admin",
        "role_user",
    ...
    assert (
        publish_source_options["target_knowledge_space_manager_department_admin"] == "目标知识空间 Manager 的部门管理员"
    )
