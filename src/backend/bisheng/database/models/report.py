...
class ReportBase(SQLModelSerializable):
    ...
    # 会存储模板生成的最终报告的记录`
    flow_id: str = Field(index=False, description='前端展示名称')
    file_name: str = Field(index=False, description='生成报告存储路径')
    template_name: str = Field(index=False, description='报告模板数据存储路径')
