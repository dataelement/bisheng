...
| 2026-09-15 · **二次同步发版线** | 合入 `origin/feat/3.0.0-beta2` 03106e053（merge `8496897b0`）：OpenFGA 模型升 `f048-v6`（本分支与 beta2 各自的 v5 合并，含 contextual 部门成员 + `app`），托管应用入口补单点登录校验，alembic 收单头 `merge_app_factory_beta2_heads`；114 重建按迁移方案 §4（已按实查订正） |
