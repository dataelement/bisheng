...
class TestTupleLifecycle:
    ...
    @pytest.mark.asyncio
    async def test_add_file_quota_check_uses_downloaded_path_for_share_link(self, service):
        space = _make_space(auth_type=AuthTypeEnum.PUBLIC)
        added_file = _make_file(file_id=86, knowledge_id=1, file_name='doc.txt')
        added_file.status = 5
        added_file.file_size = 4
        original_path = 'http://minio.example/tmp/doc.txt?X-Amz-Signature=abc'
        local_path = '/tmp/bisheng/doc.txt'
        ...
        with patch.object(
            ...
            'bisheng.knowledge.domain.services.knowledge_space_service.file_download',
            return_value=(local_path, 'doc.txt'),
        ) as mock_download, patch(
            'bisheng.knowledge.domain.services.knowledge_space_service.os.path.getsize',
            return_value=4,
            ...
            result = await service.add_file(1, [original_path])

        assert result[0].id == 86
        mock_download.assert_called_once_with(original_path)
        mock_getsize.assert_called_once_with(local_path)
        call_kwargs = mock_process_one_file.call_args.kwargs
        assert call_kwargs['file_info'].kwargs['file_path'] == local_path
        assert call_kwargs['cleanup_file_path'] == original_path

    @pytest.mark.asyncio
    async def test_add_file_size_check_failure_is_not_reported_as_quota_limit(self, service):
        space = _make_space(auth_type=AuthTypeEnum.PUBLIC)
        ...
        with patch.object(
            ...
            'bisheng.knowledge.domain.services.knowledge_space_service.file_download',
            side_effect=ValueError('bad temp url'),
        ), patch(
            'bisheng.knowledge.domain.services.knowledge_space_service.KnowledgeService.process_one_file',
        ) as mock_process_one_file:
            with pytest.raises(SpaceFileNotFoundError):
                await service.add_file(1, ['http://minio.example/tmp/missing.docx'])

        mock_process_one_file.assert_not_called()

    @pytest.mark.asyncio
    async def test_add_file_raises_size_limit_when_downloaded_file_exceeds_remaining_quota(self, service):
        space = _make_space(auth_type=AuthTypeEnum.PUBLIC)
        ...
        with patch.object(
            ...
            'bisheng.knowledge.domain.services.knowledge_space_service.file_download',
            return_value=('/tmp/bisheng/doc.txt', 'doc.txt'),
        ), patch(
            'bisheng.knowledge.domain.services.knowledge_space_service.os.path.getsize',
            return_value=4,
        ), patch(
            ...
            with pytest.raises(SpaceFileSizeLimitError):
                await service.add_file(1, ['http://minio.example/tmp/doc.txt'])
