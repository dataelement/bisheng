...
async def get_tool_list(
    *,
    ...
    action: Literal["visible", "use"] = "use",
    login_user: UserPayload = Depends(UserPayload.get_login_user),
):
    """Query all visibletool Vertical"""
    res = await ToolServices(login_user=login_user).get_tool_list(
        is_preset,
        action,
    )
...
async def add_tool_type(
    request: Request, login_user: UserPayload = Depends(UserPayload.get_login_user), req: dict = Body(default={})
):
...
async def get_tool_schema(
    request: Request,
    login_user: UserPayload = Depends(UserPayload.get_login_user),
    download_url: str | None = Body(default=None, description="MengunduhurlIf it is not empty, download it firsturl"),
    file_content: str | None = Body(default=None, description="files uploaded"),
):
...
async def get_mcp_tool_schema(
    request: Request,
    login_user: UserPayload = Depends(UserPayload.get_login_user),
    file_content: str | None = Body(default=None, embed=True, description="mcpService Configuration Content"),
):
...
async def tool_run(*, login_user: UserPayload = Depends(UserPayload.get_login_user), req: TestToolReq):
    ...
    tool_params = OpenApiSchema.parse_openapi_tool_params(
        "test", "test", json.dumps(extra), req.server_host, req.auth_method, req.auth_type, req.api_key
    )
...
async def mcp_tool_run(login_user: UserPayload = Depends(UserPayload.get_login_user), req: TestToolReq = None):
    ...
    mcp_tool = McpTool.get_mcp_tool(
        name=tool_name,
        description=extra.get("description"),
        mcp_client=client,
        mcp_tool_name=tool_name,
        arg_schema=extra.get("inputSchema", {}),
    )
