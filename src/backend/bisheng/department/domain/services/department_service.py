...
class DepartmentService:
    ...
    async def aget_tree(cls, login_user) -> list[DepartmentTreeNode]:
        ...
        default_root_dept_id = await aget_default_root_dept_id()
        nodes = {}
        for d in depts:
            nodes[d.id] = DepartmentTreeNode(
                ...
                is_default_root=(default_root_dept_id is not None and int(d.id) == default_root_dept_id),
                children=[],
            )
    ...
    async def aget_department(cls, dept_id: str, login_user) -> dict:
        ...
        data["is_default_root"] = await ais_default_root_department(dept)
        return data
    ...
    async def aget_global_members_search(
        ...
        async with get_async_db_session() as session:
            ...
            page_stmt = base_group.order_by(User.user_name).offset((page - 1) * limit).limit(limit)
