...
* **Never prints ``结论: 通过``.** SKILL.md tells the model to iterate until that
  exact string appears, and it is owned by ``inspect_workbook.py`` — the delivery
  gate. Recalculation is only one step before it, so a passing recalc says
  ``重算完成`` instead. Emitting the reserved string here would let the model stop
  one step early, shipping a workbook that was never health-checked.
"""
...
def calc_binary_found(soffice: str) -> bool:
    """Whether an ``scalc`` launcher is visible next to this soffice.

    ADVISORY ONLY — never a gate. Calc ships as a separate package on some
    hosts, so a missing ``scalc`` is worth mentioning *after* a failure, but a
    present Calc is not guaranteed to expose that file: macOS bundles ship a
    single ``Contents/MacOS/soffice`` with no per-app launchers at all, and
    refusing to convert there was a 100% false negative. Try first, diagnose
    after.
    """
...
def recalc(path: str, timeout: int, force: bool) -> dict:
    ...
    calc_hint = (
        ""
        if calc_binary_found(soffice)
        else "另：这套 LibreOffice 旁边找不到 scalc，若反复失败就是没装 Calc 组件（手工部署常只装 libreoffice-writer），让运维补装 libreoffice-calc。"
    )
    ...
    with tempfile.TemporaryDirectory(prefix=f"lo_recalc_{uuid.uuid4().hex[:8]}_") as tmp:
        ...
        try:
            ...
            return {"error": f"LibreOffice 重算超时（{remaining}s）。加大 --timeout 重试。{calc_hint}"}

        # The file is the only evidence that counts. soffice exits 0 after doing
        # nothing at all, and it exits non-zero on shutdown noise (macOS prints
        # "Task policy set failed") after a perfectly good rewrite — so read the
        # stat first and treat the return code as commentary.
        after = os.stat(abs_path)
        rewritten = (after.st_mtime_ns, after.st_size) != before_stamp
    ...
    if not rewritten:
        ...
        elif "could not be loaded" in detail:
            hint = "（报了 could not be loaded = 这套 LibreOffice 打不开 .xlsx，基本就是缺 Calc 组件）"
        return {
            "error": (
                f"LibreOffice 退出码 {proc.returncode} 但文件没被重写，等于什么都没算{hint}。"
                f"soffice 输出：{detail[:400] or '（空）'}。"
                f"重试一次；仍失败就把文件另存一个新名字再试。{calc_hint}"
            )
        }

    result = scan(path)
    if proc.returncode != 0:
        result["soffice_returncode"] = proc.returncode
    return result
...
def report(result: dict) -> None:
    ...
    if "error" in result:
        ...
        print()
        print("重算未通过 —— 公式没有缓存值，交付出去用户和 pandas 读到的都是空。")
...
def main() -> None:
    ...
    try:
        ...
        if "not a zip file" in str(exc).lower():
            ...
            print("改法：.xls/.csv 用 pandas 读进来，再用 openpyxl 写一份新的 .xlsx，然后重算这一份。")
