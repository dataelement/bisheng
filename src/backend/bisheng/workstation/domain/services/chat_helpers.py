    ...
    msg = json.dumps(
        {
            ...
            "responseMessage": (await WorkstationMessage.from_chat_message(response_message)).model_dump(),
        },
...
def _is_new_format(msg: ChatMessage) -> bool:
    ...
    New format signatures:
      ...
      - category == 'task' (F035 Track J): plain answer string (possibly empty
        for an in-flight/placeholder row) + extra.linsight_session_version_id
    """
    if not msg:
        return False
    # F035 Track J: task turns are "new format" regardless of message body —
    # even an empty placeholder row (in-flight/HITL) must surface with its
    # category + SV pointer so a refresh can re-hydrate the inline panel.
...
def _format_new_message(msg: ChatMessage) -> dict:
    ...
    # F035 Track J: task turns carry the execution-detail pointer in `extra`
    # (`linsight_session_version_id`). Surface it so the inline task bubble can
    # lazy-load the rich panel by SV. `message` stays the plain answer string.
    try:
    ...
    return {
        **_message_base_fields(msg),
        ...
        "extra": extra,
        "linsight_session_version_id": extra.get("linsight_session_version_id"),
