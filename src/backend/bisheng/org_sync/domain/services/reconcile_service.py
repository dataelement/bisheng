...
class OrgReconcileService:
    ...
    async def reconcile_config(cls, config_id: int) -> ReconcileResult:
        ...
        if config.provider == cls.SKIP_PROVIDER:
            return ReconcileResult(skipped=True, skip_reason="sso_realtime_seed")
        if config.status != "active":
            return ReconcileResult(skipped=True, skip_reason=f"status={config.status}")
    ...
    async def _run(cls, config) -> ReconcileResult:
        ...
        for ev in event_rows:
            try:
                ...
                logger.exception(f"event_row persist failed for config {config.id}: {e}")
    ...
    async def _apply_upsert(
        ...
        *,
        ...
        try:
            ...
            logger.exception(f"upsert failed for {op.external_id} (config {config.id}): {e}")
    ...
    async def _apply_archive(
        ...
        *,
        ...
        try:
            ...
            # can investigate.
            is_same_ts_conflict = (
                int(existing.last_sync_ts or 0) == op.incoming_ts and int(existing.is_deleted or 0) == 0
            )
            ...
            if is_same_ts_conflict:
                ...
                try:
                    ...
                    logger.exception(f"audit_log.ainsert_v2 failed for {op.external_id}: {e}")
                ...
                logger.warning(
                    f"{SsoSameTsRemoveAppliedWarnError.Code} {SsoSameTsRemoveAppliedWarnError.Msg}: {op.external_id}"
                )
                result.conflicts += 1

            await DepartmentTopologyProjectionService.aarchive_synced_department(
                department_id=int(existing.id),
                last_sync_ts=op.incoming_ts,
            )
            try:
                await DepartmentArchiveCleanupService.arun_for_archived_department(
                    int(existing.id),
                    reason="celery_reconcile_archive",
                )
            except Exception as e:  # noqa: BLE001
                logger.warning(
                    "archive_cleanup failed for dept %s: %s",
                    existing.id,
                    e,
                )
            try:
                await DepartmentDeletionHandler.on_deleted(
                    existing.id,
                    DeletionSource.CELERY_RECONCILE,
                )
            except Exception as e:
                logger.exception(f"DepartmentDeletionHandler.on_deleted failed for dept {existing.id}: {e}")
                result.errors.append(f"on_deleted {existing.id}: {e!s}")

            buffer.dept_archived += 1
            result.applied_archive += 1
        except Exception as e:
            logger.exception(f"archive failed for {op.external_id} (config {config.id}): {e}")
    ...
    async def _apply_move(
        ...
        *,
        ...
        try:
            ...
            logger.exception(f"aget_user_ids_by_department failed for {mv.dept_id}: {e}")
        ...
        for uid in user_ids:
            try:
                ...
                logger.exception(f"UserTenantSyncService.sync_user({uid}) failed: {e}")
    ...
    async def _acquire_lock(cls, config_id: int):
        ...
        try:
            ...
            try:
                ...
                logger.exception(f"failed to release reconcile lock {key}; TTL fallback")
    ...
    def _build_provider(cls, config):
        ...
        try:
            ...
            logger.exception(f"decrypt_auth_config failed for config {config.id}: {e}")
            auth_dict = {}
