...
- [x] **T034**: `[MVP-核心]` 上游回写四项 + 顺延项落点核对
  ...
  **实际偏差记录**（2026-08-19 完成并勾选）: 四项**全部已交付**，且**代码先于文档条目落地**——
  - ① `WhoamiResponse.resource_owner`：commit **`874bd688b`**（2026-08-18），`open_api/domain/schemas/credential.py` 新增 `WhoamiResourceOwner` + 可选字段；④ 本 spec AC-05 措辞同批改定（现文：「使 `login` 在该环境**不可用并给出可读原因**」）。两条已登记进 `features/v3.0.0/049-openapi-auth-baseline/tasks.md` 文末新增的「跨 Feature 回写受理」表。
  - ②③ `logs` 的 `app_state` / `pending_reason` 与轮询载荷的 `entry_url`：commit **`95c0ed98e`**（2026-08-18）。已登记进 `features/v3.0.0/055-app-publish-pipeline/tasks.md` 文末同名表。**`entry_url` 由 F054 的唯一实现按 slug 推导**，F055 不拼、CLI 更不拼。
  - 另一项「由 F055 侧发起、本 Feature 只需接受」的 `deploy-limits` 口径：F055 T050 ③ 已于 2026-08-19 回写进本 Feature 的 **spec AC-32 与 §3 边界**（上限经 `GET /api/v2/apps/deploy-limits` 取，取不到按内置默认值提示并继续上传、由服务端 16201 兜底）——T017 / T022 的实现本就是这个口径，两处现已对齐。
  - **未做的半项**：「顺延项落点核对」（design §1 非目标表 ↔ 本文 Wave 3–5 任务标题一一对应）**没有逐条核对**，因为 Wave 3 已在 2026-08-18 实际开工并落了 T035–T040/T049，非目标表与任务标题的对应关系已经变化，一次纸面核对给不出可信结论。建议并入下一次 `/sdd-review tasks`。
...
- [x] **T038a**（F051 落地后追加）: 「平台能力接线」包的**模型一节**改写——F051 已交付，模型章不再是「暂未提供」。按 F051 [design §4.2 ①③](../051-model-protocol-gateway/design.md) 与 **D5 限定名规则**写：base URL 从 `GET /api/v2/auth/whoami` 的 `model_base_url` 取（唯一出口，不要在包里拼 `/api/v2/model/v1`）；`dev` 与线上注入同名的 `OPENAI_BASE_URL` / `OPENAI_API_KEY` / `BISHENG_MODEL_BASE_URL`，样例用官方 `openai` 客户端、代码零差异；模型名写模型管理页原名，**跨服务商同名时必须写限定名 `服务商名/模型名`**（裸名会被 26214 拒并列出可用限定名，`GET /models` 返回的 `id` 恒可直接调用）；承诺面只有对话补全与模型列表，其余 OpenAI 路径返 26201、Anthropic 路径返 26202。另附 **`X-BiSheng-Access-Token` 转发说明**：app-proxy 把它注入到**应用**而不是浏览器，应用调模型面时必须由应用代码显式转发，否则调用记录的 subject 一律是「应用自身」——不报错，只是审计里没有用户维度（F051 spec 决议-5 允许，但 owner 通常并不想要）。
  **文件**: `src/backend/bisheng/dev_toolkit/skills/platform-wiring/SKILL.md`（模型章）+ `example/`
  **覆盖 AC**: AC-17, AC-18（F051 AC-31 / AC-33 的文档侧）
  **2026-09-16 完成证据**（分支 `wt/f053-skillpack-model`）: `SKILL.md` 第 5 章由「暂未提供」整章重写为「平台模型（OpenAI 兼容）」——三个注入名表（`OPENAI_BASE_URL` / `OPENAI_API_KEY` / `BISHENG_MODEL_BASE_URL`，线上与 `bisheng dev` 同名）、官方 `openai` 客户端零配置样例、「地址一个字都不要自己拼」（点名 `BISHENG_PLATFORM_API_BASE` 不是模型端点，唯一出口是 `whoami.model_base_url` 产出的注入值）、三名可能一个都没有时的启动期检查、D5 名称解析（原名基准 / 26214 歧义 / `服务商名/模型名` 限定名 / `GET …/models` 的 `id` 恒可调 + `bisheng_qualified_name`）、清单 `capabilities.models` 声明与「未声明即 26215、零声明连变量都不注入」、本地 vs 线上三行对照表、`X-BiSheng-Access-Token` 转发（线上转发、本地 `bisheng dev` **不能**转发否则 26204）、承诺面两条路径 + 26201 / 26202、含 `bisheng_code` 的错误速查表（13 行）与两条流式专属坑。同批：第 1 章 `X-BiSheng-Access-Token` 行补第二个去处、第 7 章自检清单的模型条补齐、`skills/README.md` 两处「模型一章暂未提供」订正。回归断言见 F057 T035a。`cd src/backend && pytest test/dev_toolkit -q` → **119 passed**；ruff check / format --check 全绿；`scripts/arch-guard.sh` 零输出。
  **偏差**: ① **未改 `example/` 与 `example-sdk/`**——两个样例的清单被既有断言钉死为「不含模型声明」（`test_skill_packs.py::test_example_manifest_is_valid_against_the_platform_schema` 要求 `capabilities.is_empty()`；`test_platform_wiring_sdk.py::test_example_sdk_manifest_is_valid_and_declares_a_knowledge_base` 要求 `not manifest.capabilities.models`），加声明会同时红两条**姊妹切片拥有的**断言。改为在章内以内联代码块教清单写法与官方客户端用法，不新增第三个样例。② 任务原文写「base URL 从 `whoami.model_base_url` 取」，落地为**应用侧读注入的 `OPENAI_BASE_URL`**（`whoami.model_base_url` 是平台产出该值的唯一出口，由 `bisheng dev` / runtime-manager 注入给应用）——应用进程里没有 `whoami` 可调，章内把这层关系写明。③ 新增一条包级断言 `test_no_pack_file_hardcodes_the_model_face_path`（整包不得出现 `/api/v2/model/v1` 字面量），F057 T035a 的口径②本来只覆盖模型章。
  **2026-09-16 同批复审（逐条对着实现 grep）**: 三个注入名、`whoami.model_base_url` 出口、`bisheng_code` / `candidates` 扩展键、26201–26205 / 26211–26217 / 26231 / 26233 的 HTTP 状态、`n != 1` 拒、流式先预取首块、`BISHENG_APP_VERSION` 线上是版本号本地是 `dev`、`OPENAI_API_KEY` 线上 = `BISHENG_APP_TOKEN` 同值 / 零声明不注入 —— 全部与实现一致。改掉四处与实现不符或会误导的表述：① **第 6 章「login 用的密钥不会进入应用进程的环境变量」是错的** —— `devdb.build_dev_env` 只 pop 掉 `BISHENG_API_KEY`，同一把密钥以 `OPENAI_API_KEY` 进了子进程（`tests/test_command_dev.py` 正是这样断言的：`[name for name, value in env.items() if value == FAKE_KEY] == ["OPENAI_API_KEY"]`），与第 5 章自相矛盾，改为「只以 `OPENAI_API_KEY` 一个名字进」；② 第 6 章对照表补模型面三名一行（原表漏了 `dev` 的这项同名注入，T038a 原文点名要教），并把与表格行数对不上的「它做三件事」改成不带数字的说法；③ 「服务商被删 → 26213」不准 —— `model_catalog._explain_miss` 明说删服务商会连模型行一起删、名字直接查无此模型，26213 只在 60s 目录缓存窗口内可见，改为以 26211 为主、26213 为窗口内特例；④ 「`X-End-User` / `X-On-Behalf-Of` 带了就是 26205」不准 —— 只有 `X-End-User` 走 `resolve_range_and_subject` 拿 26205，`X-On-Behalf-Of` 在 `dependencies.admit_request` 就被基座拒（26004；密钥带委托位则 26051，`model:invoke` 属 `LOCAL_DEV_TOOLKIT_SCOPE_CODES`），正文与错误表同步订正。复审后 `test/dev_toolkit` **119 passed**（主检出同选择基线 **118 passed**，差值 = 本批新增的包级断言），ruff / arch-guard 仍全绿。
...
10. **`26002` 的用例按「三条码互不相同」写，不写「四种成因」** —— 用例名 `test_malformed_header_and_rejected_key_are_distinguishable` 断言的是 `26001` / `26002` / `26027` 三条文案互不相同；`26002` 一码覆盖「不存在 / 已撤销 / 已过期」，服务端不给区分信号，CLI 造不出来。
