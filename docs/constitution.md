...
> - Last revised: 2026-09-10 (C5: 260 open_api band assigned to F053, sub-bands recorded; 26050+ issue-time scope-policy sub-band registered).
...
- **260 is assigned** to Open API authentication and identity (`/api/v2`; F053, `common/errcode/open_api.py`). Do not reuse it. Sub-bands as of 2026-09-10: 26001–26019 open face (`/api/v2` credential / scope / delegation / identity headers) · 26020–26031 service-account management face (`/api/v1/service-accounts/**`) · 26040–26043 personal tokens (`/api/v1/personal-tokens/**`) · 26050 onward issue-time scope policy shared by service-account keys and personal tokens (26050 `OpenApiDelegateExclusiveScopeError`: `delegate` combined with a local development toolkit scope, refused at issue / edit time, HTTP 400); 26032–26039 and 26044–26049 stay reserved. Holes inside those ranges are not free by default (26013 / 26014 were retired and are never reused) — read the file before claiming a number. Every 260xx carries a real `http_status`, which `open_api/api/exception_handlers.py` returns on `/api/v2` paths (200 elsewhere); copy for each code must land in `packages/locales/src/api_errors/*.json` (all three languages) in the same change. Codes in this file are declared `Code = NNNNN` without the `: int` annotation — the derive command above matches both styles for that reason.
