...
@pytest.mark.asyncio
async def test_user_repository_returns_primary_department_display_projection() -> None:
    repository = UserRepositoryImpl(_Session((12, "设备管理部", "  设备  ")))

    projection = await repository.get_primary_department_name_projection(7)

    assert projection is not None
    assert projection.department_id == 12
    assert projection.name == "设备管理部"
    assert projection.short_name == "设备"
    assert projection.display_name == "设备"
...
async def test_user_service_resolves_primary_department_name(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        DepartmentDao,
        "aget_by_id",
        AsyncMock(return_value=SimpleNamespace(id=12, name="设备管理部", short_name=" 设备 ")),
    )

    assert await UserService.get_primary_department_name(7) == "设备管理部"
    projection = await UserService.get_primary_department_name_projection(7)
    assert projection is not None
    assert projection.display_name == "设备"
...
def test_user_info_endpoint_exposes_primary_department_name() -> None:
    ...
    assert "UserService.get_primary_department_name_projection(user_id)" in source
    assert "department_name=department_projection.name" in source
    assert "department_short_name=" in source
    assert "department_projection.short_name" in source
    assert "department_display_name=" in source
    assert "department_projection.display_name" in source
