...
def stream_env(monkeypatch: pytest.MonkeyPatch):
    ...
    # Driven from the fixture's state so a test can switch on the with-tools branch.
    monkeypatch.setattr(
        chat_service,
        "_prepare_tools",
        AsyncMock(side_effect=lambda **_kwargs: (state["tools"], [])),
    )
    # (file_context, image_bases64, merged_files) — three values, per _process_agent_files.
