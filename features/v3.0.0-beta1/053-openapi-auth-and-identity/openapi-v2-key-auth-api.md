...
| 请求头 | 必填 | 说明 |
|---|---:|---|
| `Authorization: Bearer <API_KEY>` | 是 | 所有接口都必须携带 |
| `X-On-Behalf-Of` | 否 | 代表平台用户执行；不能与 `X-End-User` 同时传 |
| `X-End-User` | 否 | S 模式下区分外部使用者；不能与 `X-On-Behalf-Of` 同时传 |

## 通用错误响应

所有 HTTP 接口均声明 400、401、403、404、422、500、503。错误 JSON：
...
```bash
curl -X GET '$BASE/api/v2/auth/whoami' \
...
| `data` | `WhoamiData` | 当前密钥信息 |
| `data.subject_kind` | `string` | 密钥主体类型 |
| `data.subject` | `WhoamiSubject` | 密钥主体 |
| `data.subject.id` | `string` | 密钥主体 ID |
| `data.subject.name` | `string` | 密钥主体名称 |
| `data.resource_owner` | `WhoamiResourceOwner / null` | 服务账号的资源归属人；个人访问令牌为 null |
| `data.resource_owner.user_id` | `integer` | 服务账号资源归属人的用户 ID |
| `data.resource_owner.user_name` | `string` | 服务账号资源归属人的用户名 |
| `data.tenant_id` | `integer` | 密钥所属租户 ID |
| `data.scopes` | `array<string>` | 密钥拥有的权限位 |
| `data.key_mask` | `string` | 脱敏后的密钥 |
| `data.expires_at` | `string(date-time) / null` | 密钥过期时间；长期有效时为 null |
...
```bash
curl -X GET '$BASE/api/v2/flows/123' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
```bash
curl -X POST '$BASE/api/v2/workflow/invoke' \
  -H 'Authorization: Bearer <API_KEY>' \
...
```bash
curl -X POST '$BASE/api/v2/workflow/stop' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X POST '$BASE/api/v2/assistant/chat/completions' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X GET '$BASE/api/v2/assistant/info/123' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
```bash
curl -X GET '$BASE/api/v2/assistant/list' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
```bash
curl -X POST '$BASE/api/v2/llm/workbench/asr' \
  -H 'Authorization: Bearer <API_KEY>' \
...
```bash
curl -X POST '$BASE/api/v2/llm/workbench/tts' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
  --data '{"text":"需要合成的文字"}'
```
...
```bash
curl -X GET '$BASE/api/v2/citation/123' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
```bash
curl -X GET '$BASE/api/v2/filelib/' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
```bash
curl -X GET '$BASE/api/v2/filelib/detail_qa?id=123' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
只允许 S 模式。参数只接收固定统计目录下的文件名，不接收服务器绝对路径。
...
| 位置 | 参数 | 类型 | 必填 | 默认值 | 说明 |
|---|---|---|---:|---|---|
| query | `file_name` | `string` | 是 | `—` | 固定统计目录下的 .log 文件名；不能包含目录或 .. |

#### 请求示例

```bash
curl -X GET '$BASE/api/v2/filelib/download_statistic?file_name=statistic.log' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
```bash
curl -X GET '$BASE/api/v2/filelib/file/list?knowledge_id=123' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
```bash
curl -X POST '$BASE/api/v2/filelib/query_qa' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X POST '$BASE/api/v2/filelib/retrieve' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X POST '$BASE/api/v2/filelib/' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X PUT '$BASE/api/v2/filelib/' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X POST '$BASE/api/v2/filelib/add_qa' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X POST '$BASE/api/v2/filelib/add_relative_qa' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X POST '$BASE/api/v2/filelib/chunks' \
  -H 'Authorization: Bearer <API_KEY>' \
  -F 'knowledge_id=123' \
  -F 'metadata={}' \
  -F 'split_mode=auto' \
...
```bash
curl -X POST '$BASE/api/v2/filelib/chunks_string' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X DELETE '$BASE/api/v2/filelib/clear/123' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
```bash
curl -X POST '$BASE/api/v2/filelib/delete_file' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
  --data '[789,790]'
```
...
```bash
curl -X DELETE '$BASE/api/v2/filelib/file/123' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
```bash
curl -X POST '$BASE/api/v2/filelib/file/123' \
  -H 'Authorization: Bearer <API_KEY>' \
  -F 'file=@/path/to/合同.pdf' \
  -F 'split_mode=auto' \
  -F 'hierarchy_level=3' \
...
```bash
curl -X DELETE '$BASE/api/v2/filelib/qa/123' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
```bash
curl -X POST '$BASE/api/v2/filelib/update_qa' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X DELETE '$BASE/api/v2/filelib/123' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
```bash
curl -X POST '$BASE/api/v2/knowledge/add_metadata_fields' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X DELETE '$BASE/api/v2/knowledge/delete_metadata_fields' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X POST '$BASE/api/v2/knowledge/file/add_user_metadata' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X DELETE '$BASE/api/v2/knowledge/file/delete_user_metadata' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X POST '$BASE/api/v2/knowledge/file/list_user_metadata' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X PUT '$BASE/api/v2/knowledge/file/modify_user_metadata' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X GET '$BASE/api/v2/knowledge/get_metadata_fields/123' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
```bash
curl -X PUT '$BASE/api/v2/knowledge/modify_metadata_fields' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X GET '$BASE/api/v2/chat/info?chat_id=chat-001' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
```bash
curl -X GET '$BASE/api/v2/chat/list' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
```bash
curl -X POST '$BASE/api/v2/knowledge/upload' \
  -H 'Authorization: Bearer <API_KEY>' \
...
```bash
curl -X POST '$BASE/api/v2/workstation/chat/completions' \
  -H 'Authorization: Bearer <API_KEY>' \
  -H 'Content-Type: application/json' \
...
```bash
curl -X GET '$BASE/api/v2/workstation/config' \
  -H 'Authorization: Bearer <API_KEY>'
```
...
OpenAPI 3.1 不定义 WebSocket operation，因此导入工具时不会自动生成下面两条连接；这里单独给出完整连接方式。
