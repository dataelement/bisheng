...
| LinsightSkill.frontend_hidden（字段级协同改动,对象仍归 F035） | F047-skill-frontend-hidden | 预置 Skill 的【前端隐藏】开关:隐藏=业务前台不可见不可选,任务模式由服务端强制并入已选并下发;开启隐藏同保存自动启用;停用优先于隐藏。按租户生效。经 F035 owner 口径认可(INV-7 例外的字段级扩展);不新增表/对象,不改 F035 的技能生命周期与门控归属 |
| —（无新增；新增一个只读聚合查询端点） | F046-file-menu-permission-prefetch | 文件菜单动作权限改为空间级判定+细粒度例外清单的一次性预取；只读复用 PermissionService 检查与 F029/F033 的 binding 不变量,不新增权限写路径/关系/角色/表；推翻 F040「菜单打开时逐文件懒查」策略（经口径知会） |
| DepartmentKnowledgeSpace（空间管理员关系 + 待配置状态） | F045-department-space-single-admin | 部门空间唯一空间管理员的配置/原子更换/失效标记与存量归一写行为；下线「部门管理员自动同步」对该表关联成员的写入；创建操作人只写审计。不拥有 `Knowledge` 本身；`space_channel_member` 空间成员行的常规增删仍走既有空间成员链路 |
