...
| Toast 轻提示 | 🟨 v1 已定稿 | 全局 Toast 1 套（`useToastContext` 135 文件 / 404 个调用点，单例只显 1 条）+ 私有撤销 toast 1 处（2026-08-13 扫描） | 四类型（不做 loading）+ 顶部最多 3 条堆叠 + 至多一个操作按钮 + 时长按字数 3～10 秒动态；剩：按规范重做基准壳（壳解剖 13 项差距见文档 §10.2）→ 收敛 `severity`/`status` 双入参 → z-index 待 Modal 期层级表 | [组件-Toast轻提示.md](组件-Toast轻提示.md) |
