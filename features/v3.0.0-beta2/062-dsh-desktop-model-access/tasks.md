...
### 2026-09-10：已批准的额度配置表结构修订

- [x] 用户确认：取消用户级 JSON，直接修改 dsh_user_policy 为用户＋模型一条记录；不新增明细表，不新增 Alembic。
- [x] 单模型保存/读取、索引分页、独立 SQL/Redis 版本、保留撤销版本及历史用量；更新设计及管理端契约。
- [x] 完成独立 MySQL/Redis 回归、109 表结构转换；证据见 model-policy-row-revision.md。不部署应用镜像。
交付方式：提交并推送当前 feature 分支，应用部署由用户处理。


## 2026-09-10 两层开关与下载配置

- [x] 用户确认部署开关与业务开关职责，默认均关闭；License 不在界面管理。
- [x] 复用全局 Config 项保存类型化业务配置，管理接口不依赖已开启的业务 runtime。
- [x] 每次新 DSH 准入检查业务状态；不停止在途结算及后台投影。
- [x] 管理页、模型开放范围与 desktop-login 使用两层状态；下载链接运行期更新。
- [x] 补齐配置 API、页面、HTTP URL、权限和缓存 runtime 的关闭/重开回归。
- [x] 同步设计、验收、部署说明和技术图稿；不修改 109。

交付仍为两仓各自 feature 分支提交推送，应用部署由用户处理。
...
- [x] 用户确认本人查询和逐会话吊销。
- [x] 工作台桌面/移动头像菜单、本人弹窗、下载/唤起及同一业务开关。
- [x] 本人 API、Gateway HMAC 归属检查和幂等吊销；主部门仅在毕昇侧展示。
- [x] 上述接口/页面回归和静态质量检查。
