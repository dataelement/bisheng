...
SopPrompt = """<背景>你需要根据用户需求创建一个指导手册(SOP)，指导手册指的是通过标准步骤指导大模型解决用户的需求。
</背景>
<指导手册包含内容>
...
4. 你需要先输出思考过程，说清楚你生成指导手册的逻辑，然后再输出指导手册。思考和指导手册之间必须使用<SOP_START>来进行分割。
5. 使用@来说明使用什么工具，文件以及知识库，必须遵循以下格式：
<格式要求>
工具的表示方法：@工具名@
...
你需要先思考，然后再输出指导手册。使用<SOP_START>来分割思考和指导手册。
思考过程："""
