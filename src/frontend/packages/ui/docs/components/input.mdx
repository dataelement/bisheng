...
后缀放「明暗切换」——这个动作属于这个框，做成框外按钮会把表单排版撑乱。图标默认取 `bisheng-icons` 的 Eye / EyeClose，不用传。
...
嵌在**自带描边的容器**里（面板头部、拼装的组合控件），框的描边、悬停、聚焦环整套交给容器，`borderless` 一个 prop 关掉——不要用 `border-none` 类名去压，聚焦环是阴影画的，压不干净。尺寸和带边形态完全一致（1px 透明边留着）。

```tsx
import { SearchInput } from '@bisheng/ui';

export default () => (
  <div style={{ maxWidth: 380, border: '1px solid rgb(var(--border-base))', borderRadius: 8 }}>
    <div style={{ borderBottom: '1px solid rgb(var(--border-base))', padding: 4 }}>
      <SearchInput borderless placeholder="容器自己画边，聚焦不再出灰环" clearLabel="清空" />
    </div>
    <div style={{ padding: 12, color: 'rgb(var(--text-3))', fontSize: 13 }}>面板内容区……</div>
  </div>
);
```
...
| `borderless` | `boolean` | `false` | 嵌在自带描边的容器里用：描边 / 悬停 / 聚焦环整套关掉，态归容器 |
...
| prop | 类型 | 说明 |
|---|---|---|
| `revealIcon` / `hideIcon` | `ReactNode` | 默认 `Outlined.Eye` / `Outlined.EyeClose`（bisheng-icons），少数要换图标的场景才传 |
