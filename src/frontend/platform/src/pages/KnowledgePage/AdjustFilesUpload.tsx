...
export default function AdjustFilesUpload() {
    ...
    const [resultFiles, setResultFiles] = useState([
        // 固定从路由状态生成文件列表（跳过上传）
        {
            ...
            split_rule: initFileData.split_rule,
            status: initFileData.status
        }
    ]);
    console.log(resultFiles, 22);
    ...
    const [originalSplitRule, setOriginalSplitRule] = useState(resultFiles[0].split_rule);
    ...
    const handleNext = () => {
        switch (currentStep) {
            ...
            case 2: // 步骤2→步骤3（对比→处理）
                setCurrentStep(3);
                if (segmentRules) {
                    console.log("=== 调用 handleSave，segmentRules:", segmentRules);
    ...
    const handleSave = (_config) => {
        ...
        console.log(originalSplitRule, 2222);

        // 调整模式API参数格式（复用文件数据）
        const apiConfig = {
            knowledge_id: Number(_config.rules.knowledgeId || initFileData.knowledgeId || knowledgeId), // 从文件数据取知识库ID
            separator: originalSplitRule.separator,
            separator_rule: originalSplitRule.separator_rule,
            chunk_size: originalSplitRule.chunk_size,
            chunk_overlap: originalSplitRule.chunk_overlap,
            excel_rule: originalSplitRule.excel_rule,
            ...
            //   })),
            retain_images: originalSplitRule.retain_images,
            enable_formula: originalSplitRule.enable_formula,
            force_ocr: originalSplitRule.force_ocr,
            fileter_page_header_footer: originalSplitRule.filter_page_header_footer
        };
        console.log("=== 提交给 rebUploadFile 的参数:", apiConfig, _config.rules.knowledgeId);
        console.log("=== kb_file_id 是否有效:", !!apiConfig.kb_file_id);
        console.log("=== knowledge_id 是否有效:", !isNaN(apiConfig.knowledge_id), knowledgeId);
        captureAndAlertRequestErrorHoc(rebUploadFile(apiConfig).then(res => {
            console.log(res, 45);
            let _repeatFiles
            if (res.status === 3) {
                _repeatFiles = res
                ...
                if (_repeatFiles) {
                    ...
                    message({ variant: 'success', description: t('调整分段策略成功') }); // 调整成功文案
                    setCurrentStep(3);
                }
            }
            // 更新文件ID
    ...
    const handleRetry = (objs) => {
        setRetryLoad(true);
        const params = {
            knowledge_id: Number(_tempConfigRef.current.knowledge_id || knowledgeId),
                    ...
                    <div className="h-full py-4">
                        {currentStep === 1 && (
                            <FileUploadStep2
                                ...
                                // 新增：传递文件状态（可选，如子组件需根据状态禁用操作）
                                fileStatus={resultFiles[0].status}
                            />
                        )}
                        ...
                        {/* 步骤3：数据处理 */}
                        {currentStep === 3 && (
                            <FileUploadStep4 data={resultFiles} kId={knowledgeId} originalSplitRule={originalSplitRule} isAdjustMode />
                        )}
                    </div>
                </div>
            </div>
