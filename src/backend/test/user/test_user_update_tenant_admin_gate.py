...
The tenant#admin check itself has since moved behind
``bisheng.permission.application.is_tenant_admin`` (4386d9c05, "expose lazy
application runtime"), so the helpers no longer spell ``object_type='tenant'``
inline — ``_asserts_tenant_admin_check`` accepts either shape.
"""
...
USER_PY = Path(__file__).resolve().parent.parent.parent / "bisheng" / "user" / "api" / "user.py"
...
def _asserts_tenant_admin_check(src: str) -> None:
    """The helper must consult the ``tenant#admin`` FGA tuple, either through
    the original inline ``check(object_type='tenant', ...)`` or through the
    ``is_tenant_admin()`` application helper that replaced it."""
    assert "object_type='tenant'" in src or 'object_type="tenant"' in src or "is_tenant_admin(" in src, (
        "helper must check the tenant#admin relation"
    )
...
def test_tenant_admin_helper_defined():
    ...
    _asserts_tenant_admin_check(src)
...
def test_update_endpoint_calls_unified_gate_not_dept_only():
    ...
    assert "_dept_admin_can_manage_member_account_status(" not in src, (
        "update() should delegate to the unified gate; direct dept-admin call is the bug being fixed"
    )
...
def test_tenant_admin_scoped_user_ids_helper_defined():
    ...
    assert "ROOT_TENANT_ID" in src, "Root tenant must be short-circuited"
    assert "UserDepartment" in src, "tenant scope must reverse-lookup user_ids via UserDepartment"
...
def test_list_user_combines_dept_and_tenant_scope():
    ...
    assert "_department_admin_scoped_user_ids(" in src, "list_user must keep dept-admin scope as well"
