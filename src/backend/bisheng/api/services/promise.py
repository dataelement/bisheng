...
class PromiseService:
    ...
    def create_promise(cls, login_user: UserPayload, business_id: str, promise_id: str):
        """ 给某个业务新建承诺书 """
        logger.info(f'act=create_promise user={login_user.user_name}, business_id={business_id}, promise_id={promise_id}')
    ...
    @classmethod
    def user_write_promise(cls, login_user: UserPayload, business_id: str, promise_id: str) -> BusinessUserPromise | None:
        """ 用户签署某个业务的某个承诺书 """
        logger.info(f'act=user_write_promise user={login_user.user_name}, business_id={business_id}, promise_id={promise_id}')
        business_list = PromiseDao.get_promise(business_id=business_id, promise_id=promise_id)
        if len(business_list) == 0:
            return None

        res = PromiseDao.create_user_promise(UserPromise(business_id=business_id,
                                                         promise_id=promise_id,
                                                         user_id=login_user.user_id,
