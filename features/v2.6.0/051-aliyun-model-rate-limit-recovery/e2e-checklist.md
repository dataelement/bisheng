...
- [ ] 当前页面点击重试；预期从同一原业务记录恢复。刷新后限流卡、失败片段和次数允许消失，历史只显示原用户问题；不要求两个标签页共享状态。
- [ ] 在同一页面对同一原消息连续三次点击重试，并让三次结果都再次返回临时限流；仅第三次限流终态后出现“当前模型仍未恢复”的换模建议。任一次成功、非限流错误或恢复拒绝都清零；页面不显示次数，刷新后允许重新计数。
- [ ] 关闭弹窗或点“稍后再试”，预期零模型调用且原请求不变。
- [ ] 选择模型 B 并确认，预期无需后端次数前置条件即可在原位置继续，当前页面计数清零；确认前将模型 B 改为繁忙，预期返回统一 `12048/recovery_rejected` SSE 终态并要求重新选择；其他模型错误沿用旧流程。
- [ ] 删除原消息、撤销资源权限或让日常 checkpoint 不再安全可继续，再点击重试；预期统一返回 `12048/recovery_rejected`，不创建新的 busy 状态、不显示 provider detail，当前 Retry 入口结束。
...
- [ ] 打开换模弹窗，预期复用当前模型列表并排除当前模型和 busy 模型，不新增离线、权限或配置集合判断。
- [ ] 弹窗打开后撤销文件/空间权限再确认，预期服务端拒绝并保留原请求。
