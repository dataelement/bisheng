#!/usr/bin/env python3
r"""Move classified files from multiple spaces into public knowledge spaces.

The script scans every successful file in the supplied source knowledge spaces,
resolves its first- and second-level portal category labels, and routes it to the
uniquely named public knowledge space and direct root folder. Complete document
version chains move as one business unit. The default mode is a read-only
dry-run; pass ``--apply`` only after reviewing the generated JSON report.
...
Run from ``src/backend``::
    ...
    PYTHONPATH=./ .venv/bin/python scripts/move_knowledge_space_files.py \
      --source-space-id 10 --source-space-id 11 --apply
"""
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        action="append",
        type=_positive_int,
        help="source knowledge-space ID; pass multiple times to scan multiple spaces",
    )
    parser.add_argument(
        "--report-dir",
        type=Path,
...
def _resolve_single_unit(
    ...
    route = target_index.resolve(category.category_label, category.subcategory_label)
    ...
    *,
    ...
    for record in eligible:
        ...
        if version is not None:
            ...
            continue
        unit, file_skip = _resolve_single_unit(record, source_spaces, category_index, target_index)
        if unit is not None:
            preliminary.append(unit)
...
async def build_migration_plan(args: argparse.Namespace) -> MigrationPlan:
    ...
    with _tenant_scope(tenant_id):
            ...
            if public_space_ids:
                public_spaces = list(
                    (
                        await session.exec(
                            select(Knowledge).where(
                                col(Knowledge.id).in_(public_space_ids),
                                Knowledge.type == KnowledgeTypeEnum.SPACE.value,
                            )
                        )
                    ).all()
                ...
                target_records = list(
                    (
                        await session.exec(
                            select(KnowledgeFile).where(col(KnowledgeFile.knowledge_id).in_(public_space_ids))
                        )
                    ).all()
                )
        ...
        target_index = TargetRouteIndex.from_records(
            ...
            {int(owner.user_id): owner for owner in owners if owner.user_id is not None},
        )
...
class BishengMoveOperations:
    ...
    async def copy_file(self, source_file: KnowledgeFile, target: TargetContext) -> KnowledgeFile:
        ...
        try:
            ...
            return await KnowledgeFileDao.async_update(target_file)
...
def _parameters_for_report(args: argparse.Namespace) -> dict[str, Any]:
    return {"source_space_ids": list(args.source_space_ids)}
