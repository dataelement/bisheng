...
def _services(*, channel=None):
    ...
    query_service = SimpleNamespace(validate_creation_grants=AsyncMock())
    authorization_service = SimpleNamespace(authorize_channel=AsyncMock())
...
async def test_create_then_authorize_channel():
    ...
    query_service.validate_creation_grants.side_effect = validate
    ...
    validation_kwargs = query_service.validate_creation_grants.await_args.kwargs
    assert validation_kwargs["resource_type"] == "channel"
    assert validation_kwargs["login_user"] is login_user
    assert len(validation_kwargs["grants"]) == 1
    assert validation_kwargs["grants"][0].subject_id == 11
...
async def test_invalid_subject_rejected_before_create():
    ...
    query_service.validate_creation_grants.side_effect = validation_error
