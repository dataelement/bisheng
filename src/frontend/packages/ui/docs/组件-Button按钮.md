...
2. 颜色全走 token：品牌用 `--primary` / `--brand-*`（跟蓝⇄绿主题）；中性灰系、危险红系建语义变量（`--btn-*` 系，落 style.css + tailwind `btn-*`），组件内**不留裸 hex**。危险三态：base `#f53f3f`、hover `#d6373a`、active `#d02f33`。中性：文字 `#525865`、边框 `#e5e6e9`、hover 底 `#f8f8f8`。disabled 三 token：浅色 `#F5F5F5` 底 / `#BFBFBF` 字 / `#D9D9D9` 边，深色自动翻转为可见的低对比灰。深色下品牌与危险均随各自深色阶翻转（2026-08-26 建档）：`blue-*` 走品牌深色阶（tokens.css `.dark`），`--btn-danger*` 惰性引用 `--danger*` 随功能色深色阶走——组件内不再有任何品牌 `dark:` 覆盖。
...
| 2026-08-26 | **品牌 / 功能色深色阶建档，收编按钮内深色补丁**（色阶本体见 基础-色彩规范 §1/§3，Arco 官方深色算法）：primary outlined / filled / text 三组 `dark:` 品牌透明底补丁删除，`blue-*` 类直接经 `.dark` 的品牌深色阶解析（浅色 tint 档在深色成为深藏青色底）；`--btn-danger*` 由字面量改为惰性引用 `--danger*`（浅色取值不变），危险系随功能色深色阶整体翻转；`.theme-green .btn-brand-primary` 换肤 hack 补深色三态（`#3CB062` / hover `#2A9C54` / 触屏 active `#60C47E`） | design-token.cjs、tokens.css、Button.tsx、本文件「落地」§2 | 待 committer 窗口提交（token 一笔） |
