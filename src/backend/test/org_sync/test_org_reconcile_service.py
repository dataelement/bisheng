...
def reconcile_mocks(monkeypatch):
    ...
    monkeypatch.setattr(f"{MODULE}.decrypt_auth_config", MagicMock(return_value={}))

    # DepartmentDao
    aget_active_by_tenant = AsyncMock(return_value=[])
    aget_by_source_ext = AsyncMock(return_value=None)
    aarchive = AsyncMock()
    monkeypatch.setattr(f"{MODULE}.DepartmentDao.aget_active_by_tenant", aget_active_by_tenant)
    monkeypatch.setattr(f"{MODULE}.DepartmentDao.aget_by_source_external_id", aget_by_source_ext)
    monkeypatch.setattr(
    ...
    monkeypatch.setattr(f"{MODULE}.DeptUpsertService.upsert_from_sync_payload", upsert_from_payload)

    # DepartmentDeletionHandler + UserTenantSyncService
    on_deleted = AsyncMock()
    monkeypatch.setattr(f"{MODULE}.DepartmentDeletionHandler.on_deleted", on_deleted)
    sync_user = AsyncMock(return_value=MagicMock(id=1))
    monkeypatch.setattr(f"{MODULE}.UserTenantSyncService.sync_user", sync_user)

    # UserDepartmentDao
    aget_user_ids = AsyncMock(return_value=[])
    monkeypatch.setattr(f"{MODULE}.UserDepartmentDao.aget_user_ids_by_department", aget_user_ids)
    ...
    monkeypatch.setattr(f"{MODULE}.OrgSyncLogDao.acreate_event", acreate_event)
    flush_log = AsyncMock()
...
class TestReconcileHappyBranches:
    ...
    async def test_reconcile_primary_dept_change_triggers_user_tenant_sync(
        ...
        mount_a = _make_dept(
            ...
            external_id="MA",
            is_tenant_root=1,
            mounted_tenant_id=10,
        )
        mount_b = _make_dept(
            id=2,
            external_id="MB",
            is_tenant_root=1,
            mounted_tenant_id=20,
        )
        ...
        reconcile_mocks.provider.fetch_departments.return_value = [
            ...
            RemoteDepartmentDTO(external_id="E1", name="Dept", parent_external_id="MB"),
        ]
...
class TestSameTsRemoveWins:
    async def test_reconcile_same_ts_upsert_then_remove_prefers_remove(
        ...
        event_calls = [
            c for c in reconcile_mocks.acreate_event.await_args_list if c.kwargs.get("event_type") == "ts_conflict"
        ]
...
class TestLockingAndSkip:
    async def test_reconcile_redis_lock_busy_raises_19314(
        ...
        await reconcile_mocks.redis.set("org_reconcile:14", b"1", nx=True, ex=1800)
        ...
        assert str(expected_code) in msg or expected_code in getattr(excinfo.value, "args", ())
...
class TestLockSemantics:
    async def test_concurrent_same_ts_same_config_deduped_by_setnx(
        ...
        first = asyncio.create_task(OrgReconcileService.reconcile_config(71))
        await started.wait()
        ...
        try:
            ...
            expected_code = SsoReconcileLockBusyError.Code
            assert str(expected_code) in str(excinfo.value) or (expected_code in getattr(excinfo.value, "args", ()))
        finally:
            release.set()
            await first
    ...
    async def test_different_config_ids_lock_independently(
        ...
        task1 = asyncio.create_task(OrgReconcileService.reconcile_config(81))
        await started1.wait()
        ...
        result2 = await asyncio.wait_for(
            OrgReconcileService.reconcile_config(82),
            timeout=1.0,
        )
    ...
    async def test_lock_released_even_on_exception(self, reconcile_mocks):
        ...
        reconcile_mocks.provider.authenticate.side_effect = RuntimeError("simulated auth failure")
