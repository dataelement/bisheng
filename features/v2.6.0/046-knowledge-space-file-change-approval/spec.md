...
## 0. 变更备注（cofco 分支 · 2026-08）

> **背景**：0811 中粮分支对文件变更审核的开关口径做出调整，落到 `feat/2.6.0-cofco-0811`。本节记录新的需求口径，并在冲突处取代下方对应 AC 的原表述；本节生效，其余 AC 不变。

- **CN-01 三开关"与"关系** — 某次文件变更是否需要走审批，由以下三个开关共同决定，**任一关闭即跳过审批、直接执行**（私密空间与所有者/管理者本人操作仍按原有直接执行规则）：
  1. **审批场景开关** —— 审批中心的【知识空间文件变更审批】场景 `enabled`；
  2. **总开关** —— 平台管理员在知识空间构建界面配置的"启用文件变更审核"；
  3. **空间单独开关** —— 该知识空间的单空间"是否需要审核"配置（按审核范围取默认值）。
- **CN-02 审批场景改为可关闭** — 【知识空间文件变更审批】场景由原来的"系统固定、永久启用、不可关闭"改为**支持在审批中心独立启用/停用**。其审批流程、审批人（所有者/管理者或签）、路由/分支仍为系统固定只读，仅提供启用/停用开关；不支持改名、删除、增改路由与流程。**取代 AC-01 中"仅总开关可关闭"的隐含口径**（原 AC-01 关于总开关与审核范围的部分继续有效）。
- **CN-03 "按知识空间配置"未配置空间默认无需审核** — WHERE 审核范围为"按知识空间配置"，未单独配置的非私密知识空间默认为**"无需审核"**（仅被显式打开的空间才走审批）。**取代 AC-03、AC-06 中"未单独配置默认为需要审核"的表述**。
- **CN-04 "全部知识空间"下单空间可显式豁免** — WHERE 审核范围为"全部知识空间需要审核"，未单独配置的非私密知识空间默认"需要审核"，但**允许对单个非私密知识空间显式关闭以豁免审批**；显式关闭优先于该范围默认值。**补充/放宽 AC-02**（原"统一启用、忽略单空间"的口径不再适用）。
- **CN-05 场景关闭时的执行语义** — WHEN 审批场景被停用，THE SYSTEM SHALL 对之后发起的文件变更直接执行、不创建审批实例，也不得因场景停用而抛出错误或产生审批异常。
...
- **AC-01** — THE SYSTEM SHALL 允许平台管理员为当前租户关闭文件变更审核，或在开启后选择“全部知识空间需要审核”与“按知识空间配置”两种审核范围。（**cofco 分支另新增审批中心场景开关，见 §0 CN-01/CN-02；三开关任一关闭即免审**）
- **AC-02** — WHERE 审核范围为“全部知识空间需要审核”，THE SYSTEM SHALL 对当前及后续新建的非私密个人知识空间、集团知识空间和部门知识空间统一启用文件变更审核；私密知识空间始终免审。（**cofco 分支按 §0 CN-04：默认需审核，但允许对单个空间显式关闭以豁免**）
- **AC-03** — WHERE 审核范围为“按知识空间配置”，THE SYSTEM SHALL 允许平台管理员对非私密知识空间逐空间选择“无需审核”或“需要审核”，且未单独配置的非私密知识空间默认为“需要审核”；私密知识空间不得因单空间配置进入审核。（**cofco 分支已按 §0 CN-03 调整为"未单独配置默认无需审核"**）
...
- **AC-06** — IF 租户尚无文件变更审核总控配置，THEN THE SYSTEM SHALL 按“总开关开启、按知识空间配置、各空间默认需要审核”处理。（**cofco 分支按 §0 CN-03 调整为"按知识空间配置下各空间默认无需审核"**）
