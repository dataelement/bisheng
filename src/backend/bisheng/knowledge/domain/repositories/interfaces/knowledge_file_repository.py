...
class FolderDescendantStatusFlags:
    has_abnormal_files: bool
    has_processing_files: bool
...
class KnowledgeFileRepository(BaseRepository[KnowledgeFile, int], ABC):
    ...
    async def find_folder_descendant_status_flags(
        ...
        """Return descendant flags for one non-overlapping folder-prefix group.

        `excluded_file_ids` are files the caller has already decided the current
        viewer must not see (F046 staged/pending-approval uploads). They are left
        out of the aggregate entirely, so a folder never lights up "存在异常" over
        a file that is absent from the listing underneath it.
        """
