...
| AC-38 | 部门管理员 | 打开页面进入**模式 B** | 只能看到其管理范围内知识空间产生的待审核/已驳回标签；范围外标签不出现在列表和计数中 |
| AC-42 | 部门管理员 | 打开页面进入**模式 A** | 看到当前租户下的全部已入库标签，**不按部门范围收窄**（见 AD-12）；跨租户数据仍然不可见 |
...
| AD-12 | 模式 A 的可见范围 | A: 仅租户 / B: 按 `tag_link` 的文件所属空间 / C: 按标签库绑定的空间 | **A** | 见下方 AD-12 说明。 |
...
### AD-12 说明：模式 A 只按租户过滤

标签库是**租户级词表**，不是某个知识空间的私有资产：左栏的标签库列表接口
（`GET /knowledge/space/tag-libraries`）本来就只按租户过滤，`tag_count` 也是全租户口径。

否掉另外两个选项的理由：

- **按 `tag_link` 过滤**：左栏显示「工序 (4)」而右栏只出 2 条，两边数字对不上；
  更要命的是**从没被用过的标签会直接消失**——管理员刚「添加」完一个标签就看不见它，
  这个表现一定会被当成 bug 报回来。
- **按标签库绑定的空间过滤**：语义最顺，但必须连左栏一起过滤，
  而左栏走的是工作台旧页面同一个接口，改它就破坏 AC-40。

**已知代价（用户 2026-08-07 知情后选定）**：本页的后端门槛是
`resolve_reviewable_space_ids()`，部门管理员可以进入；而工作台旧页面只有超管 / 子租户管理员能开。
因此**部门管理员会在本页第一次看到全租户的标签库与标签**。
这是一次有意的可见性放宽，限于「标签名 + 所属库 + 审核留痕」这类词表元数据，
不涉及任何知识文件正文；模式 B 的空间范围过滤不受影响（AC-38）。
