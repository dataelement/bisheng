...
class FineGrainedPermissionService:
    ...
    def _binding_matches_current_user(
        ...
        if not binding_path:
            return False
        return any(
            bool(user_path) and user_path.startswith(binding_path) for user_path in user_department_paths.values()
        )
    ...
    async def get_matching_binding_subject_types_async(
        ...
        user_department_paths = (
            await cls.get_current_user_department_paths(user_subject_strings) if needs_department_paths else {}
        )
    ...
    def build_binding_index(
        ...
        """F036-③: group bindings by (resource_type, str(resource_id)), preserving list order.

        Lets ``_resolve_binding_for_tuple`` / ``_permission_ids_from_bindings`` iterate only
        the bindings touching a given resource instead of linearly scanning the whole list per
        tuple. Order within each group is preserved so "first match wins" stays identical to the
        un-indexed scan. Build once per request and thread through; do **not** rebuild per item.
        """
        index: dict[tuple[str, str], list[dict]] = {}
        for binding in bindings:
            key = (binding.get("resource_type"), str(binding.get("resource_id")))
    ...
    async def _permission_ids_from_bindings(
        ...
        *,
        ...
        for resource_type, resource_id in lineage:
            ...
            # F036-③: when a prebuilt (resource_type, resource_id)->bindings index is
            # supplied, iterate only the matching group instead of scanning the whole
            # bindings list. The inner resource_type/resource_id checks below are kept so
            # the linear (binding_index=None) path stays byte-for-byte equivalent; for the
            # indexed path they are always true (the group is already filtered).
            candidates = (
                binding_index.get((resource_type, str(resource_id)), []) if binding_index is not None else bindings
            )
    ...
    async def _resolve_binding_for_tuple(
        ...
        # F036-③: iterate only the (resource_type, resource_id) group when an index is
        # supplied; the inner resource_type/resource_id checks are kept so the linear
        # (binding_index=None) path stays byte-for-byte equivalent.
        candidates = binding_index.get((resource_type, str(resource_id)), []) if binding_index is not None else bindings
        for binding in candidates:
            if binding.get("resource_type") != resource_type:
                continue
            if str(binding.get("resource_id")) != str(resource_id):
    ...
    async def get_effective_permission_ids_async(
        ...
        *,
        ...
        # F036-③: build the (resource_type, resource_id)->bindings index once here when callers
        # did not supply one. Per-item callers (knowledge-space child filtering) build it once
        # per request and pass it in to avoid rebuilding per item.
