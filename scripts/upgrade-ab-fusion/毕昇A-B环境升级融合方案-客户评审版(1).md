...
密码、JWT、登录 Session 和 Token 不迁移。B 独有本地用户在 A 中采用重置密码、统一身份接入或客户批准的重新激活流程。
...
- 积分、计费、审计等数据是否进入 A 在线库，由客户确认其业务与合规属性后决定。
...
|---|---|---|---|
| D01 | 最终统一环境 | A 环境 | 待确认 |
| D02 | A 知识空间处理 | 原地保留，禁止重解析和重建 | 待确认 |
| D03 | B 升级目标构建 | 与 A 实际镜像摘要、提交和 Alembic head 完全一致 | 待补齐 |
| D04 | 用户权威标识 | 首钢员工编码，A 用户为目标主体 | 待确认 |
| D05 | B 独有本地用户 | 在 A 新建本地/统一身份账号，保留业务 | 待确认 |
| D06 | 歧义用户 | 人工逐项确认，不按姓名自动合并 | 待确认 |
| D07 | 部门权威体系 | A 的首钢组织树；B 本地组独立迁入 | 待确认 |
| D08 | B 旧个人知识库 | 升级后作为 B 业务资源迁入 A | 待盘点确认 |
| D09 | B 知识库解析产物 | 兼容时物理迁移，不重新 Embedding | 待确认 |
| D10 | 不兼容 B 文件 | 仅逐项审批后重解析，并锁定/回挂业务属性 | 待确认 |
| D11 | 两边会话和消息 | 全部保留并在 A 中合并展示 | 待确认 |
| D12 | 迁入会话能否继续对话 | 依赖应用验证通过则允许；缺失应用只读 | 待确认 |
| D13 | 同名工作流/知识库 | 默认增加“[B迁移]”，不自动合并 | 待确认 |
| D14 | 分享链接和 API Token | A 中重新生成/签发 | 待确认 |
| D15 | 标注、报表、收藏等 | 随核心业务完整迁移 | 待确认 |
| D16 | 审计、积分、遥测数据 | 逐类确认在线迁移或专库归档 | 待确认 |
| D17 | 租户映射 | 明确每个 B 租户对应的 A 租户 | 待补齐 |
| D18 | 共享存储路由 | 以 A 当前路由为准，与融合分批实施 | 待确认 |
| D19 | RPO | 最终冻结并完成增量后 RPO=0 | 待确认 |
| D20 | 维护窗口 | 以演练实测最大耗时 + 30% 缓冲确定 | 待演练确认 |
| D21 | B 只读观察期 | 建议 2–4 周 | 待确认 |
| D22 | 快速回滚截止点 | 演练后确定，超过后优先前向修复 | 待确认 |
| D23 | 可接受例外 | P0=0；P1 原则上为 0；P2 逐项签字 | 待确认 |
