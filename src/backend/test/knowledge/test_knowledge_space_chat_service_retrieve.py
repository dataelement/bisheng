...
async def test_aretrieve_chunks_for_kb_returns_empty_when_visibility_path_has_no_survivors(monkeypatch):
    """The shared visibility path owns empty/non-primary/F046 short-circuiting."""
    svc = _make_service()
    monkeypatch.setattr(svc_mod.KnowledgeDao, "aquery_by_id", AsyncMock(return_value=MagicMock(id=1)))
    ...
    assert out == []
    svc._retrieve_and_filter.assert_awaited_once()
