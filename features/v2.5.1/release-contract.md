...
| F022-llm-system-config-tenant-isolation | F011, F012, F019, F020 | 5 类系统级模型默认配置按租户隔离 + Root 共享 fallback；修订 F020 AD-07/AC-11；沿用模块编码 198（无新增）；alembic 编号 F034 |
...
| 2026-05-07 | **F022 实施完成**：5 类系统级 LLM 默认配置按租户隔离落地 — 新建 `tenant_system_model_config` 表（alembic F034）+ ORM/DAO（含 sync/async resolve 5 fallback 分支）+ envelope DTO；`LLMService` 5 类 get/update/sync_get 加 `tenant_id` 参数 + 5 个 `*_with_meta` envelope 变体；`/api/v1/llm/{type}` 5 POST 端点 router 层 `_assert_can_write_system_config` 防御性校验 + `audit_log('llm.system_config.update')`；Workflow → GraphEngine → BaseNode tenant_id 透传链 + `tasks.py` 一次解析 Flow.tenant_id 透传 RedisCallback 与 Workflow；INV-T17 / INV-T18 落地；19803 Msg 修订；**修订 F020 AD-07 + AC-11**；前端 `SystemModelConfig.tsx` 3 状态 ScopeBanner + 5 Tab inheritedBadge / fallback_blocked Banner + 6 i18n key（三语）；96 backend 测试 + 16 frontend source-inspection 测试全绿 | F022 + 修订 F020 spec.md（AC-11 / AD-07 / 19803 Msg）|
