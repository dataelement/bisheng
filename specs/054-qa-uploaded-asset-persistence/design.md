...
| REQ-008 | AC-REQ-008-01..02 | ORM-free field snapshot + clean response reconstruction | committed ORM regression test |
...
- Strategy: 通过 `model_dump()` 提取纯字段快照，合并 resolved 字段后用原模型 `model_validate()` 构造拥有独立 state 的响应对象。
...
| REQ-008 ACs | high/V2 | clean ORM response resolve/state isolation | committed ORM service test | EG-009 | Question/Answer 不报错、签名生效且原实例不变 |
