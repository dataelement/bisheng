...
class TestFileDownloadHonoursAWholeConversationShare:
    """A share recipient must be able to resolve the task's deliverables.

    ``file_download`` is what turns a workspace object key into a presigned link,
    so every preview, inline deliverable image and download on a share page goes
    through it. A ``workbench_chat`` share carries ``resource_id`` = the session
    and no ``meta_data.versionId``, so the grant rests entirely on the
    ``shared_to_session`` branch below — drop it and a recipient 403s the moment
    they open any produced file, which the client's global 403 handler turns into
    a whole-page bounce to ``/c/new?error=11403``.
    """
    ...
    def _whole_conversation_share(self, resource_id: str):
