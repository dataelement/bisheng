...
F045/F046 的 policy 禁止 `pass`，也不创建 `ApprovalOutbox`。F045 的知识空间/频道授权编排在场景缺失/关闭时不进入 submission，而是把新增个人用户降级为既有 direct 授权；一旦进入 F045 submission，审批人为空或固定流程不匹配仍必须失败。F046 场景异常始终不得降级为直接执行业务。
...
| `channel/domain/services/channel_authorization_service.py` | F045 频道 owner 适配；场景启用时新增个人用户经 Permission Application Service 建单，缺失/关闭时降级 direct；executor 实时重读频道/租户/授权边界并通过 `PermissionService.authorize` 唯一写入、随后权威 verify | `ChannelAuthorizationService.authorize_channel()`、`ChannelResourceGrantExecutor` |
...
- **关闭场景语义**：知识空间/频道授权（含新建资源初始授权）在场景缺失/关闭时不调用 `request_invite()`，而是按既有实时校验直接授权；场景启用但本人流程非法时仍失败且不得降级。`request_invite()` 若被直接调用，仍以 18106 失败关闭并回滚业务请求与 submission bundle。
