...
class _Session:
    """Answers exec() by statement shape: the per-folder status aggregate, the abnormal
...
async def test_invisible_failed_upload_does_not_mark_the_folder(monkeypatch):
    aggregate = [(10, KnowledgeFileStatus.SUCCESS.value, 3), (10, KnowledgeFileStatus.FAILED.value, 1)]
...
async def test_visible_failed_upload_marks_the_folder(monkeypatch):
    aggregate = [(10, KnowledgeFileStatus.TIMEOUT.value, 1)]
...
async def test_no_abnormal_files_skips_the_visibility_pass(monkeypatch):
    aggregate = [(10, KnowledgeFileStatus.SUCCESS.value, 2), (10, KnowledgeFileStatus.PROCESSING.value, 1)]
