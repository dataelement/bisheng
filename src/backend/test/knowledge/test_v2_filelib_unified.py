...
def _patch_identity(monkeypatch):
    """F049 removed the ``default_operator`` mechanism; every endpoint under test
    here now resolves the acting identity via ``get_open_api_operator_async``
    (sync counterpart: ``get_open_api_operator``), which reads the
    ``PermissionActor`` the router dependency installs from the open-API
    credential. Patch both entry points to a fake user so no principal/DB
    round-trip is needed.
    """

    async def _fake_operator_async():
        return _FakeUser()

    def _fake_operator():
        return _FakeUser()

    monkeypatch.setattr(filelib, "get_open_api_operator_async", _fake_operator_async)
    monkeypatch.setattr(filelib, "get_open_api_operator", _fake_operator)
...
async def test_list_dispatch_kb(monkeypatch):
    ...
    # F049 dropped the per-request `user_id` delegation kwarg: identity delegation
    # is now supplied only via the `X-On-Behalf-Of` header, resolved centrally
    # into the principal the `_patch_identity` fixture fakes out above.
