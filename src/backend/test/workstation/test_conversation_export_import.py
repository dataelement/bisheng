...
def _make_chat_message(id_: int, category: str, content: str, *, parent_msg_id: int | None = None) -> ChatMessage:
    import json
...
class _FakeSpaceService:
    ...
    Tests configure ``require_action_raises`` / ``add_file_raises`` /
    ...
    def __init__(self):
        self.require_action_raises: Exception | None = None
        self.add_file_raises_sequence: list[Exception | None] = []
        self.add_file_returns_sequence: list[list] = []
        self.add_file_attempts: list[dict] = []

    async def _require_action(self, object_type, object_id, action):
        if self.require_action_raises is not None:
            raise self.require_action_raises
...
def patch_import_deps(monkeypatch: pytest.MonkeyPatch):
    ...
    async def _fake_messages_get(_cls, message_ids, user_id, chat_id):
        return [m for m in state["messages"] if m.id in message_ids and m.user_id == user_id and m.chat_id == chat_id]

    async def _fake_chat_questions(
        _cls,
        chat_id,
        category_list=None,
        limit=500,
    ):
        del category_list, limit
        return [
            message for message in state["messages"] if message.chat_id == chat_id and message.category == "question"
        ]
    ...
    monkeypatch.setattr(
        ...
        classmethod(_fake_chat_questions),
    )
    ...
    monkeypatch.setattr(
        type(bisheng_settings),
        "aget_all_config",
        staticmethod(_fake_all_config),
    )
    ...
    class _Registry:
        ...
        @staticmethod
        def set_children(rows):
            state["children"] = list(rows)

        @staticmethod
        def saved_calls():
            return state["save_calls"]

    return _Registry()


def _build_request(parent_id: int | None = None) -> ImportMessagesToKnowledgeRequest:
...
async def test_import_permission_denied_pre_check(patch_import_deps):
    """AC-20: preflight concrete action denied → 12067 before any write."""
    space_service = _FakeSpaceService()
    space_service.require_action_raises = SpacePermissionDeniedError()
...
async def test_import_quota_exceeded(patch_import_deps):
    ...
    space_service.add_file_raises_sequence = [
        TenantStorageQuotaExceededError(
            msg="Storage quota exceeded", used_gb=10, quota_gb=10, tenant_name="t", tenant_id=1, reason="cap"
        ),
    ]
...
async def test_import_to_folder_calls_correct_permission_check(patch_import_deps):
    """AC-20/22: parent_id uses the folder upload_file action."""
