...
_INSTALLATION = re.compile(r"^[A-Za-z0-9_-]{1,64}$")
...
class ServiceKey:
    ...
    def __post_init__(self):
        if (
            not _INSTALLATION.fullmatch(self.installation_id)
            or not _TOKEN.fullmatch(self.key_id)
...
def canonical(method: str, path: str, body: bytes, installation: str, key: str, timestamp: str, nonce: str) -> bytes:
    ...
    return "\n".join((method, path, hashlib.sha256(body).hexdigest(), installation, key, timestamp, nonce)).encode()
...
class ServiceAuth:
    ...
    def sign(self, method: str, path: str, body: bytes) -> dict[str, str]:
        ...
        message = canonical(method, path, body, self.key.installation_id, self.key.key_id, timestamp, nonce)
        return {
            "X-DSH-Installation-Id": self.key.installation_id,
    ...
    async def verify(self, method: str, path: str, body: bytes, headers: Mapping[str, str]) -> str:
        ...
        installation = values.get("x-dsh-installation-id", "")
        ...
        if (
            installation != self.key.installation_id
            or key_id != self.key.key_id
        ...
        try:
            message = canonical(method, path, body, installation, key_id, timestamp, nonce)
        except ValueError:
        ...
        digest = hashlib.sha256(f"{installation}:{key_id}:{nonce}".encode()).hexdigest()
        if not await self.nonces.claim(f"dsh:service-nonce:{digest}", 120):
            raise DshInvalidAccessTokenError()
        return installation
