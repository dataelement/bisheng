...
import { resolveFileChangeSpaceId } from "./approvalCenterFileChangeUtils";
...
export function ApprovalPane({
  ...
  const runTaskDecision = async (action: "approve" | "reject") => {
    ...
    // Read the space id BEFORE deciding — the detail is replaced by the reload below.
    const refreshSpaceId = resolveFileChangeSpaceId(taskDetail);
    try {
      await decideApprovalTaskApi(selectedTaskId, { action, comment });
      setDecisionComment("");
      // F046: the knowledge file page renders pending uploads as virtual rows and
      // has no way to learn a decision was made here, so tell it to refetch.
      if (refreshSpaceId != null) dispatchFileChangeApprovalRefresh(refreshSpaceId);
      // A processed task must leave the pending list. Reload WITHOUT carrying its
      // old id: loadTasks would resolve it as processed and silently switch the
      // sub-filter over to 已处理.
      await loadTasks();
      onPendingCountMaybeChanged?.();
      toast(true);
    }
