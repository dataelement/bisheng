...
class SQLDatabaseChain(Chain):
    ...
    *Security note*: Make sure that the database connection uses credentials
    ...
    def raise_deprecation(cls, values: Dict) -> Dict:
        if 'llm' in values:
            warnings.warn('Directly instantiating an SQLDatabaseChain with an llm is deprecated. '
                          'Please instantiate with llm_chain argument or using the from_llm '
                          'class method.')
            if 'llm_chain' not in values and values['llm'] is not None:
                database = values['database']
                prompt = values.get('prompt') or SQL_PROMPTS.get(database.dialect, PROMPT)
        ...
        try:
            ...
            if not self.use_query_checker:
                ...
                checked_sql_command: str = query_checker_chain.predict(
                    callbacks=_run_manager.get_child(), **query_checker_inputs).strip()
                intermediate_steps.append(checked_sql_command)  # output: sql generation (checker)
...
class SQLDatabaseSequentialChain(Chain):
    ...
    def _call(
        ...
        table_names_to_use = [
            name for name in table_names_from_chain if name.lower() in _lowercased_table_names
        ]
        _run_manager.on_text('Table names to use:', end='\n', verbose=self.verbose)
        _run_manager.on_text(str(table_names_to_use), color='yellow', verbose=self.verbose)
        new_inputs = {
            self.sql_chain.input_key: inputs[self.input_key],
            'table_names_to_use': table_names_to_use,
        }
        return self.sql_chain(new_inputs,
                              callbacks=_run_manager.get_child(),
