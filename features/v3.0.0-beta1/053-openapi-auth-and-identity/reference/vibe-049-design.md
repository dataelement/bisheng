...
> **2026-08-18 增补 — AC-23 归属人租户口径:超管不必切 ScopeBar（用户反馈落地）**
> 原实现:创建服务账号时账号租户 = 当前 scope,归属人必须匹配该 scope;超管想给子租户的人当归属人须先用 ScopeBar 切进那个子租户。用户反馈"超管本就该有最大权限、不该为此切视图"。改法(`service_account_service.py`):**未切 scope 的全局超管,账号租户由所选归属人推导**(选谁就建在谁的租户、归他名下);已切 scope 的超管与租户/子管理员仍限本 scope、归属人必须匹配。配套:未切 scope 的超管其读/写 scope = 全租户(`_scope_tenant_id` 返 None → list/get/update/enable/disable/delete 跨租户),这样超管建完能看见能管;归属人若落在**归档**租户仍拒(`_resolve_owner` 加租户 active 守卫,租户行缺失时回落到 `UserTenant.status`)。**不变量守住**:账号与归属人始终同租户;`update` 改归属人仍须匹配账号既有租户(不能借改归属人迁租户);审计 `operator_tenant_id` 仍记操作者本租户。前端弹窗归属人选择器本就按可见子树取人(超管无 scope = 全租户可见),仅改 `tenantFixedTip` 文案为"归入所选归属人所在的租户"。测试:`test_service_account_service.py` 新增 2 例(超管跨租户派生 / 归档租户仍拒),存量 13 例全绿。
...
| 模块 / 文件 | 职责 | 不做什么 |
|---|---|---|
| **`open_api/`（新模块）** `api/router.py`（`/service-accounts` 挂 `/api/v1`；`/auth/whoami` 挂 `router_rpc`）· `api/exception_handlers.py`（`/api/v2` 专属 handler 的函数体，`main.py` 只调 `register_open_api_exception_handlers`）· `api/endpoints/service_account.py` · `api/endpoints/service_account_keys.py` · `api/endpoints/service_account_grants.py` · `api/dependencies.py`（`verify_open_api_access` router 级依赖 + `open_api_subject(scope=…)` 依赖工厂（返回可调用对象，用法 `Depends(open_api_subject(...))`）供 F053 / F055 新 router）· `domain/models/{api_credential,service_account}.py` · `domain/services/{credential_service,credential_validator,service_account_service}.py` · `domain/scopes.py`（`OPEN_API_SCOPES` + `@open_api_scope` 标记）· `domain/context.py`（ContextVar）· `domain/schemas/` | 凭据全生命周期、服务账号 CRUD、权限位判定、主体侧授权页读端点与 mutate 编排、审计事件 | 不写 OpenFGA、不自建授权镜像、不判具体资源权限（交 F048）、不含 UI 文案 |
...
| `main.py` | 在 `create_app` 内调 `register_open_api_exception_handlers(app)`（注册 `OpenApiAuthError` 与 v2 上 `PermissionServiceUnavailableError`（19002）/ `PermissionBackendUnavailableError`（19201）→ 503） | 不改 `handle_http_exception` 全局语义；handler 函数体在 `open_api/api/exception_handlers.py` |
...
| 14 | **`visible_tenant_ids` 是 IN-list 不是严格隔离**：子租户管理员的可见集是 `{leaf, Root}`（`http_middleware.py:210-231`），自动过滤对管理面读取会一并放出 Root 的服务账号 | AC-07「跨租户查询返回空」不成立：子租户管理员能列出并按 id 打开 Root 的服务账号 | `ServiceAccountService.get_row / get_detail / list_page` 显式带 `tenant_id` 条件（T024），不依赖自动过滤 |
| 14b | **UNION / 子查询绕过租户过滤**：租户监听器只对顶层 SELECT 注入（`tenant_filter.py:164`）；反查 SQL 若写成 UNION 或 `text()`，子查询不带 `tenant_id`；密钥表批量 UPDATE/DELETE 同样无注入 | 跨租户看到别人的授权 / 撤销别人的密钥 | 反查用单条 select + join；密钥写路径单行 ORM（K6） |
