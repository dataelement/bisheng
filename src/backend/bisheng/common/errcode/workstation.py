...
class LLMRateLimitError(BaseErrorCode):
    """Upstream LLM throttling (RPM / TPM / burst-rate) surfaced in daily-mode chat.

    Replaces the raw provider 500 so the frontend renders the friendly
    ``api_errors.12046`` copy instead of an English vendor message.
    """
