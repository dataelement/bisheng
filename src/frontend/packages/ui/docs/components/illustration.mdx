...
空状态 / 状态反馈用的主题化插画（`@bisheng/ui` → `src/components/Illustration/`，内联 SVG；client 经 `~/components/illustrations` re-export 同一实现），颜色走独立的插画调色板（比 UI 品牌色更亮），随蓝⇄绿主题自动切换，另有主题无关的灰稿模式。规范见「文档 → 设计规范 → 插画 Illustration」与「色彩 Color」§5。
...
const ILLUSTRATIONS = [
  ...
  ['SystemErrorIllustration', '系统错误 — 前端路由异常兜底页', SystemErrorIllustration],
] as const;
...
- `KnowledgeSpaceIcon` 不在上表内，也没进组件库：它是两张定制画稿按品牌主题整张替换的图标，不走 `--illus-*`、没有灰稿模式，且需要读 Recoil 主题状态（组件库契约禁止依赖状态管理），仍留在 client 的 `~/components/illustrations/`。
