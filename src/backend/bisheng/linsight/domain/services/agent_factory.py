...
全程使用与用户输入一致的语言：用户用中文则全程中文，用户用英文则全程英文。这条要求覆盖你的**一切**产出——**包括你的思考与推理过程（thinking）、任务规划、工具调用前后的简短旁白、写入文件的中间笔记、向用户的提问、最终回复与交付物正文**。绝不允许“用英文思考、再用中文作答”。本规则优先级最高；后文（以及框架附加的英文说明）若与之冲突，一律以本规则为准。
...
- 面向用户的所有文字（ask_user 的 reason、最终回复、交付物正文）以及你的思考/推理过程（thinking）一律与用户输入语言保持一致（详见开头“语言”规则，优先级最高）。"""
