...
const MD_COMPONENTS: Components = {
    h1: ({ children }) => <h1 className="text-[20px] leading-[1.35] font-bold tracking-tight text-[#161B26] dark:text-gray-100 mt-0 mb-3 break-words">{children}</h1>,
    h2: ({ children }) => <h2 className="text-[15.5px] leading-snug font-semibold text-[#1F2430] dark:text-gray-100 mt-[18px] mb-2 break-words">{children}</h2>,
    h3: ({ children }) => <h3 className="text-[14px] leading-snug font-semibold text-[#1F2430] dark:text-gray-100 mt-3.5 mb-1.5 break-words">{children}</h3>,
    p: ({ children }) => <p className="text-[14px] leading-[1.7] text-[#3F4654] dark:text-gray-300 my-0 mb-3 break-words">{children}</p>,
    ...
    li: ({ children }) => <li className="leading-[1.7] pl-1 break-words">{children}</li>,
    a: ({ children, href }) => <a href={href} target="_blank" rel="noreferrer" className="text-primary underline underline-offset-2 break-words">{children}</a>,
        ...
        <Sheet open={!!detail} onOpenChange={onOpenChange}>
            <SheetContent className="sm:max-w-[860px] w-[92vw] p-0 gap-0 flex flex-col bg-background">
                <div className="flex flex-col flex-1 min-h-0 px-7 pt-6 pb-6">
                    {/* header — title owns its row so a long name truncates instead of
                        shoving the actions off-screen; ID + status drop to a meta row below */}
                    <div className="flex items-start gap-3 pr-10">
                        <SheetTitle
                            className="flex-1 min-w-0 truncate text-[19px] leading-tight font-semibold text-[#161B26] dark:text-gray-100 mb-0"
                    ...
                    {/* meta row — ID chip truncates within the row width; status never wraps mid-word */}
                    <div className="flex items-center gap-2.5 flex-wrap mt-2">
                        <span className="min-w-0 max-w-full truncate text-xs font-mono font-medium text-[#7A8194] dark:text-gray-400 border border-[#E5E6EB] dark:border-gray-700 rounded-md px-[7px] py-[3px]" title={detail.name}>
                            ID: {detail.name}
                        </span>
                        <span className={`shrink-0 text-xs font-medium rounded-full px-[11px] py-1 whitespace-nowrap ${detail.enabled ? 'bg-[#E6EDFC] text-[#024DE3] dark:bg-primary/20 dark:text-primary' : 'bg-[#EEF0F3] text-[#6B7280] dark:bg-gray-800 dark:text-gray-400'}`}>
                            {detail.enabled ? t('skillManage.statusEnabled') : t('skillManage.statusDisabled')}
                        </span>
                    </div>
                    <p className="text-[13.5px] leading-relaxed text-[#6B7280] dark:text-gray-400 mt-2.5 break-words line-clamp-2" title={detail.description}>{detail.description}</p>
                    ...
                    <div className="flex gap-4 flex-1 min-h-0 mt-[18px]">
                        ...
                        <div className="flex-1 min-w-0 flex flex-col">
                            <div className="flex items-center justify-between gap-3 mb-2.5">
                                <span className="min-w-0 truncate text-[12.5px] font-mono text-[#9AA0AC]" title={selectedPath}>{selectedPath}</span>
                                {isSkillMd ? (
