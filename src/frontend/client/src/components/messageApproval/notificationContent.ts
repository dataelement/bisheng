...
export const APPROVAL_CENTER_ACTION_CODES = new Set([
    ...
    // NOT approval_execute_failed: business execution is a separate domain from
    // the approval fact (see the approval/business decoupling refactor), so an
    // execution outcome must not deep-link as if it were an approval to act on.
    // resource_user_invite_effective / _failed are excluded for the same reason —
    // they get display text below, but never approval-center routing.
...
/**
 * Whether a notification belongs to the approval centre at all. Falls back to the
 * system_text content part when `action_code` is absent — some senders only carry
 * the code there, and matching on the bare field silently misses them.
 */
export const isApprovalCenterNotification = (notification: MessageItem): boolean =>
    isApprovalMessageType(
        notification.message_type,
        notification.action_code || getActionCode(notification),
    );
...
export const resolveApprovalCenterTarget = (notification: MessageItem) => {
    ...
    for (const part of parts) {
        const metadata: PartMetadata = part?.metadata ?? {};
        const data = metadata?.data ?? {};
        // Read the task id straight out of `data` first: a part can carry both ids
        // while declaring business_type "approval_instance_id", and the type-driven
        // branch below would then only pick up the instance and drop the task —
        // the deep link lands on the request list instead of the task to act on.
    ...
    const tab =
        isPendingApprovalStatus(notification.status) ||
            ...
            // F045: the invitee is the approver, so this belongs in 我的审批.
