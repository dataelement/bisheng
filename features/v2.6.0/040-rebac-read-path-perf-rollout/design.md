...
**最后更新**: 2026-07-13（F 组在线 / 未分类列表兼容优化）
...
- **非目标**：
  ...
  - 不改 `/chat/online`·`/workstation/app/uncategorized` 的 `page/limit` 与裸列表响应契约；本轮不迁移其多端调用方。
...
- **INV-6**（F027）：走 ReBAC 过滤的高频列表用 cursor 分页。本特性 C 组遵循；B 组申请豁免（见决策 3）；F 组登记**兼容窗口**（见决策 7）：外部保留页码，内部仍使用 DM8-safe keyset 有界扫描，且不计算 total。
...
- **无 schema 变更**：F 组只扩展既有异步 DAO 查询能力，不新增表、索引或迁移；会话聚合子查询须显式注入 tenant 条件，因为它被 subquery 包装后不会命中 ORM 外层自动过滤。
...
### 决策 7：在线 / 未分类列表保留页码契约，内部改 keyset 有界扫描

- **背景实证（2026-07-13）**：普通用户请求 `/workstation/app/uncategorized?page=1&limit=20` 实测约 **11.49s**、响应仅约 0.3KB。根因是 `get_uncategorized_flows` 同步全量查应用（还执行无用 `COUNT(*)`）→ 对全部候选逐项精确 ReBAC → Python 切片；候选越多而用户可见越稀疏，首屏越慢。`/chat/online` 默认排序同样 `skip_pagination=True` 全取、全量装饰，再全取用户会话、Python 排序和切片，只是当前普通用户样本的候选集较小，尚未放大到同等耗时。
- **备选**：
  - A. 两端直接迁移 INV-6 cursor envelope，并同步所有前端调用方。
  - B. 保持外部页码，数据库用 OFFSET 分批，精确权限过滤后累计到目标页。
  - C. **保持外部页码，内部从稳定 keyset 起点扫描到目标页可见前缀**；数据库先用可访问 ID 粗过滤，每批精确复核，凑够即停，响应装饰只做最终页。
- **选定**：C（用户 2026-07-13 确认）。
- **原因**：A 会同时影响 client 应用广场/标签页/应用切换器与 Platform 多个页面，范围显著大于本次性能修复；B 在候选行增删时 OFFSET 批间可能重复/遗漏，且深 OFFSET 对 DM8/MySQL 都不理想；C 复用 F027 已验证的 `build_keyset_where` 展开式 OR 条件，首屏成本由租户全量降为“粗过滤后的少量候选批 + 一页精确权限”，同时零外部契约变更。
- **`/chat/online` 默认排序**：在 `FlowDao.aget_all_apps` 的既有异步能力上增加可选“当前用户最近会话排序”。DAO 将当前用户未删除会话按 `(flow_id, flow_type)` 聚合 `max(create_time)`，与应用 union 外连接；排序键为 `(used_rank ASC, coalesce(last_used_time, app.update_time) DESC, app.id DESC)`。游标使用同三键和 mixed-direction `build_keyset_where(..., descending=[False, True, True])`。这在数据库中等价保持“用过优先 / 最后使用倒序 / 未用按更新时间倒序”，并消除全量会话读取与 Python 排序。
- **`/workstation/app/uncategorized`**：标签与关联改走已有异步 DAO；没有关联时排除 ID 集为空（修复原先误返回空列表）。应用按既有 `(update_time DESC,id DESC)` keyset 扫描，`id_list_not_in` 在 SQL 层排除已分类应用；不再执行 count。
- **权限与装饰**：非管理员每次请求只构建一次应用权限上下文（models / bindings / subject strings / tuple cache），跨扫描批复用；每批同时求请求权限、`edit_app`、`share_app`，既用于精确过滤，也直接生成页内 `write/can_share`，避免第二轮 share 检查。管理员保持短路。`add_extra_field` 或 Logo 转换只接收最终页。
- **页码兼容语义**：目标可见前缀大小为 `page × limit`；两个外部接口都不返回 `has_more`，因此不为一个不可见的 `+1` 探针继续做权限扫描。深页需从首 keyset 扫到目标前缀，成本随页深增长；首屏/浅页不再随租户资源总量线性增长。
- **INV-6 兼容窗口**：这不是新增长期分页范式；release-contract 登记为 F040 的遗留 API 兼容窗口。约束为：不得 count、不得 fetch-all、内部必须 keyset、仅扫描到目标可见前缀。未来若改 cursor，另立契约迁移并一次性更新全部调用方。
- **何时该重新考虑**：应用广场出现稳定深翻需求（例如常态 page≫10）或应用切换器需加载超过当前上限时，启动 A；若会话聚合成为热点，再基于真实 EXPLAIN/trace 评估复合索引，不能在无证据时为双库增加迁移。
...
- **F 在线 / 未分类**：两端先取应用权限粗过滤集合，再调用同一有界扫描编排。在线默认排序把会话聚合与稳定排序下推 `FlowDao.aget_all_apps`；未分类把标签排除集合下推 SQL。扫描批只做精确权限，最终页才装饰并返回。
...
| `/chat/online`·`/workstation/app/uncategorized` | **不变**（`page/limit` → 裸应用列表） | 内部由全量加载改为 keyset 有界扫描，不返 cursor/total |
...
| `FlowDao.aget_all_apps` | 应用 union 异步查询、过滤、DM8-safe keyset；可选会话排序 | 不计算 F 组 total；不做权限判断 |
| `WorkFlowService` | F 组权限预过滤、批扫描、精确复核、页内装饰 | 不把粗过滤当最终授权；不全量取候选 |
...
| 11 | `message_session` 聚合被包装为 subquery 后，外层 tenant auto-filter 看不到内部表 | 普通用户最近会话可能跨租户参与排序 | 会话子查询显式 `build_tenant_filter_clause(MessageSession.tenant_id)`，与应用 union 的显式租户过滤一致 |
| 12 | DM8 运行时不支持行值比较；混合 ASC/DESC 排序也不能用单个 tuple comparator | MySQL 通过、DM8 `CODE:-2007` | 复用 `build_keyset_where` 展开 OR 梯形条件；在线排序传逐列方向 `[False, True, True]` |
| 13 | 粗粒度 `list_accessible_ids` / binding ID 集不能替代最终权限语义 | 仅凭预过滤结果放行会越权，或漏掉 legacy binding | 粗集合只进 SQL 缩候选；最终仍走 `ApplicationPermissionService` 精确 permission map，fail-closed |
| 14 | 应用标签存在但 tag link 为空，不代表“没有未分类应用” | 旧代码 `if not ret: return []` 把全部未分类应用吞掉 | 空 link 解释为“排除集合为空”，继续查应用 |
...
| `/chat/online`·`/workstation/app/uncategorized` 兼容列表 | HTTP（现有 `page/limit` + 裸列表） | client / Platform 现有调用方；无前端改动 |
...
| `message_session` 最近会话 | 既有 DB 表 | F 组在线默认排序；显式 tenant 过滤、只聚合当前用户未删除会话 |
...
### 6.4 Constitution Check（F 组增补）

| 条款 | 结论 | 依据 |
|---|---|---|
| C1 分层 / DDD | PASS | Router 只传参；扫描编排在 `WorkFlowService`；SQL 仅扩展既有 `FlowDao.aget_all_apps` 能力，不新增旁路 DAO 入口 |
| C2 MySQL + DM8 | PASS | SQLAlchemy 表达式；keyset 复用展开式 OR helper；无 row tuple / MySQL 函数 / migration |
| C3 多租户 | PASS | 应用 union 已显式 tenant；新增会话 subquery 同样显式 tenant；无跨租户缓存 |
| C4 权限 | PASS | 粗过滤仅缩候选，精确权限 map 才决定可见性；管理员/受限超管语义沿用 |
| C5 错误码 | PASS | 对外契约不新增错误场景或错误码；权限故障沿用现有异常 |
| C6 安全 | PASS | fail-closed；无日志敏感字段、无动态 SQL、无凭据变化 |
| C7 前端边界 | PASS | 无前端改动、无 store HTTP、无新依赖 |
...
| F 组两列表首屏候选/权限处理量 | 全量候选 + 全量权限 + Python 切片 | 正常粗过滤命中下 ≤1 个扫描批；稀疏可见性按批补取，凑满 `limit` 即停 | AC-33/34 |
| F 组响应装饰量 | 在线列表为全部候选；未分类 share 为最终页但权限已扫全量 | **≤limit**，Logo/用户/版本/标签/share 仅最终页 | AC-35 |
| `/chat/online` 用户会话读取 | 全量会话聚合结果回 Python | DB 内聚合并参与排序，只返回扫描批应用行 | AC-34/36 |
...
- **F 组等价测试**：用全量 oracle 对比 page 1/page 2、普通用户稀疏可见、管理员、同时间兜底序；断言扫描在凑满目标前缀后停止、页外不装饰。DAO 编译测试覆盖 MySQL 与 DM8 默认 dialect，确认无行值比较。
...
- **F 组外部 cursor 迁移**：本轮刻意保留遗留页码；若深页成为常态，另立契约迁移，不能继续扩展页码兼容窗口。
