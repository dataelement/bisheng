...
def convert_doc_to_docx(input_doc_path, output_dir=None):
    ...
    try:
        ...
        if os.path.exists(expected_file_in_outdir):
            ...
            logger.debug(f"Successfully converted '{input_doc_path}' to '{output_docx_path}'")
        ...
        logger.debug(f"Error: The LibreOffice executable '{soffice_path}' was not found.")
        logger.debug("Ensure LibreOffice is installed and the command is in your PATH or provide the full path.")
        return None
