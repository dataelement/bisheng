...
class CitationRegistryService:
    ...
    @classmethod
    def _extract_rag_chunk_id(cls, metadata: dict[str, Any]) -> str | None:
        """Extract a stable chunk identifier from common retrieval metadata fields."""
        return cls._parse_optional_text(metadata.get("id") or metadata.get("chunk_id") or metadata.get("pk"))

    @classmethod
    def _extract_rag_document_name(cls, metadata: dict[str, Any]) -> str | None:
        """Extract a stable document display name from common retrieval metadata fields."""
        return cls._parse_optional_text(
            metadata.get("document_name") or metadata.get("file_name") or metadata.get("name") or metadata.get("source")
        )

    @classmethod
    def _extract_rag_file_type(cls, metadata: dict[str, Any]) -> str | None:
    ...
    @classmethod
    def _extract_rag_bbox(cls, metadata: dict[str, Any]) -> str | None:
        ...
        if isinstance(parsed_bbox, dict):
            normalized_bbox = {key: value for key, value in parsed_bbox.items() if value not in (None, "", [], {})}
    ...
    def to_registry_item(self, citation: MessageCitation) -> CitationRegistryItemSchema:
        ...
        return CitationRegistryItemSchema(
            ...
            # Legacy rows persisted before F041 have no access_scope → default strict 'per_user'.
            accessScope=getattr(citation, "access_scope", None) or "per_user",
    ...
    async def get_citation(self, citation_id: str) -> CitationRegistryItemSchema | None:
