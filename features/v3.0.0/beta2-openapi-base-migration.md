...
6. **发布当前代码的 OpenFGA 模型**（执行时为 `f048-v6`，2026-09-16 起同一模型改名 `f048-v5-app`；以 dry-run 的目标 checksum 为准；服务全停、等 45s 心跳过期）：
