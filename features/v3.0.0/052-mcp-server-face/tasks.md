...
| **F057 T043**（本地 `bisheng dev` 期） | 受理**平台签发**的本地短时访问凭据：`dev` 的迷你代理每请求注入一个访问者凭据句柄，改由平台签发后，`/api/v2/filelib/retrieve` 与 MCP 检索工具须认得它并据以确立访问用户 | 🔲 **未交付**（2026-09-16 核实：全仓只有 `bisheng_cli/devproxy.py:117` 出现 `bsdev`，后端零受理） | = F057 design §6.2 阻塞项 ③ 的服务端半边（CLI 半边已回写 F053 T042）。在此之前本地期 retrieve 端到端不可验，SDK **不开兼容分支**，只把错误照实抛出并指路「发布后用真实账号验证」 |
