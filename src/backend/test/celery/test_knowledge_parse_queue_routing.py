...
def test_other_points_tasks_use_default_queue():
    """刷榜、月奖、对账继续使用默认队列。"""
...
@pytest.mark.parametrize(
    ...
    [
        {},
        {POINTS_AWARD_TASK: {"queue": "points_award_celery"}},
        {"bisheng.worker.points.*": {"queue": "points_award_celery"}},
    ],
)
def test_points_award_routes_to_default_even_with_legacy_config(configured_routes):
    """旧专用队列配置不能把发分任务路由回已移除的队列。"""
    routes = build_celery_task_routes(configured_routes)
    assert _resolve_queue(POINTS_AWARD_TASK, routes) == DEFAULT_CELERY_QUEUE
...
def test_runtime_yaml_declares_parse_only_knowledge_queue(config_path: Path):
    ...
    assert _resolve_queue(POINTS_AWARD_TASK, build_celery_task_routes(routes)) == DEFAULT_CELERY_QUEUE
    assert all(
        (route if isinstance(route, str) else route.get("queue")) != "points_award_celery"
...
def test_worker_entrypoints_use_default_worker_for_points_award():
    """发分复用默认 Worker，不再启动专用队列消费者。"""
    ...
    for source in (backend, deploy):
        assert "start_points_award" not in source
        assert "points_award_celery" not in source
        assert "POINTS_AWARD_CONCURRENCY" not in source

    backend_bundle = backend.split("start_all_workers()", 1)[1].split('case "$START_MODE"', 1)[0]
    assert "run_background start_default" in backend_bundle

    deploy_bundle = deploy.split('elif [ "$start_mode" = "worker" ]', 1)[1].split("else", 1)[0]
    assert "start_default &" in deploy_bundle
