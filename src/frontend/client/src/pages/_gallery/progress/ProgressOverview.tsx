 ...
 * 「设计规范」 mode instead; this board mirrors packages/ui/docs/00-总纲.md §四.
 */
...
export function ProgressOverview() {
  return (
    <ComponentPage
      ...
      whenToUse={[
        <>工作方式、双窗口提交规则见 <code>packages/ui/docs/00-总纲.md</code>。</>,
        <>
      ...
      bodyTitle={null}
    >
      <ExampleGroup title="现状看板" subtitle="数字为业务文件/出现次数实测值；扫描日期以各行标注为准（未标注 = 2026-07-20 口径）。">
      <CompareTable
        ...
        rows={[
          ...
          [
            '色彩 Colors',
            '🟨 进行中',
            '裸 hex 1126 处 / 327 值（2026-07-30 扫描；原 2469 处，四轮 codemod 已折叠 1548 处中性灰系）',
            '两层 token 已落地并统一色温（264°）；剩：近似值 ≈160 处待拍板、SVG stroke 批次、LibreChat 语义 token 重指向、platform 待接入 tokens.css',
          ],
          [
            '多端适配 Responsive',
            '🟨 v1 已建',
          ...
          ['Select / 下拉', '⬜', '多个', '待定（下一个优先）'],
          ['Dropdown 菜单', '⬜', '2（Dropdown / DropdownMenu）', '待定'],
          ['Input 输入框', '⬜', '待扫描', '待定'],
          ['Tabs 标签页', '⬜', '待扫描', '待定'],
        ]}
      />
      ...
      <ExampleGroup title="规范索引" subtitle="各规范页的成熟度与源文档；已定稿的直接照用，未定稿的以页内说明为准。">
        <CompareTable
          head={['规范页', '内容', '成熟度', '源文档（packages/ui/docs/）']}
          rows={[
            ['字体 Typography', '系统字体栈 + 十档 semantic 字号 + 双字重', '✅ 已定稿落地', '基础-字体规范.mdx'],
            ['色彩 Colors', '两层 token：primitive 灰阶（264° 统一色温）→ semantic（文字/填充/边框/语义色）', '✅ v1 已定稿落地', '基础-色彩规范.mdx'],
            ...
            ['二次确认弹窗', 'useConfirm() 服务 + 壳与按钮两档标准', '✅ 已定稿', '组件-Confirm二次确认.md'],
            ['Tooltip / Popover', '气泡层姊妹篇：深底白字提示 + 气泡卡片（2026-07-30 建档）', '✅ v1 已定稿', '组件-Tooltip文字提示.md / 组件-Popover气泡卡片.md'],
