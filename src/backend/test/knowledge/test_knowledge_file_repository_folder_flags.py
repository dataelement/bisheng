...
async def test_file_change_hidden_ids_are_excluded_in_sql():
    """909 only: F046-hidden files are filtered out inside the aggregate itself."""
    result = SimpleNamespace(all=lambda: [(10, 0, 0)])
    session = SimpleNamespace(execute=AsyncMock(return_value=result))
    repository = KnowledgeFileRepositoryImpl(session)

    await repository.find_folder_descendant_status_flags(7, {10: "/10"}, excluded_file_ids={501, 502})

    sql = _compile_sql(session.execute.await_args.args[0])
    assert "knowledgefile.id not in (501, 502)" in sql


async def test_hidden_id_exclusion_is_chunked_for_dm8():
    """DM8/Oracle cap an expression list at 1000 entries, so it is split and AND-ed."""
    result = SimpleNamespace(all=lambda: [])
    session = SimpleNamespace(execute=AsyncMock(return_value=result))
    repository = KnowledgeFileRepositoryImpl(session)

    await repository.find_folder_descendant_status_flags(7, {10: "/10"}, excluded_file_ids=set(range(1200)))

    sql = _compile_sql(session.execute.await_args.args[0])
    assert sql.count("not in (") == 3
