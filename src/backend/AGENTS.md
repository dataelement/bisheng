...
uv run celery -A bisheng.worker.main worker -l info -c 100 -P threads -Q celery -n celery@%h
# Points awards use the default queue: `sh entrypoint.sh default`.
# Compose `entrypoint.sh worker` already includes the default Worker.
...
| `celery` (default) | 100 threads | Telemetry stats, points async award/rank/monthly/reconcile, org_sync, permission, etc. |
