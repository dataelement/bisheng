...
> 调研来源（不进展示层）：antd Segmented（定位为单选控件；尺寸 24/32/40 与 BISHENG 控件阶梯同构；block 模式）、Apple HIG（iPhone 上 ≤5 段、各段内容长度接近、不混用文本与图标段）、Material 3（segmented buttons 2～5 段）。「白浮块 + 灰底槽」为 iOS / antd 主流画法。设计师拍板（2026-08-26）：选中态用白浮块 + 灰底槽（不用品牌色）；三档 24/32/40；与 Tabs 分写两份文档。
...
- **槽内边距 2px**，浮块圆角 = 外圆角 − 2px（嵌套同心，见《圆角与阴影规范》使用原则）。
- **各段等宽**，取最长一段的宽度——段宽参差会让浮块滑动时忽胖忽瘦；`block` 模式下撑满父容器、各段平分。

## 3. 尺寸 Size

三档，medium 是默认。高度走控件阶梯（同 [组件-Button按钮.md](组件-Button按钮.md) §3、[组件-Input输入框.md](组件-Input输入框.md) §3），同排控件天然对齐。

| size | 高度 | 字号 / 行高 | 外圆角 | 浮块圆角 | 段水平内边距 | 什么时候用 |
|---|---|---|---|---|---|---|
| `small` | 24px | 14 / 22 | 4px | 2px | 8px | 表格工具条、卡片角落 |
| `medium`（**默认**） | 32px | 14 / 22 | 6px | 4px | 12px | 绝大多数场景 |
| `large` | 40px | 16 / 24 | 8px | 6px | 16px | 页头、大表单 |

- 字号跟《字体规范》、外圆角跟《圆角与阴影规范》控件档，随档取值不单独定义。
- 和旁边的按钮、输入框用同一档：32px 的搜索框旁边配 32px 的分段控制器。
...
- 字重全档 400，选中态靠浮块区分、不加粗——同按钮口径，避免文字宽度跳动。
...
1. 组件位置建议 `packages/ui/src/components/Segmented/`，cva `variants: { size }`（高 24 / 32 / 40，外圆角 4 / 6 / 8，槽 padding 2px，浮块圆角 2 / 4 / 6，段水平 padding 8 / 12 / 16）。浮块单独一个绝对定位元素测量滑动（200ms ease），别靠每段自己换底色——那样没有滑动感。
2. 无障碍走 `role="radiogroup"` + 每段 `role="radio"` / `aria-checked`（它语义上是单选，不是 tabs）；方向键在段间移动并即时选中。
3. 颜色全走 token：槽底 `bg-fill-2`、浮块白底、选中 `text-text-1`、未选中 `text-text-2`、disabled 复用按钮 disabled 文字 token；无裸 hex。
...
| 日期 | 改了什么 | 提交 |
|---|---|---|
| 2026-08-26 | 建档 v1：定位为即时生效的单选控件，与 Tabs / Radio 的判别表归本文 §1；白浮块 + 灰底槽（fill-2）、浮块不加投影、选中不用品牌色；尺寸三档 24/32/40 对齐控件阶梯、槽内边距 2px 嵌套同心；内容三形态不混用、段数 2～5、纯 icon 必配 Tooltip；状态六态；移动端 block 撑满 + ≥44px 热区。与 [组件-Tabs标签页.md](组件-Tabs标签页.md) 同日建档 | 待 committer 窗口提交 |
