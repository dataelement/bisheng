...
It also ensures and verifies service-account resource-state markers from
CURRENT SQL modes, including when the model is already current. This repairs
older resources without changing Grants or inheritance/custom boundaries.
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--apply",
        action="store_true",
        help="Ensure service-account resource markers and publish/reuse the model and CURRENT Catalog pin",
    )
    parser.add_argument(
        "--confirm-store-id",
        default=None,
...
async def execute(args: argparse.Namespace, *, live_settings: Any = settings) -> int:
    ...
    try:
        ...
        resource_markers = await load_service_account_resource_markers()
        report = PublishReport(
            ...
            service_account_marker_tuple_count=len(resource_markers),
        )
        ...
        await ensure_service_account_resource_markers(target_client, resource_markers)
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (AuthorizationModelPublishBlockedError, VisibleReconcileBlockedError) as exc:
