...
> 调研来源（不进展示层）：antd Tag（filled / solid / outlined 三款、5 状态色 + 11 预设色、CheckableTag；单一高度 22px）、Arco Tag（20/24/28/32 四档、字重 500、浅底默认 + bordered）、TDesign Tag（20/24/32 三档、light / dark / outline / light-outline 四款、square / round / mark 三形、maxWidth 省略号）。设计师拍板（2026-08-28）：**配色只留浅底深字**，描边 / 实底 / 灰底款不做；尺寸**两档 20 / 24**；收展示型、可关闭、可选中、带 icon / 头像四类。
...
- 列表里每行都要标状态、要一眼扫过去，用状态点，判别表见 [组件-Badge徽标.md](组件-Badge徽标.md) §1。
- 说「有多少 / 有没有新的」，用徽标，不用标签。
...
- **关闭按钮**：× 图标 12px（`Outlined.Close`），与文字间距 4px，右内边距收到 4px；默认 text-3，悬停 text-1；热区是标签全高。
- icon 与头像二选一，别同时挂。
...
1. 组件位置建议 `packages/ui/src/components/Tag/`，cva `variants: { size: small | medium, color: default | brand | success | warning | danger }`，行为用 props：`closable` / `onClose`、`checkable` / `checked` / `onChange`、`icon` / `avatar`、`maxWidth`、`disabled`。可选中标签渲染为 `<button aria-pressed>`，可关闭的 × 是独立 `<button aria-label="移除 {label}">`，展示型渲染为 `<span>`。
2. 颜色全走 token：default `bg-fill-2 text-text-2`；brand `bg-brand-50 text-brand-500`（随蓝⇄绿主题，深色由品牌深色阶自动翻转）；success / warning / danger 用 `--success-tint` / `--success` 等功能色四态 token（深色 tint 已是深而饱和的色底，无需覆盖）；固定例外「审批中蓝」「技能紫」按《色彩规范》§4 的 TAG 色对写成独立 token（`--tag-approving-*`、`--tag-skill-*`），不写裸 hex。选中态 `bg-primary/[0.07]`、选中悬停 `bg-primary/10`；禁用 `bg-fill-1 text-text-4`。
3. 圆角 `rounded-sm`（4px）；带 `avatar` 时切 `rounded-full`。高度 `h-5` / `h-6`，字号 `text-caption`（12/20），`font-normal`；padding `px-1.5` / `px-2`，带头像或 × 的一侧收到 `pl-1` / `pr-1`。
...
6. 迁移映射（本次只读扫描，2026-08-28，范围 `client/src`，排除 node_modules）：
   ...
   - `components/ui/Tag.tsx`（绿描边 chip，`rounded-3xl border-2 border-green-600`）：0 处引用，直接删除。
...
7. 站点接线（元规范 §5）：`rspress.config.ts` 侧栏注册本文 + `components/tag.mdx` demo 页（ASCII 文件名，front matter `component: Tag` 在组件进库后再写）；00-总纲 §四 与 01-设计规范 §0 索引行本次已随建档更新。
...
- 标签组的横向间距 8px 为本次建档取值（Arco 8px、antd 8px），待验收。
