...
沿用 BiSheng OpenAPI（`/api/v2/*`）的统一模式：**每个请求必须携带服务账号密钥**。

- 请求头 `Authorization: Bearer <密钥>`。无密钥一律拒绝，没有任何兼容开关或回落路径。
- 不接收用户 JWT，也不读取 `access_token_cookie`。
- 该密钥能检索到什么，取决于管理员给这个服务账号授予了哪些知识库。这就是这条集成的数据边界。

> 部署前置条件：在管理界面创建服务账号、按最小必要授予它需要的知识库、再为它签发带 `knowledge:read` 权限位的密钥。

### 关于 `default_operator`

这个配置项**不再用于 `/api/v2/*`**。它现在只服务一件事：工作流与助手的免登录分享链接。访客没有账号，平台以它配置的那个用户的身份执行访客的请求。

因此**它的权限范围就是访客能触达的范围**。早期文档曾建议把它配成超级管理员「以避免权限边界问题」，那条建议已作废：照做等于让任何拿到分享链接的人都拥有超级管理员的视野。请配置一个只被授予了必要资源的普通账号。

不想对外开放免登录链接时，把同一配置块下的 `enable_guest_access` 置为 `false`，全平台的免登录链接立即不可访问。
