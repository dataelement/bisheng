...
from bisheng.api.v1.schemas import UploadFileResponse, resp_200
from bisheng.common.dependencies.user_deps import UserPayload
from bisheng.common.errcode.server import (
    SystemConfigEmptyError,
    SystemConfigInvalidError,
...
# Static list of file extensions the platform advertises as uploadable (returned
# by GET /env, consumed by frontend accept lists). OFD is supported by default
# (F032): converted to PDF at parse time, no config gate.
UNS_SUPPORT_FORMATS = [
    "doc",
    "docx",
    "ppt",
...
def get_env():
    ...
    uns_support = list(UNS_SUPPORT_FORMATS)
    ...
    env["knowledges"] = {
        "version_management": {
            ...
            "simhash_similarity_threshold": (vm.simhash_similarity_threshold if vm else 0.85),
        }
    }
...
def save_config(data: dict, admin_user: UserPayload = Depends(UserPayload.get_admin_user)):
    ...
    try:
        ...
        if isinstance(config, dict) and "linsight_invitation_code" in config.keys():
            if config["linsight_invitation_code"] is not None and bool(config["linsight_invitation_code"]) not in [
                True,
                False,
            ]:
...
async def update_web_config(
    request: Request, admin_user: UserPayload = Depends(UserPayload.get_admin_user), value: str = Body(embed=True)
):
...
async def _upload_file(
    file: UploadFile, object_name_prefix: str, file_supports: list[str] = None, bucket_name: str = None
) -> UploadFileResponse:
    if file.size == 0:
...
async def upload_icon(
    ...
    try:
        ...
        resp = await _upload_file(
            file, object_name_prefix="icon", file_supports=["jpeg", "jpg", "png"], bucket_name=bucket
        )
...
async def upload_icon_workflow(
    request: Request,
