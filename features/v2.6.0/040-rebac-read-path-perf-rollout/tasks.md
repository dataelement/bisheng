...
| T8b | **C 工作台列表无限滚动** — **工作台部分 N/A**：最近/常用回退 offset 契约（D6），`AgentGrid`/`getFrequently` 不变；文件搜索部分阻塞于 T6c | **Client** | `src/frontend/client/`：文件搜索组件（待 T6c） | AC-12, AC-13 | T6b, T6c | 🔲(文件搜索待 T6c) |
