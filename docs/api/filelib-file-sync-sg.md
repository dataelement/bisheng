...
| 扩展名 | 说明 |
|---|---|
| `png` / `jpg` / `jpeg` / `bmp` | 仅当服务端已配置图像解析能力时可用（与平台 `enable_etl4lm` 一致，即 `loader_provider` 对应的外部解析服务 URL 已配置）。未启用时同步这些格式将返回 `400`。 |

> **说明**
>
> - 格式校验在入库前执行；不支持 `.zip`、`.rar`、无扩展名文件等。
