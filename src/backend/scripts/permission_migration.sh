...
if [ "$run_mode" = "execute" ]; then
    ...
    python bisheng/script/permission_rbac_to_rebac_migration.py --step "${step}" "${extra_args[@]}"
elif [ "$run_mode" = "dry_run" ]; then
    echo "Previewing F006 RBAC to ReBAC permission migration..."
    python bisheng/script/permission_rbac_to_rebac_migration.py --dry-run --step "${step}" "${extra_args[@]}"
elif [ "$run_mode" = "only_step" ]; then
    echo "Running only F006 migration step ${step}..."
    python bisheng/script/permission_rbac_to_rebac_migration.py --only-step "${step}" "${extra_args[@]}"
elif [ "$run_mode" = "dry_run_only_step" ]; then
    echo "Previewing only F006 migration step ${step}..."
    python bisheng/script/permission_rbac_to_rebac_migration.py --dry-run --only-step "${step}" "${extra_args[@]}"
elif [ "$run_mode" = "verify" ]; then
    echo "Verifying F006 RBAC to ReBAC permission migration..."
    python bisheng/script/permission_rbac_to_rebac_migration.py --verify --step "${step}" "${extra_args[@]}"
elif [ "$run_mode" = "replay" ]; then
    echo "Replaying F006 RBAC to ReBAC permission migration from scratch..."
    python bisheng/script/permission_rbac_to_rebac_migration.py --force --step "${step}" "${extra_args[@]}"
