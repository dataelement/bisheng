...
def patches(monkeypatch):
    ...
    monkeypatch.setattr(m, "_uses_f048_department_projection", lambda: True)
    monkeypatch.setattr(
        m.DepartmentDao,
        "aget_by_dept_id",
        AsyncMock(return_value=None),
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(return_value=SimpleNamespace(tenant_id=1, is_active=1)),
    )
    monkeypatch.setattr(
...
class TestCrossSourceReuseAndMigrate:
    ...
    async def test_cross_source_reuse_syncs_user_attrs(self, patches):
        ...
        local_user = _user(
            user_id=7, source="local", external_id="u1", user_name="Old", email="old@x.com", phone_number=None
        )
    ...
    async def test_new_sso_user_created_when_truly_missing(self, patches):
        ...
        created = _user(user_id=11, source="sso", external_id="u2", user_name="Bob", email="bob@x.com")
        patches.UserDao.aget_by_source_external_id = AsyncMock(return_value=None)
