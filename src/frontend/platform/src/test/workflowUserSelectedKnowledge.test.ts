...
} from "@/pages/BuildPage/flow/FlowChat/userSelectedKnowledge";

describe("workflow user-selected knowledge helpers", () => {
  it("detects workflows that contain user-selected knowledge nodes", () => {
    expect(
      hasUserSelectedKnowledgeNode({
        nodes: [
          { data: { type: "input" } },
          { data: { type: "user_selected_knowledge_rag" } },
        ],
      }),
    ).toBe(true);
  ...
  it("shows the runtime picker for dialog and form input states", () => {
    ...
    expect(
      shouldRenderRuntimeKnowledgePicker({
        requiresRuntimeKnowledge: false,
        inputDisabled: false,
      }),
    ).toBe(false);
    expect(
